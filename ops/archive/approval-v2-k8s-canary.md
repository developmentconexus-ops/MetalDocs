> **Superseded by Compose v1 deploy — 2026-07-05; retained for the Approval-v2 canary procedure reference; not the current deploy path.**
> The current v1 deploy target is Docker Compose. See [`ops/DEPLOY.md`](../DEPLOY.md).
> This document is re-homed verbatim from the original `ops/DEPLOY.md` (which assumed a Kubernetes
> deployment that does not match the actual v1 stack) for historical/reference purposes only.
> Any live `kubectl` commands below are **not** runnable against the current deployment.
