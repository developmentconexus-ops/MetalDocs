...
| documents `created_by_display_name_snapshot` value preserved (single-tenant) — live create → row read-back | **yes (resolved by fix-feature `f4.1a-documents-testdb-rehab`)** | `TestCreateDocumentTx_PopulatesAllSnapshotColumns` now runs under the **real** `iampg.NewUserDisplayNameRepository(db)` (not Noop) and asserts `created_by_display_name_snapshot == "Snapshot Author"` — the value the port resolved from `metaldocs.iam_users` for the seeded actor. **PASS** live (`-tags integration -count=1 -p 2 ./internal/modules/documents/application/` → `ok … 161.952s`). The pre-existing testdb harness root cause (wrong search_path resolving bare `documents` to the dead legacy `metaldocs.documents`; missing app-level system_admin seed; `SetMaxOpenConns(1)` deadlock vs the off-tx port read) is fixed in `f4.1a` — see that feature's evidence. |
