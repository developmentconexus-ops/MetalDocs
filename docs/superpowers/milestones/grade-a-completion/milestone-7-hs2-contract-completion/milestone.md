...
| **F7.4** `documents-fillin-view-placeholder-typed` | Documents **has** a codegen pipeline (`internal/modules/documents/api/api.gen.go`). Contract-first: declare OpenAPI 200 body schemas for the 4 currently-undeclared routes (`getFillInSchema`, `putPlaceholderValue`, `getPlaceholderOptions`, `viewDocument` — only `viewDocument` has an operationId today), run `GOFLAGS=-mod=mod go generate ./internal/modules/documents/api/...`, then emit the generated types at the 4 response sites: `fillin_handler.go:58-62` and `:116-119`; `placeholder_options_handler.go:67,74`; `view_handler.go:46-51`. Note `view_handler.go:46` is `payload := map[string]any{"pdf_status":...}` built then written via `writeFillInJSON` — the multi-line/alias pattern Grep A is blind to. **HS-6 scope extension (2026-06-20, operator-approved):** the full-repo Part-B scan surfaced a **5th** response-literal site overlooked in the original enumeration — `pdf_webhook_handler.go:113` (`writeFillInJSON(... map[string]any{...})`), the missing 10th H-D site. That route is **unwired** (`RegisterRoutes` uncalled, H-1e) and **deliberately off the OpenAPI spec** (Phase C wont-fix, `:120-121`), so contract-first codegen is inappropriate; per the operator's "convert it" decision it gets a **hand-rolled typed struct** (ADR 0012 posture, like F7.2/F7.3) + swap, **no OpenAPI change**. This keeps the F7.5 allowlist purely non-response. | OpenAPI declares 200 body schemas for all 4 codegen ops. Generated `documents/api/api.gen.go` types match. Handlers emit the generated types. `pdf_webhook_handler.go:113` emits a hand-rolled typed struct. Handler tests assert wire JSON keys equal the OpenAPI declaration (4 codegen routes) / unchanged from baseline (pdf_webhook). 0 response-literal `map[string]any` (incl. via `writeFillInJSON`) in the 4 files. BE codegen fresh (no uncommitted diff after regen). Build + tests green. | 5 H-D sites (4 codegen + 1 off-spec hand-rolled; incl. the Grep-A-blind alias sites) |
