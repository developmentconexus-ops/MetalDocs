...
| F1.2 | `f1.2-status-and-body-conformance` | **P2-light scope expansion (2026-06-15, operator-approved):** the original A2/A4/A5 sites plus the `toVersionResponse`-emitting cluster — typing the whole `TemplateVersion` shape at the consumer-contract boundary in one coherent feature. Sites: (1) `renameDocument` → 200 no-content — `documents/delivery/http/handler.go:519` (A2, raw-domain drop). (2) `createNextVersion` → **keep 201** (genuine create) + flat typed `TemplateVersion` body — `templates/delivery/http/routes_create.go:36` (A4, H-D; HS-6 deviation from mission §5 "201→200" — operator pre-approved, see `f1.2-…/spec.md` interview Q2). (3) `presignTemplateAutosave` → 200 + flat typed `TemplatePresignAutosaveResponse` (no `{data:{...}}` envelope) — `templates/delivery/http/routes_autosave.go:42` (A5, H-D). (4) **`commitTemplateAutosave`** → 200 + flat typed `TemplateVersion` body — `templates/delivery/http/routes_autosave.go:90` (envelope/`map[string]any` sunset). (5) **`getTemplateVersion`** → 200 + flat typed `TemplateVersion` body — `templates/delivery/http/routes_query.go:160` (envelope/`map[string]any` sunset). Authors new OpenAPI component schemas `TemplateVersion` + `TemplatePresignAutosaveResponse`. Same-commit FE adapter updates in `frontend/apps/web/src/features/templates/api/templates.ts` (`presignAutosave`, `commitAutosave`, `getVersion`). Legacy mapper `toVersionResponse` survives for `createTemplate` (F1.3 turf). | Handler tests assert status code + body shape equal the OpenAPI declaration for all five routes; H-D grep on the five sites returns 0; the new `TemplateVersion` / `TemplatePresignAutosaveResponse` schemas exist in `api/openapi/v1/openapi.yaml`; FE adapters return the flat shape (no `body.data.…` indirection on the three changed sites); FE build + typecheck + tests green. |
| F1.3 | `f1.3-declared-fields-only` | `createTemplate` returns only the declared schema (drop undeclared `id` / `version_id`) — `templates/delivery/http/routes_generated.go:64` (A3, H-D). May also retire the legacy `toVersionResponse` mapper if `createTemplate` is the last call site by F1.3 close. | Response is a strict subset of the OpenAPI schema (no extra keys); FE codegen consumers get the declared shape; H-D grep on the site returns 0. |
| F1.4 | `f1.4-typed-responses-class` | **Shrunk (2026-06-15) — F1.2 P2-light absorbed `commitTemplateAutosave` + `getTemplateVersion` from the templates-module envelope sunset.** F1.4 now owns: documents module raw-domain / `map[string]any` sweep at `documents/delivery/http/handler.go:816(+5)` (A6) and `documents/delivery/http/handler.go:317` (raw stats, A8); taxonomy module at `taxonomy/delivery/http/routes_profiles.go:67,111,126,169` (A7, H-D); `Allow` header on `audit/delivery/http/handler.go:81` 405 (A8). Regenerate FE codegen last. **Out of F1.4 scope (F1.2 already shipped them):** `commitTemplateAutosave`, `getTemplateVersion`, the two new templates schemas. | **H-D grep (mission report §6 commands) returns 0**; FE codegen regen is clean; no public route emits raw `domain.*` or `map[string]any` (post-F1.2 + F1.3 + F1.4); the audit 405 response includes a correct `Allow` header. |
