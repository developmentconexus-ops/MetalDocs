...
| F2d.5b | `f5b-pdf-read-canvas` | **PDF-driven viewing (operator intent ratified 2026-07-09):** ALL viewing is PDF; docx/TipTap is only for edit (author modes) + review (suggesting). Backend contract (OpenAPI): serve the frozen-revision PDF rendition to in-approval viewers — today `/documents/:id/view` is gated to `approved/scheduled/published` and serves `final_pdf_s3_key` only (`view_service.go:43`); the render pipeline already materializes the rendition at freeze (ADR 0015 Pin/Materialize → docx-renderer → PDF outbox). FE: `PdfCanvas` (presigned-URL embed + pdf-status polling per `useDocumentPdfStatus` pattern) replaces the docx read canvas in `approving`/`observing`/`author-waiting`/`lifecycle`; the TipTap editor chunk becomes **genuinely** lazy (relocated from F2d.5g). Integrity win: the approver signs over the PDF rendition = the exact published-artifact bytes (Veeva viewable-rendition pattern; industry evidence 2026-07-09 adversarial review). Consumer: approvers/observers (bundle + integrity), auditors. | Real-DB test: in-approval viewer receives a PDF URL (authz-scoped, tenant-safe); approved/scheduled/published behavior unchanged. FE per-mode test: read modes render `PdfCanvas`; pending/failed rendition states covered. **Real** lazy-load assertion: editor chunk absent from the read modes' static import graph AND not fetched on initial read-mode render. |
...
- **F2d.5 addendum (amended 2026-07-09):** approver/observer modes must not pay the editor bundle —
  goal intact but relocated to **F2d.5b**: real saving requires the PDF read canvas (DocumentShell's
  static TipTap import makes in-F2d.5 lazy a no-op).
