...
| F2d.6 | `f6-author-comment-replies` | **DONE 2026-07-09** (base `ce005dc3`; commits `33e4eb3d` commentText extractor, `53610f3f` AuthorCommentsPanel, `6239a9cf` workspace wiring, `88ec9188` reopen-assertion nit; evidence `f6-author-comment-replies/evidence.md`; independent whole-feature review clean; tsc 0, documents 274/274, zero-backend 7-file gate; editor-ui import type-only; author never edits content). Real-DB authz test = bounded defer (asserts already-covered backend behavior; author holds `CapDocumentView` on own doc, targeted-verified `handler.go:1083-1206` at the gate; zero backend diff). Surface the author's reply-to / resolve on instance comments in `author-waiting` (never edits content) — brief §9.1. **No authz work:** comment writes (`createComment`/`updateComment`→resolve, `handler.go:1083,1116`) gate only on `CapDocumentView` tenant-grade (`handler.go:1201-1206`); the author already holds it on their own document. This is FE-only surfacing of an existing capability — HS-2 not in play. | Real-DB authz test (confirming existing behavior): author can reply/resolve on own document under review; a non-`CapDocumentView` actor cannot. UI test: reply composer visible in `author-waiting`, content editing not. |
| F2d.7 | `f7-cockpit-retirement` | **DONE 2026-07-09** (base `a22d2289` → HEAD `645d7199`; evidence `f7-cockpit-retirement/evidence.md`; ADR 0080 closure note; gates: all deletion greps EMPTY, tsc 0, documents+approval 399/399, zero-backend). Redirects were ALREADY built in F2d.5 S3 — F2d.7 KEEPS them and deletes the orphaned page. Deleted: `ApprovalCockpitPage`+test, wholly-cockpit-only `useDocumentApprovalArtifact`+test (whole hook, not a branch), dead `DocumentEditorRoutePage`+test, cascade-orphaned `ApprovalSidebar`/`StageContextHeader`/`SuggestionList` (+css) — **zero dead code left**. Retargeted 4 live `/edit` constructors → `/documents/:id`. The pre-existing `?decision=reject` cockpit fail is RESOLVED by deletion. `CancelInstanceDialog` KEPT (bounded defer — deferred S2b "Cancelar instância" wiring, not superseded). Whole-feature review 0🔴 5🟡 all disposed. Original spec text below. `/approvals/:documentId` becomes a redirect to `/documents/:id` (forwarding `location.search` — `?decision=` preserved, see F2d.5f); `ApprovalCockpitPage`, the hollowed-shell composition (`screenModel` override), and `useDocumentApprovalArtifact`'s cockpit-only reduced model are deleted; worklist deep links target `/documents/:id`. Mechanical cleanup: remaining `/documents/${id}/edit` literal constructors (`DocumentDetailRoute.tsx:101,145`, `NewDocumentWizardPage.tsx:179`, `documentWorkflow.ts:30`) retargeted to `/documents/${id}` (dead paths, not left to bounce through redirects). ADR: single artifact destination. Consumer: worklist navigation + bookmarks. | Route test: old URL redirects (params + query preserved). Grep gates: no `ApprovalCockpitPage` references; no `/edit` path constructors outside the redirect route itself. Worklist link test targets `/documents/:id`. Build clean after deletion. |
