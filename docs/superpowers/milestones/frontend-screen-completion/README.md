...
> **Status:** In progress — M0 + M1 + M2 passed (validator PASS + operator HS-1); **M3 validator PASS 2026-06-22 — awaiting operator HS-1**; M4 next
...
| 3 | `milestone-3-notifications` | Notifications center real end-to-end (new backend + lifecycle emitters + wired screen) | validator PASS — awaiting operator HS-1 — F3.1–F3.4 all closed (F3.3 ADR-0044 domain events; F3.4 top-bar bell menu + additive `read-all`) | [PASS](milestone-3-notifications/qa/milestone-qa.md) |
