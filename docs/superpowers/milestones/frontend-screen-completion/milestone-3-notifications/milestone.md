...
| F3.4 | `f3.4-notifications-wire` | **RE-SHAPED 2026-06-22 (operator redirect): a top-bar notification *menu*, not a page.** Build to grade-A (CSS-Module components + **generated-type consumption**, no legacy `NotificationsPanel`/`catalog-*`, no snake→camel mapper): a **bell + unread badge** in `AppToolbar` → click opens a **popover** (latest ~8 notifications, per-row mark-as-read) → **"Mostrar Todas"** opens a **spotlight** (`components/ui/Dialog` overlay) with the full scrollable list + **"Marcar todas como lidas"**. Real TanStack Query hooks (`useNotificationsQuery`, `useUnreadCountQuery`, `useMarkNotificationReadMutation`, `useMarkAllNotificationsReadMutation`) against the F3.1 endpoints + a small **additive `POST /notifications/read-all`** bulk endpoint (grade-A mark-all; contract-first regen, `api-lint`=0, integration-tested — the only backend touch). Delete the noop `notifications.ts` `{ items: [] }`/`never[]` stubs (list + mark-read); **remove the standalone `NotificationsPage` + `/notifications` route** and update the M0 route tracker; **retire** legacy `NotificationsPanel`/`NotificationItem` (if unused). SSE rebuild out of scope (leave `subscribeOperationsStream`/`listAuditEvents` as-is). | `grep -nE "items: \[\]\|never\[\]\|Stubbed pending" notifications/api/notifications.ts` = **0** for list/mark-read (noop deleted); the bell badge reflects real unread-count, the popover lists real rows, per-row mark-read flips to READ, the spotlight lists all + mark-all-read flips every unread; `/notifications` route + `NotificationsPage` removed (tracker updated, no dead stub); consumer shapes == generated types (no mapper); the `read-all` endpoint passes `api-lint -strict`=0 + all 6 CI guards + integration test; query-hook + component tests pass against fixtured responses; **`frontend-screen-reviewer` + `frontend-code-reviewer` APPROVE on record** (D2; design-source N/A — reviewed vs tokens/primitives + the operator-specified interaction); FE tests green; `npm run build` clean. |
