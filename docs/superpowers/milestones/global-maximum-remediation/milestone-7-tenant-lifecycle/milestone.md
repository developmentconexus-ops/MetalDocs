...
| F7.4 | `f7.4-rls-truth-sweep` | Flip CI/dev DB role to **NOSUPERUSER + NOBYPASSRLS + non-owner** (reassign table ownership away from the app role — owner-bypass trap). Run the existing integration suite as the census; fix every surfaced-RED sole-RLS query with an explicit `tenant_id` predicate; add a lint blocking new sole-RLS tenant-scoped reads/writes; close `approval_signoffs` (add FORCE RLS + `tenant_isolation` policy on `actor_tenant_id`, or ADR-recorded lint-guarded exception). | CI role is NOSUPERUSER+NOBYPASSRLS+non-owner; the full integration suite runs **green under it** (every prior false-green isolation test now genuinely exercises RLS). **Negative+positive proof:** a sole-RLS query **leaks under the wrong tenant / is blocked under the right one**, for real, under the non-bypassing role. Every flip-surfaced query carries an explicit `tenant_id` predicate; the lint fails a synthetic new sole-RLS query. `approval_signoffs` gap closed. Prod-parity: CI role matches the prod deployment constraint. |
