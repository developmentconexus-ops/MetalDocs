...
|-------|-------------|-------------------------|
| ASYNC-TENANT-SEED lint **port blind spot** | Lint can't see writes behind a cross-module port (documented function-local scan). The §4.2 human backstop that caught this surfacer is now satisfied; hardening the lint is a real follow-up out of F6.4 boundary | Trigger: lint-hardening / next tenancy sweep; owner: mission tracker |
