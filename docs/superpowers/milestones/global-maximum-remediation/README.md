...
> **Status:** M0 + M1 + M2 passed (operator-approved HS-1, 2026-07-03). M3 milestone-validator **PASS** (2026-07-03) — negative-RLS proof **run GREEN for real** — **HS-1 operator gate pending**. M4 milestone-validator **PASS** (2026-07-04, after HS-4 fix F4.4); F4.2 live-run defer subsequently **CLOSED** by fix-feature F4.5 (authz soft-GUC NULL hardening) — publish-race now **live-green on real Postgres**, `task_e03a4383` resolved, re-validation pending. F4.3 HS-7 ADR-split **ratified by operator ("HS-7 okay")**. **HS-1 operator gate pending.** Commits local, not pushed.
...
| 4 | `milestone-4-versioning-kernel` | Unified 9-status state machine; publish race proven safe + **live-green** (F4.5 closed the defer); concurrency idiom decided (ADR-split, operator-ratified) | validator PASS — re-validation after F4.5 — HS-1 pending (2026-07-04) | [PASS](milestone-4-versioning-kernel/qa/milestone-qa.md) |
