...
6. **The stuck-instance-watchdog's `pg_try_advisory_lock` is removed** (River's elector +
   single-dequeue subsume it), proven by a two-client singleton test. This is an *unrelated*
   single-runner lock — **H-PRE-1 stays LIVE** (it governs the audit-writer's `pg_advisory_xact_lock`,
   untouched by M5), per ADR 0067 §H-PRE-1 erratum.
...
| F5.2 | `f5.2-janitors-on-river` | The 3 surviving janitors become **River periodic jobs** (leader-elected, same intervals, unchanged bodies) hosted in `metaldocs-jobs`; the custom lease scheduler (`internal/modules/jobs/scheduler/`), `job_leases` table + 3 lease SQL functions, and **lease-reaper** are deleted; the stuck-instance-watchdog `pg_try_advisory_lock` is removed (an *unrelated* single-runner lock — **not** an H-PRE-1 retirement; H-PRE-1 stays LIVE per ADR 0067 §H-PRE-1 erratum). **Consumer:** the `metaldocs-jobs` River client registers each as a `NewPeriodicJob`; ops observe the same maintenance effects. | Per-janitor integration proof (testdb): each preserves its behavior (watchdog cancels a seeded 7-day-stuck instance; idempotency-janitor deletes expired keys; audit-validator flags a tampered chain). **Singleton proof:** two River clients on one DB → watchdog periodic job executes **exactly once/tick** (elector, advisory lock removed). `grep` census of `job_leases`/`acquire_lease`/`scheduler.go`/`lease_reaper`/`pg_try_advisory_lock` = **0** (or allowlisted w/ reason). `metaldocs-api` builds + starts with no scheduler. Watchdog-lock removal recorded with the singleton evidence (H-PRE-1 stays LIVE — ADR 0067 §H-PRE-1 erratum). All matches `validation-contract.md` §2 per-job table. |
...
| F5.6 | `f5.6-fanout-queue-wiring-fix` | **Unplanned — HS-6 scope surface, discovered by M5's own live QA drive (item 6 below), operator-directed fix-in-place 2026-07-04.** `RiverLifecycleEventEnqueuer` passed `nil` `InsertOpts`, landing lifecycle-fanout jobs on River's implicit `"default"` queue, which `metaldocs-jobs` never subscribes — every `notification_fanout` job (publish/supersede/obsolete/approve/reject) sat permanently unconsumed. Fix: set `Queue: "temporal"` explicitly, matching the convention every other M5-touched enqueue site already follows. Follow-up (same feature): `fanoutToReaders` rewritten as a single set-based `INSERT...SELECT` (fixed a `driver: bad connection` open-cursor-exec bug, global-max over per-recipient round-trips). **Consumer:** `NotificationsFanoutWorker` (already correct; never ran). | Unit test asserting `InsertOpts.Queue == "temporal"` passed to `InsertTx`. Live re-drive: direct publish on a company-scope CD → obligated reader's `/api/v1/notifications` shows the row. No queue-subscription or worker-logic change. |
| F5.7 | `f5.7-close-proof-gaps` | **Unplanned — HS-4, milestone-validator FAIL 2026-07-04.** The validator executed F5.2/F5.3's deferred integration proofs (DB reachable) and found 6/12 red on first real run: all 4 `dispatchjobs` proofs (`river_job` absent — testdb template never provisions the River schema), watchdog `AutoCancelEquivalence` + audit `DetectsTamperedChain` (fixture literals violate real CHECK constraints). Fixes: (T1) provision River schema in the testdb template (global-max — unblocks the whole River-job test class), (T3) correct the audit tamper fixture literal without weakening assertions, (T4) align F5.1's lagging "H-PRE-1 retired" wording to LIVE (ADR 0067 §H-PRE-1 erratum). **T2 (watchdog) SUPERSEDED by F5.8** — the audit proved it a genuine production defect, not a fixture bug; the HS-2/HS-6 "surface, don't fold" clause fired. **No production consolidation code changes.** **Consumer:** the named F5.2/F5.3 acceptance proofs + the re-dispatched validator. | The 4 dispatch proofs and audit P3 integration test **PASS** green from a River-provisioned testdb; already-green proofs (F5.2 P4/P2, F5.4, F5.5) still pass; `go build`/`go vet` clean; no doc says H-PRE-1 retired; re-dispatched milestone-validator returns **PASS**. |
| F5.8 | `f5.8-watchdog-alert-only` | **Unplanned — HS-2/HS-6, genuine production defect surfaced by F5.7's proof run, root-cause-audited + operator-directed 2026-07-04.** Three read-only investigators proved the watchdog's `auto_cancel` branch is the orphaned remnant of an abandoned stuck-timeout-action concept that lost a semantic collision on `on_eligibility_drift` (schema always allowed only `{reduce_quorum, fail_stage, keep_snapshot}`; frontend corrected in `14e48071`, Go watchdog never synced) — dead code, never fired in production, pre-existing since phase-8 `84b0507f` (not introduced by M5). Operator chose the global maximum: **collapse the watchdog to alert-only + remove the orphans.** Remove the dead branch + `SystemCancelInstance` + its `system` authz-bypass path (reduces bypass surface); user-facing `CancelInstance` (document.edit-gated) unchanged; replace F5.2's auto-cancel equivalence fiction with an honest alert-only proof; correct `wiki/modules/approval.md`. Decision recorded in **ADR 0068**. **Consumer:** `metaldocs-jobs` watchdog wiring (drops the CancelService dep) + the milestone-validator's honest alert-only proof + the approval module's unchanged public cancel surface. | `go build`/`go vet` clean with the new `NewWorker` signature; `grep` census of `auto_cancel`/`SystemCancelInstance`/`stuck_watchdog_auto_cancel` = **0** (prod + test); honest alert-only integration proof PASSES on real Postgres (stuck instance w/ valid drift value → `in_progress` + `under_review` + one `stuck_alert`; fresh instance untouched); user-cancel regression (`-run Cancel …/approval/...`) green; `authz.BypassSystem` caller count in approval drops by one; ADR 0068 Accepted + indexed; `approval.md` reflects alert-only. |
...
1. **Per-feature acceptance** — every feature meets its declared "what to validate", and each
   ...
   §2 (per-janitor schedule/idempotency/failure table + singleton + watchdog-lock removal, H-PRE-1 LIVE); F5.3
...
- **Risks (named, with disposition):**
  ...
  - *Watchdog-lock removal is unsafe* (River doesn't actually singleton the periodic job):
    mitigation — the two-client singleton integration test is a **gate**; the lock is not removed
    until it is green. If it fails → advisory lock stays (HS-2 boundary call). **Materialized
    partially (HS-7):** the original "H-PRE-1 retirement" framing was itself wrong — H-PRE-1 governs
    the *audit-writer* advisory lock, not the watchdog's; corrected in place, H-PRE-1 stays LIVE, only
    the watchdog's unrelated lock is removed (ADR 0067 §H-PRE-1 erratum). The singleton gate still binds.
