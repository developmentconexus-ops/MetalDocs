...
2. **F5.2 §2.1 — watchdog `TestIntegration_Watchdog_P1_AutoCancelEquivalence`** — **SUPERSEDED BY F5.8.**
   The proof run's HS-2/HS-6 clause (Non-goals below) fired: the root-cause audit (2026-07-04) proved
   the `'auto_cancel'` seed is not a fixture typo but a **genuine pre-existing production defect** — the
   watchdog's `auto_cancel` branch is dead code (the column can never hold that value; the auto-cancel
   timeout-action concept was abandoned before schema). Per the clause, this was surfaced to the
   operator, who directed the global-maximum fix: collapse the watchdog to alert-only and remove the
   orphans (**ADR 0068**, feature **F5.8**). The honest alert-only proof lives in F5.8; T2 is **removed**
   from F5.7. Do **not** swap the fixture to a passing value — that would mask the dead branch.
...
- **T2 — REMOVED (superseded by F5.8).** The scoping premise ("the watchdog's auto-cancel is not
  encoded by this column; just needs a valid value") was **wrong** — the audit proved the watchdog IS
  keyed off this column and the branch is dead. The correct resolution is the production fix in F5.8
  (watchdog → alert-only, ADR 0068), not a fixture swap. F5.7 no longer touches the watchdog test.
...
- **No change to production consolidation code** (River wiring, janitors, purge, enqueuers, fanout worker). The validator already passed C3/C5 on the migration itself. If any proof fix reveals a genuine production defect (not a fixture/harness bug), that is an HS-2/HS-6 stop — surface it, do not fold it in silently. **This clause fired for the watchdog (T2): the production defect was surfaced and routed to F5.8/ADR 0068, not folded in.**
...
2. **T2:** REMOVED — superseded by F5.8. The watchdog proof (honest alert-only) is an F5.8 acceptance gate, not F5.7's.
