...
**Objective:** Onboarding, export, and erasure become designed capabilities, resolving audit-immutability vs GDPR-erasure. (Finding 15.) **Also closes the M6-carried tenancy-truth defect (F7.4): CI role doesn't exercise RLS, so tenant-isolation tests are false-green in dev.** **Requires developing-new-work gate + ADR (crypto-shredding decision) before planning (D7).**
...
| F7.4 rls-truth-sweep *(carried from M6)* | **Root:** dev/CI `metaldocs_app` is superuser+BYPASSRLS → FORCE RLS **inert in dev** (documented in migrations `0234`/`0237`; prod is NOSUPERUSER+NOBYPASSRLS per ADR 0022 Phase-5 §7 / ADR 0027, so **prod is safe** — this is a *test false-negative*, not a prod leak). Any tenant-scoped query relying **solely** on RLS gives a false-green isolation test in dev, so a future tenant-scoping bug escapes CI. **Global-max fix, not a grep hunt:** flip the CI/dev app role to **NOSUPERUSER + NOBYPASSRLS (non-owner)** so the *existing integration suite becomes the census* — every sole-RLS query flips its isolation test RED; fix each with an explicit `tenant_id` predicate (correct-by-construction, as M6 F6.4 did for the 2 review-due ports); add a lint to keep new sole-RLS tenant-scoped queries out. **Trap:** even under FORCE, RLS does **not** apply to the table **owner's** statements (0234) — the CI role must be a **non-owner**, else NOBYPASSRLS alone won't activate RLS. | CI role is NOSUPERUSER+NOBYPASSRLS+non-owner; the full integration suite runs green under it (every prior false-green isolation test now genuinely exercises RLS); every query the flip surfaced RED carries an explicit `tenant_id` predicate; a lint blocks new sole-RLS tenant-scoped reads/writes with negative proof. Prod-parity: CI role now matches the prod deployment constraint. |

**Milestone gate:** multi-tenant + DB-invariant + docs gates; live drive of onboarding; **F7.4 prod-parity gate** — integration suite green under the NOSUPERUSER+NOBYPASSRLS CI role (isolation tests now real, not false-green). Validation contract defines exact data inventory in/out of erasure scope before implementation, **and the exact sole-RLS query set surfaced by the role flip**.
