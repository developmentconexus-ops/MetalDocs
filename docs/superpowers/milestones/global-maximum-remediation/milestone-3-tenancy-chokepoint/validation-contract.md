...
| Integration-drive execution if the local box cannot run `-tags integration` | 20-min box constraint / env (mission §10) | Run on CI or a capable box before program close-out; drives authored regardless. **Update 2026-07-03 (F3.5):** the load-bearing §2.5 negative-RLS proof does NOT rely on this defer — it was **run GREEN for real** against live Postgres (operator-mandated), after F3.5 retargeted it from the tripwired `documents` to `metaldocs.notifications`. |
