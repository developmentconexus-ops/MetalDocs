...
`governance_events` is the opposite anomaly: dictionary-owned by audit, but audit is **not** its writer — approval is the sole writer (§4a #11), and audit only touches it via the tenant-erasure `TenantDataPort` (export/erase, not day-to-day query). This table's true producer is approval, not audit; the dictionary's ownership label and the write evidence disagree. Flag for correction in §1's index gap, not a new issue.
...
- Ownership-index gaps found in passing (not remediated, just noted): 10 tables missing from `wiki/database/dictionary-index.md` despite having authoritative per-table pages; `governance_events`'s dictionary owner (audit) disagrees with its actual sole writer (approval).
