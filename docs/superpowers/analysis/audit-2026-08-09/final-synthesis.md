...
6. **One transaction spine** (TxRunner-only lifecycle; sqlc-style typed
   queries adopted incrementally) (#92). `db.Tx` at application-owned ports
   remains a ratified convention (ADR 0044) — not debt.
7. **Platform never imports modules** (REQ-TOP-2); composition may.
8. **Async planes observable and fail-closed** (#95, #88).
9. **Every architecture property enforced by a guard with a negative fixture
...
| G-02 | 55 foreign reads + **12 foreign writes**; approval→documents 10× `UPDATE` (PASS 5) | no data-ownership enforcement; writer ports never generalized | R-DATA-1/2; `DocumentCompletionWriter` mirroring templates' pattern | #93/A4 | none for the guard; A4 for migration | V-ARCH-4: extend `HGCrossModule` to UPDATE/INSERT/DELETE (today it is **write-blind**) + ownership catalog as data |
...
| G-12 | ~242 hand scans, ~14 twin pairs (3 byte-identical), 17 hand 23505 checks (4 in delivery), dead lib/pq fallbacks (PASS 6-8) | persistence by visual agreement | typed query derivation (sqlc spike), central pg error mapping | #92/A5 | A4 for hotspot seams | compile-time scan mismatch for adopted repos |
...
```mermaid
flowchart TD
    A1["#87 A1 verifier spine<br/>(+V-ARCH-1..7 registration)"]
    QW["G-02 quick win: HGCrossModule write-scan<br/>(bounded, extends existing guard)"]
...
Why these edges (technical, each): A1 first because every other axis must land
its guard inside the verifier or its closure claim is untrustworthy (#87
charter). A8 before A4/A9 because access semantics and document-visibility
queries must not be migrated twice (binding in #89). A3 before A4 because seam
error translation targets the canonical problem surface. A4 before A5 because
sqlc adoption should happen against post-seam repository shapes, not shapes A4
will move. A4+A8 before A9 because the Controlled Information merge rides on
settled seam patterns and settled access semantics. A7 trace work after A3/A5
so propagation conventions aren't rebuilt.
...
- **A4.0** (early, guard-only): HGCrossModule write-scan + ownership catalog as data; **A4.1** V-ARCH-1 module SCC check (report-only→blocking ratchet); **A4.2** approval→documents writer port (`DocumentCompletionWriter`); **A4.3** sentinel translation (iam→auth pair first, 10 of 19 sites); **A4.4** producer-owned port inversions (security, iam/taxonomy); **A4.5** platform evictions (bootstrap→composition, authn/config split, docgenv2 port); **A4.6** jobs module re-homing; **A4.7** V-ARCH-5 ratchets.
...
- **A5.1** BeginTx ban + migrate 26 files to TxRunner (kills the 3 parallel abstractions, closes GUC-seed risk); **A5.2** delete `DoReadOnly`; **A5.3** pg error classifier (delete dead lib/pq); **A5.4** sqlc spike then per-repo adoption.
...
|---|---|---|---|---|---|
| 0 | #93 (guard slice A4.0) | HGCrossModule write-scan + ownership catalog data | none | negative fixture red on synthetic `UPDATE documents` from approval | report-only first; zero runtime risk |
| 1 | #87 | A1.1–A1.4 verifier product | none | CI runs manifest only; every blocking guard has failing fixture | wrappers keep old jobs until parity proven |
...
| 3 | #93 | A4.1–A4.7 | 2a+2b | SCC count monotonically ↓ under ratchet; foreign writes = 0 | seam-by-seam, adapters keep old callers compiling |
| 4a | #92 | A5.1–A5.4 | Phase 3 (hotspot seams settled) | BeginTx ban green; adopted repos compile-checked | per-repo adoption, never flag-day |
| 4b | #95 (rest) | A7.2–A7.5 | 2a, 4a | trace continuity across outbox proven in test | metadata additive |
