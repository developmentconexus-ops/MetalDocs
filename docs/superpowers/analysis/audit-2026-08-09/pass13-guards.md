...
> **Ownership normalization (post-review correction, canonical):** #87/A1 owns
> *how a guard is trusted* — verifier infrastructure, registry/reachability,
> negative-fixture capability — for every row below. The **property** owner is
> the issue whose remediation the guard proves: runtime validation + one
> problem writer + actor extraction → **#90/A3**; module seam/sentinel/
> consumer-owned-port/data-ownership rules → **#93/A4**. Where a row's original
> owner cell conflicts with this rule, this rule wins (aligned with
> `final-synthesis.md` §C).
