...
- **Layering / type-leak (#93 A4 + #92 A5):** §6.2 (db.Tx/db.DB — accepted three-way classification, corrected 2026-08-09, superseding the earlier "mostly ADR-legitimate, 1 genuine leak" phrasing: (1) ADR-0044-sanctioned event-boundary use; (2) non-event domain-port usage current-architecture/**unresolved pending explicit ruling**; (3) auth raw `database/sql` confirmed debt), §6.3 (application-layer SQL + HTTP-in-application + oversized approval/application + twin FSMs + byte-identical objectstore switch), §6.4 (delivery importing infrastructure types, Postgres-error-code-in-delivery).
