...
|---|---|---|---|---|
| 1 | Domain packages leaking `db.Tx`/`*sql.Tx`/`sql.Null*` | 9-of-15 modules | **9-of-15** (14 modules have a `domain/` dir; `jobs` has none) | Confirmed, but **reclassified**: 8/9 use the ADR-0044-sanctioned `db.Tx`/`db.DB` convention (current standard, not debt); only 1/9 (`auth`) is genuine, ADR-uncovered debt (raw `database/sql` + `sql.NullTime` on a domain DTO) |
...
→ 0015, 0022, 0044
```
**ADR 0044** (`0044-domain-event-pattern-and-river-dispatch.md`, §5, lines 93-97) explicitly rules: *"The args/port use `db.Tx`, not `*sql.Tx`, at the boundary — the concrete-tx assertion is isolated to that one adapter."* It further rejects putting the type in `internal/platform/...` because "`platform` is module-agnostic infrastructure … so domain vocabulary does not belong there." `db.Tx`/`db.DB` (the neutral structural interfaces from `internal/platform/db/tx.go`, satisfied structurally by both `*sql.Tx` and `*sql.DB`) is the **ratified target convention** for transactional domain ports, not an accidental leak.

**Verdict:** 8 of the 9 modules (`approval, audit, controlleddocuments, documents, iam, security, taxonomy, tokens`) use `db.Tx`/`db.DB` exclusively — this is **current standard, ADR-sanctioned convention**, not debt. **`auth` is the one genuine violation**: `internal/modules/auth/domain/session_admin.go` imports the raw standard-library `database/sql` package and types a domain read-model DTO's fields as `sql.NullTime` — this is not covered by ADR 0044 (which is scoped to transactional domain-event args using the neutral `db.Tx`/`db.DB` interfaces, not raw `database/sql` types on read DTOs). **Classification: current-debt, uncovered by any ADR.** Fix is mechanical (three `sql.NullTime` → `*time.Time` or plain `time.Time` + zero-value handling in the repository).
...
All 3 are **byte-identical SQL + byte-identical authz-check**, differing only in the executor plumbing — this exactly matches the historical "3 byte-identical" sub-claim. Given the ADR-0044-sanctioned `db.Tx`/`db.DB` structural-interface convention (§6.2), these pairs are a clear case where the non-`Tx` variant could be rewritten as a one-line wrapper (`func (r *X) Create(ctx, p) error { return r.db.beginRun(ctx, func(tx) error { return r.CreateTx(ctx, tx, p) }) }`) instead of carrying a second, independently-maintained copy of the SQL and the authz check.
