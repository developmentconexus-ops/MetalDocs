...
- **QA gates that apply:** contract (spec regeneration), authz (every operation's declared capability is *enforced* — the per-operation conformance suite is this gate; see the design §10.2), docs. **N/A:** multi-tenant isolation, async/idempotency, DB-invariant — nothing in those planes changes.
- **SUPERSEDED by the design §10.1 — there is no parity gate.** This bullet demanded that the derived resolver and `routeRules` return identical `(Capability, Visibility)` for every registered pattern. That is the same oracle-less comparison amended at §6 above, and leaving it standing here left the program with **two active definitions of done**, which is precisely the duplicated-enumeration defect it exists to delete. The definition of done is now the design §10.2: completeness enforced by the build and by the §5 boot assertion, and a per-operation conformance suite proving each declared capability is *enforced at tier 1*. Old/new runtime equivalence is deliberately **not** proven — see the design §10.2's stated cost.
- **Evidence shape:** `go build ./...` · `go vet -tags integration ./...` (mandatory — seam signatures change again) · `gofmt -l` · `go test ./...` · `.\scripts\check-system-runnable.ps1` · live drive of at least one route per visibility class. Plus a TDD red-proof for the conformance suite itself (delete one operation's tier-1 rule, watch its case go red).
...
- **The HEAD tier-1 defect is absorbed, not deferred.** Ruling A makes method semantics a single declaration, which dissolves it. It stops being a standing operator-decision item and becomes an acceptance criterion: HEAD must resolve to its GET row's capability, and that must be an explicitly listed behavior change with a regression test — the design §10.3 row 5 — not a silent one. (**AMENDED**: this bullet said "an explicitly listed delta in the parity gate (§8)". There is no parity gate; §10.1 explains why. The requirement itself is unchanged and is discharged where §10.3 discharges the other six.)
...
5. **`routeRules` is deleted, not deprecated.** No compatibility layer, no fallback. **AMENDED by the design §10.1:** the clause that followed — *"after an exhaustive parity gate over every registered pattern, with every intentional delta listed explicitly"* — is struck. It was authored here before any design existed, it is not one of the four operator rulings, and it demanded a proof against an artifact that is not an authority. The deletion is licensed by `routeRules` being a **second copy of a decision that now has one home** (ruling A), and the residual risk is stated in the design §10.2 and §12.
