...
| Backend authz enforcement | Existing `CapabilityService` middleware (tier 1) + `internal/modules/iam/authz.Require(ctx, tx, cap, area)` called in handler body (tier 2) + Postgres tripwire trigger (`metaldocs.asserted_caps` GUC) as ultimate enforcer | No new generated files. Spec `x-authz-area` is a **lint contract** only | CI rule `authz-call-present` (Task 8) AST-greps handler bodies to verify the call exists and matches the declared field path. Codegen wrappers were evaluated and rejected — see §4.5 |
...
| `openapi-lint` | `authz-drift` (spec rules — see §4.5) |
|  | `authz-call-present` (handler body calls `authz.Require` matching the declared `x-authz-area` field) |
|  | `tripwire-pairing` (mutating SQL lives in functions whose call graph includes `authz.Require`) |
...
Two-tier authz model documented in `wiki/decisions/0007-two-tier-authz.md`. Both tiers declared in spec. **Spec is a contract + documentation surface. It is not a code-generation input for authz.**
...
```
Existing HTTP middleware reads `security`, calls `CapabilityService.Require("template.create", userID)`. No new codegen — middleware is hand-wired per module today and stays that way.

**Tier 2 — area (in-tx, enforced by Postgres tripwire):**
```yaml
paths:
  /api/v1/templates/{id}/publish:
    post:
      ...
      x-authz-area:
        ...
        field: area_code      # top-level field name; dotted for nested (e.g. zone.area_code)
        multi: false          # default false; true → field is array → authz.RequireAll
```

Capability for tier 2 reuses the scope declared in `security[0].bearerAuth[0]` (single source of truth — no `capability:` key in the extension).

**Enforcement is at the database layer.** `authz.Require(ctx, tx, capability, areaCode)` (`internal/modules/iam/authz/authz.go:44`) does three things in the caller's transaction:

1. Permission check against `role_capabilities` + `user_process_areas` (tenant + area filter).
2. `system_admin` bypass check.
3. **Sets the tx-scoped GUC `metaldocs.asserted_caps`** which a Postgres tripwire trigger (`migrations/0142b_role_capabilities_v2_enforce.sql:138-172`) reads on every mutation. If the required cap isn't in `asserted_caps`, the database itself raises `ErrCapabilityNotAsserted` and rejects the write.

That tripwire is the real enforcer. The Go `authz.Require` call is the way handlers prove the cap was checked. A pre-tx wrapper cannot replace it — the tx itself must carry the GUC.

**No code generation.** `cmd/authzgen` (the embedded-spec post-process generator) was evaluated and rejected in `docs/superpowers/notes/2026-05-10-authz-codegen-feasibility.md` (see addendum dated 2026-05-10). Reasons:

- The Postgres tripwire already provides the static enforcement guarantee. Adding a generated wrapper duplicates the check, adds a maintenance surface, and provides zero additional safety.
- Industry pattern (Stripe, GitHub, AWS IAM) is lint + runtime enforcement, not codegen wrappers.
- Spec declarations remain valuable as (a) operator-facing doc via Redoc, (b) lint input for CI drift detection.

**`x-authz-area` is a lint contract**, not a code-gen input. It declares the field path that the handler MUST pass to `authz.Require`. CI verifies the declaration matches the handler call.

**Escape hatches:**
- `x-authz-skip-area: "<reason>"` — explicit opt-out for routes with no area dimension (e.g. `/v1/auth/login`, `/v1/health`). Reason string is non-empty and surfaces in operator docs.
- `x-authz-custom: true` — silences lint for ops that compute area at runtime (rare; documented in handler comment).

**CI lint job `authz-drift` — four rules:**

1. Every operation declares `security` (or explicit `security: []` to opt out).
2. Operations matching state-transition pattern (`POST /api/v1/{module}/{id}/{verb}` where verb ∈ {publish, submit, approve, reject, archive, unarchive}) declare `x-authz-area` OR `x-authz-skip-area` OR `x-authz-custom: true`.
3. **`authz-call-present`** — for each op with `x-authz-area: { field: X, multi: M }`, locate the handler method `<Module>Handler.<OperationID>` in `internal/modules/<module>/...` and AST-grep for a call to `authz.Require(ctx, tx, _, req.Body.<PascalCase(X)>)` (or `RequireAll` if `M == true`). Missing or mismatched call → CI red with file:line. Handlers with `x-authz-skip-area` are skipped; handlers with `x-authz-custom: true` are required to contain at least one call to `authz.Require` or `authz.RequireAll` (field path not checked).
4. **`tripwire-pairing`** — every mutating SQL statement in `internal/modules/*/repository.go` (heuristic: `tx.ExecContext` with `INSERT|UPDATE|DELETE`) lives in a function whose call graph includes `authz.Require`. (Lightweight — single-file scan, not full call-graph analysis.)
...
| `authz-drift` | Every operation declares `security` (or explicit `security: []`); state-transition ops declare `x-authz-area` OR `x-authz-skip-area: "<reason>"` OR `x-authz-custom: true`. |
| `authz-call-present` | For each op with `x-authz-area`, the matching Go handler body calls `authz.Require(ctx, tx, _, req.Body.<PascalCase(field)>)` (or `RequireAll` if `multi: true`). |
| `tripwire-pairing` | Every mutating SQL (`tx.ExecContext` with `INSERT|UPDATE|DELETE`) in `internal/modules/*/repository*.go` lives in a function whose body contains a call to `authz.Require` or `authz.RequireAll`. |
...
| CI | New jobs: `envelope-drift`, `pagination-drift`, `authz-drift`, `authz-call-present`, `tripwire-pairing`. |
...
| Risk | Mitigation |
|---|---|
| oapi-codegen upgrade breaks generated handlers | Pin version in `go.mod`. Authz enforcement does NOT depend on oapi-codegen output beyond the request/response struct shapes — `authz.Require` is hand-called in handler bodies, verified by AST lint. Upstream template ABI changes are isolated to `api.gen.go`. CI runs `make gen` on every PR. |
