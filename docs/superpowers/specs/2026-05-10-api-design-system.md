...
| Backend authz middleware | `cmd/authzgen` (project-owned Go program; reads embedded spec) | `internal/modules/<x>/api/authz_gen.go` | Per-op wrapper: tier-1 `CapabilityService.Require` + tier-2 `authz.Require(ctx, scope, areaCodes)`. Decoupled from oapi-codegen template ABI |
...
Capability for tier 2 reuses the scope declared in `security[0].bearerAuth[0]` (single source of truth — no `capability:` key in the extension). If a route legitimately needs different scopes at tier 1 vs tier 2, that's an ADR moment, not an extension shape change.

**Codegen mechanism — `cmd/authzgen` (project-owned Go program):**

Reads embedded `openapi.yaml`, walks operations, emits `internal/modules/<module>/api/authz_gen.go` with one wrapper per op:

```go
// Code generated by authzgen; DO NOT EDIT.
func (h *handlerWithAuthz) PublishTemplate(ctx context.Context, req PublishTemplateRequestObject) (PublishTemplateResponseObject, error) {
    if err := authz.Require(ctx, "template.publish", req.Body.AreaCode); err != nil {
        return nil, err
    }
    return h.inner.PublishTemplate(ctx, req)
}
```

Decoupled from oapi-codegen template internals (no fork of `strict-http.tmpl`). Upstream upgrades cost zero. See `docs/superpowers/notes/2026-05-10-authz-codegen-feasibility.md` for the approach comparison and rationale.

**Escape hatches:**
- `x-authz-skip-area: "<reason>"` — explicit opt-out for routes with no area dimension (e.g. `/v1/auth/login`, `/v1/health`). Reason string surfaces in audit logs.
- `x-authz-custom: true` — silences spec lint for ops that hand-roll `authz.Require` (computed area resolution). Use rarely; documented in handler.
...
2. Operations matching pattern `/api/v1/{module}/{id}/{verb}` where `verb` is a state transition (`publish`, `submit`, `approve`, `reject`) MUST declare `x-authz-area` OR `x-authz-skip-area` OR `x-authz-custom: true`.
3. `git diff --exit-code` on `internal/modules/*/api/authz_gen.go` after `make gen` (drift = CI red).
...
| Risk | Mitigation |
|---|---|
| oapi-codegen upgrade breaks generated handlers | Pin version in `go.mod`. `cmd/authzgen` does NOT fork upstream templates — it reads spec independently and emits its own wrapper file, so template ABI changes are irrelevant to authz codegen. CI runs `make gen` on every PR. |
