...
- **3b-iii — template signoff + completion port** — decomposed iii-a (kernel) + iii-b (app). Same kernel gap on `approval_signoffs` (arm hardcodes `document.signoff`); signoff rows have no direct `subject_kind` column → resolved via **parent-lookup** discriminator (ADR 0083 §Consequences pre-ratified: "resolves via the parent instance").

**3b-iii-a — tripwire parent-lookup signoff discriminator — DONE, commit `4c311f16`, independent reviewer ACCEPT**
- `tripwire.Arm` gains second discriminator form: `WhenParentTable`/`WhenParentKeyCol`/`WhenParentDiscrimCol` (parent-lookup; complements #26's `WhenColumn`/`WhenValue` NEW-column form). Arm #2 split → document→`document.signoff` / template→`template.approve`. `RenderMigration()` emits `SELECT subject_kind INTO v_parent_subject_kind FROM public.approval_instances WHERE id = NEW.approval_instance_id;` then nested `CASE v_parent_subject_kind` with fail-closed P0001 ELSE + new DECLARE local `v_parent_subject_kind TEXT`. Golden migration `0300` (verified 0299 highest → 0300 true next-free); `tripwireMigrationPath` + gen-tripwire path bumped. DRIFT `unionArmCapsFor` unchanged (already unions across `(table,op)`).
- Self-caught + fixed a pgx apostrophe bug in the ledger description (same class as #26) before finalizing.
- **3b-iii-b DONE — commit `0657cf04`, independent reviewer ACCEPT.** Folded slice (submit-lock + signoff completion). `TemplateSubmitService` locks `draft→under_review` in-tx via templates-owned `TemplateVersionSubmitWriter.MarkTemplateVersionUnderReview` (CAS `WHERE status='draft'`, 0-rows→`ErrTemplateVersionNotDraft`; `template.submit` asserted before the UPDATE so the `templates_template_version` tripwire is satisfied). `recordSignoffInTx` guards all 9 subject-conditional points by `Subject.Kind`: template asserts `CapTemplateApprove`+`"tenant"` (bytes the `0300` arm), reads version content-hash via `TemplateVersionReader` (no freeze pin), skips both `CapDocumentEdit`/`pinInvoker.Pin`/both documents-UPDATEs/cancel-GUC/lifecycle-enqueue/PDF, and calls `MarkTemplateVersionApproved`(under_review→approved)/`MarkTemplateVersionRejected`(under_review→draft). Ports approval-owned, `templates/infrastructure` implements (`ApprovalCompletionWriter` satisfies both writer ports), wired at `main.go`. **Reviewer ACCEPT** (independent, reproduced): document `else` branch byte-for-byte unchanged (diffed — only reindented); boundary clean (approval has zero `templates` non-test imports / zero `templates_template_version` SQL; test-file imports excluded by the guard, run confirms `OK`); submit-lock uses the sanctioned `CanTransition` edge with correct assertion-before-UPDATE ordering; edit-lock proven via REAL `templatesapp.Service.PresignTemplateUpload`→`ErrInvalidStateTransition`; **negative control genuinely hits `P0001`** — the `SECURITY DEFINER` trigger `enforce_capability_asserted` fires role-independently (not RLS), so `document.signoff` on a template-parent signoff is rejected (no false-green); `placeholder_schema` seed fix (`'{"placeholders":[]}'`→`'[]'`) matches the real `[]domain.Placeholder` array column. Full ladder green (build, vet, api-lint 0 incl. authz-area-scope-binding + PARITY/DRIFT, boundaries, all 3 integration suites; baseline 9/4 E-PROD unchanged).
- **3b-iii-b STOP #1 → hub resolution (submit-lock gap in #24).** First #28 dispatch STOPped in the mandatory pre-code scout: the ratified completion port CASes on `templates_template_version.status='under_review'`, but `CanTransition` (`version.go:103`) models no `draft→approved` edge, and #24's `TemplateSubmitService` never flips the version out of `draft` — so the port precondition is unreachable AND a template stays editable (autosave/upload gate only on `status='draft'`, `autosave.go:51`) throughout a kernel approval. Both facts hub-verified independently (grep: no status UPDATE in submit service; read CanTransition edges). **Resolution (hub-owned, Option a):** the kernel template submit must lock the version `draft→under_review`, symmetric with the documents submit path — the sanctioned transition (`CanTransition(under_review)` unconditional), closing the concurrent-edit hole and founding the port precondition; the already-asserted `template.submit` cap covers the UPDATE tripwire in-tx; templates-owned writer port (approval never UPDATEs the table). The true atomic unit is the **template version lifecycle under kernel governance** (submit-lock + approve-complete + reject-return); #28 folded to carry all three. Not operator-escalated: resolvable from code + document symmetry, #24 unpushed same-milestone. Implementer resumed from clean uncommitted state.
- **Reviewer ACCEPT** (independent, adversarial): byte-identity of ALL other branches machine-verified three ways (manual `diff 0299 0300` → only signoff branch + 1 DECLARE + header + ledger differ; Go golden test `TestRenderMigration_UntouchedArmsByteIdenticalToPrior0299Golden`; strip-and-compare test). Parent-lookup correctness verified: FK `approval_signoffs.approval_instance_id uuid NOT NULL` exists (baseline :2032); NULL/absent parent falls to ELSE → fail-closed (no WHEN matches NULL); arrays disjoint, no union anywhere. Migration house-style/idempotency/ledger-literal clean. PARITY 0 violations; DRIFT green. Live-DB approval integration `ok metaldocs/tests/integration/approval 15.556s` (document signoff path unaffected). Full gate ladder green (build, api-lint, module-boundaries, vet, tripwire+api-lint tests).
