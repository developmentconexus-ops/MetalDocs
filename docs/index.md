...
| Access / GroupMembership / RoleAssignment / Roles | [Authorization and audit](architecture/authorization-and-audit.md) + search the Access journey/ops in [Product journeys](product/journeys.md) | [API operation census](decisions/api-operation-census.md) for operation ownership; search only implicated ops/global law in [Wire contract](architecture/wire-contract.md); open a bounded Access decision only after it is integrated/current | Whole Wire/Persistence files, provider roles, old ACL machinery, frontend effective-access inference |
...
| Frontend LOCK/P8 Evidence recovery for later P11 | [B01–B09 locator](decisions/t11-b01-b09-lock-evidence.md) + [B10 locator](decisions/t11-b10-lock-evidence.md) | Exact ref/blob named by the locator | Historical Git broadly, intermediate wireframe candidates |
