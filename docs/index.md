...
| Document confidentiality / who may read a document / clearance | [Confidentiality Launch reopen](decisions/document-confidentiality-launch.md) + [Document confidentiality seam](decisions/document-confidentiality-seam.md) | [Authorization and audit](architecture/authorization-and-audit.md) for the current T3 equation; [Access Assignment read](decisions/access-assignment-read.md) when the current grant surface is implicated | Whole Wire/Persistence files, legacy ACL machinery, per-document sharing designs |
