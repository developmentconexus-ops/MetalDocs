...
| T8-F | Frontend realization | CURRENT / OPERATOR-RATIFIED + BOUNDED T11 REOPEN | Semantic/lens realization for the current census; no frontend AuthZ authority or incomplete-page post-filter | `../architecture/frontend.md` + current bounded decisions + `t8f-ratification.md` |
...
| T11-CONF | Document confidentiality semantic model | CURRENT / OPERATOR-RATIFIED / DISPOSITION UNDER REOPEN | Governed non-hierarchical ConfidentialityClass plus clearance grants as one conjunctive term on the T3 equation, class never entering identity/code; per-document ACL and Area-as-secrecy permanently excluded. The FUTURE-only disposition is superseded by the open promotion reopen | `document-confidentiality-seam.md` |
| T11-CONF-L | Document confidentiality Launch promotion | RATIFICATION CANDIDATE / OPERATOR-AUTHORIZED | Bounded Product/T6 reopen bringing in-Area confidentiality into Launch V1: census 89 → 97, 13 idempotent creations, 14/14 ETag domains, no new permission family, Q1-Q5 ratified; B02/B03/B06/B10/B11/B12 frontend rebaseline still pending | `document-confidentiality-launch.md` |
| T11-FORMAT | Content format vocabulary | CURRENT / OPERATOR-RATIFIED | ContentFormat widened to the docx/xlsx/pptx/pdf/png/jpeg/txt/csv closed extensible set with per-format structural admission laws; governance is format-neutral, in-app editing stays DOCX-only (CNT-14), official PDF binds to converter availability; no operation, concept or permission added | `content-format-vocabulary.md` + `api-operation-census.md` |
| T11-TEMPLATE | B12 Template Configuration read precision | CURRENT / OPERATOR-RATIFIED | op43 remains the template-configuration traversal and gains exact server-side q search plus eligible-type/template-role/effective-revision filters; no operation 90, no "general template" concept, no Area-scoped templates | `template-configuration-read.md` + `api-operation-census.md` |
