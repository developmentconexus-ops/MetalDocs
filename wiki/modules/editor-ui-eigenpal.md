...
The vendor `onChange` fires for the **body view only** — header/footer ProseMirror transactions never reach it. Without a second channel, HF edits silently miss token-usage refresh (`getUsedTokens` re-run) and autosave. `MetalDocsEditor.tsx` adds a delegated `input` listener on the `display:contents` root (ProseMirror input events bubble out of every band). It is **scoped to header/footer bands** (their PM parent carries `data-hf-kind`) so the two channels are symmetric and non-overlapping: **body → vendor `onChange` prop, HF → delegated `input` listener**. This avoids double-counting body edits. Both feed the same `handleChange`, which fires the lightweight `props.onChange` notify *unconditionally* (decoupled from the autosave guard) before scheduling the debounced autosave. Regressions in `MetalDocsEditor.tokens.test.tsx`: "surfaces header/footer edits as a change signal" + "does NOT double-count body input".
...
- 2026-06-27 — Grade-A review fixes (SP-0 slice): change signal made symmetric (body→vendor onChange, HF→`data-hf-kind`-scoped delegated `input` listener; no body double-fire), `props.onChange` decoupled from the autosave guard, `insertToken` caret band resolved fresh from `document.activeElement` (stale detached-node fallback removed). Panel CSS: `--surface-hover` magic fallback → `--surface-3`, added `:focus-visible`. Tests +2 (body-no-double-fire). Deferred follow-ups filed: unify token grammar in `@metaldocs/shared-tokens` before SP-1 (UI `[A-Za-z0-9_]+` vs renderer docxtemplater grammar can drift on non-snake_case keys); migrate catalog fetch off `useEffect`+`setState` to TanStack Query (pre-existing, `5a9f3b24`).
