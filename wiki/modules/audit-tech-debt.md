...
### T-008 · Audit route served outside oapi-codegen (manual `HandleFunc`, not generated handler) — CLOSED 2026-07-02 (grade-A simplification program)
- **Severity:** minor (closed)
- **Surface (resolved):** `internal/modules/audit/delivery/http/handler.go` — `Handler.RegisterRoutes` now mounts via `auditapi.HandlerWithOptions` (the oapi-codegen-generated router) instead of three hand-written `mux.HandleFunc` registrations. `Handler` implements `auditapi.ServerInterface` directly (`ListAuditEvents`, `ExportAuditEvents`, `GetAuditExportStatus`, `DownloadAuditExport`), each a thin adapter delegating unchanged to the pre-existing private methods (`handleEvents`, `handleExport`, `handleExportSubresource`). No parsing/response/business logic was rewritten — only the mount mechanism changed. Mirrors the controlleddocuments original pattern and the CON-03 templates/approval migrations (`internal/modules/templates/delivery/http/handler.go`, `internal/modules/documents/approval/http/router.go`) and the CON-07 iam Router (`internal/modules/iam/delivery/http/router.go`).
- **Observation (original):** The `operationId` `listAuditEvents` (and the 3 export ops) existed in the spec and the generated strict-server code was current (verified under CON-09), but the handler was registered manually via `HandleFunc` rather than through the generated `ServerInterface`/`HandlerWithOptions` path used by other modules — a pure mount-pattern gap, not a missing/stale contract.
- **Verification:**
  - `go build ./...` — clean.
  - `go build ./apps/api/cmd/metaldocs-api/...` — `metaldocs-api` binary builds clean (main.go's `auditHandler.RegisterRoutes(mux)` call site unchanged; only the method body it invokes changed).
  - `go vet ./internal/modules/audit/...` — clean.
  - `go test ./internal/modules/audit/...` — all green, including the full pre-existing behavior suite (`handler_test.go`, `handler_export_test.go`, `handler_typed_test.go`, `handler_allow_test.go`) run unmodified except one assertion in `handler_allow_test.go` (`TestHandleExportSubresource_405_Allow`) updated from `Allow: "GET"` to `Allow: "GET, HEAD"` — an intrinsic, RFC 7231-correct byproduct of Go 1.22 `http.ServeMux`'s method-qualified pattern registration (a `GET`-registered pattern implicitly also matches `HEAD`), not a tier-1/authz/rate-limit behavior change.
  - New pin test `internal/modules/audit/delivery/http/route_registration_test.go`: `TestRegisterRoutes_AllSpecRoutesRegistered` (all 4 spec routes dispatch, no 404), `TestRegisterRoutes_NoExtraRoutesServed` (sibling/unspec'd paths still 404 — mount is not over-broad), `TestRegisterRoutes_WrapperValidationError` (malformed `limit` query param fails at the generated wrapper's own bind step, 400), `TestRegisterRoutes_HandlerLevelValidationStillProblemJSON` (malformed `occurred_after` passes wrapper binding and is still rejected by the delegated `handleEvents`/`parseTime` logic as RFC 9457 `application/problem+json` — proves the adapter is a real delegation, not a stub).
  - Tier-1 capability gating (`CapAuditRead`, `apps/api/cmd/metaldocs-api/permissions.go:257-259`) is unaffected: it keys off method + path, which `HandlerWithOptions`' `BaseURL: "/api/v1"` + the spec's relative paths preserve byte-for-byte (AD-1), and the migration did not touch `permissions.go`.
- **Evidence:** `_artifacts/02-flow-list.md` §1 (original finding); this closure re-verified under the grade-A simplification program 2026-07-02.
- **Linked backlog row:** `backlog/audit-refactor.md#R-008`
- **Linked ADR:** `wiki/decisions/0012-contract-first-api.md` (audit is no longer the residual — mount now matches the other 13 modules)
...
- Decisions without ADR link: 12 / 13 (T-001..T-007, T-009..T-013 = 12 missing-ADR; T-008 links ADR 0012, closed 2026-07-02 — residual eliminated)
