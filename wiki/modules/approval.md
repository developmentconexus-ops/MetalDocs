...
**Last verified:** 2026-05-18 (approval-instance contract + editor sidebar sync) | **Owner:** unassigned | **Status:** active | **Maturity:** L2
...
Inbound interfaces (16 HTTP routes â€” see Â§5.3) plus 5 Go-package consumers per cross-deps artifact Â§2 (documents, documents/delivery/http, jobs/effective_date_publisher, jobs/stuck_instance_watchdog, iam test only).
...
| `internal/modules/documents/approval/application/publish_service.go:166` | `SchedulePublish` | method | approved â†’ scheduled with `effective_at` |
...
| GET | `/api/v1/documents/{id}/approval-instance` | `GetInstanceByDocumentHandler` (`doc_approval_handler.go:16`) | tier-1 only; editor sidebar consumes this only while the document is `under_review` |
...
| GET | `/api/v1/documents/{id}/approval-instance` | `internal/modules/documents/approval/http/router.go:18` | `h.GetInstanceByDocumentHandler` | `/api/v1/documents/{id}/approval-instance` | `getApprovalInstanceByDocument` | `GetApprovalInstanceByDocument` | Aligned | Shared contract repaired on 2026-05-18 for editor sidebar consumers |
...
- Binary: single Go server `apps/api/cmd/metaldocs-api/main.go` (port `:8081` â€” see `wiki/references/local-dev-startup.md`).
...
- 2026-05-18 - Approval-instance contract sync: `/api/v1/documents/{id}/approval-instance` is now described in OpenAPI and generated surfaces, so the editor sidebar consumes the runtime-aligned payload instead of legacy handwritten unions. The sidebar only renders the approval chain while the document is `under_review`.
