...
C4Context
    ...
    System_Boundary(b1, "MetalDocs") {
        ...
        System(jobsTemporal, "metaldocs-jobs", "River temporal queue worker; hosts the release-evaluate job (ADR 0085 release coordinator)")
    ...
    Rel(jobsTemporal, approval, "ReleaseEvaluateWorker: release_evaluate (River temporal queue)")
...
C4Container
    ...
    Container(app, "application/", "Go", "Services: Submit, Decision, Cancel, ReleaseCoordinator (ADR 0085), Read, RouteAdmin, Obsolete, Delegation, FastForward, ReviewVerdict, MarkReviewed, TemplateSubmit, RoutePreview, +helpers (content_hash, events, release_facts, signoff_idemp)")
...
| `internal/modules/approval/application/submit_service.go:137` | `SubmitRevisionForReview` | method | draft â†’ under_review + instance create + eligibility snapshot; step 10 (`:493`) calls `autoApprove` on a zero-stage (livre) route |
| `internal/modules/approval/application/submit_service.go:519` | `autoApprove` | method | **[ADR 0087](../decisions/0087-livre-configured-no-approval-route.md).** Completes a stageless instance inside the submit tx via `domain.Instance.AutoApprove` + `completeDocumentTerminalApproval`/`completeTemplateTerminalApproval`; emits `EventTypeApprovalAutoApproved` |
| `internal/modules/approval/application/document_terminal_approval.go:82` | `completeDocumentTerminalApproval` | func | **ADR 0087.** SOLE completion path for a document-subject instance reaching terminal approval (signoff, review-verdict, auto-approve): instance CAS → `document.edit` assert → ADR 0085 release-fact record → documents OCC transition → ADR 0044 lifecycle enqueue |
| `internal/modules/approval/application/template_terminal_approval.go:55` | `completeTemplateTerminalApproval` | func | **ADR 0087.** SOLE completion path for a template-subject instance reaching terminal approval: instance CAS → `templates_template_version under_review→approved` |
| `internal/modules/approval/domain/instance.go:170` | `Instance.AutoApprove` | method | **ADR 0087.** Completes a STAGELESS instance at creation time; fails closed (`ErrAutoApproveHasStages`, `:19`) on any staged instance |
| `internal/modules/approval/domain/route.go:112` | `Route.Validate(policy)` | method | **ADR 0087 rework:** stage-count is policy-homed, not a blanket structural floor — `RoutePolicyNoApprovalStages` (livre) requires exactly 0 stages (`ErrRouteStagesNotPermittedForProfile`), `RoutePolicyRequireApprovalStage` (controlado) ≥1 approval-kind stage, `RoutePolicyApprovalOptional` (simples) ≥1 stage; `Validate("")` stays structural-only |
| `internal/modules/approval/application/decision_service.go:159` | `RecordSignoff` | method | Eligibility + SoD + quorum + freeze + outbox; terminal completion delegates to `completeDocumentTerminalApproval`/`completeTemplateTerminalApproval` (ADR 0087) |
...
| `internal/modules/approval/application/release_coordinator.go:123` | `ReleaseCoordinator.Evaluate` | method | **ADR 0085.** Single release-predicate evaluation (approval fact × artifact facts × effective-date gate × supersession head) under an in-tx lock; idempotent — re-reads every input from the job's `domain.ReleaseGenerationKey` payload, never trusts enqueue-time state; no-ops on a lost optimistic-concurrency race (`IsLostReleaseCAS`, `:538`); emits events under the system principal `ReleaseSystemPrincipal` (`:29`, `"system:release-coordinator"`) |
| `internal/modules/approval/application/release_facts.go:302` | `RecordArtifactFactTx` | method | **ADR 0085.** Records a final-artifact fact (DOCX or PDF) for a release generation and enqueues re-evaluation via the injected `ReleaseEvaluationEnqueuer` in the same tx (transactional outbox) |
| `internal/modules/approval/domain/release_hold_port.go:56` | `ReleaseHoldReader` (port) + `StuckReleaseHoldView` (:18) | iface/type | **ADR 0085 Stage C W2.** Published read-port for the `jobs` module's River periodic `release_hold_reconciler` — the ONLY approval-owned surface it may call for stuck release generations, never raw SQL against `public.release_generations`. Read-only by construction (no writer twin — recovery from a stuck hold is always "re-run the idempotent evaluation," never a reconciler mutation). Mirrors `SLAOverdueReader`'s (`sla_port.go`) shape |
| `internal/modules/approval/infrastructure/release_hold_reader.go:17` | `ReleaseHoldReaderPG` + `NewReleaseHoldReaderPG` (:23) | type/func | Postgres adapter implementing `ReleaseHoldReader`; both `ListStuckHolds`/`ListTenantsWithStuckHolds` reference one shared predicate constant `releaseHoldStuckCorePredicate` (:58) so "stuck" cannot drift between the cross-tenant enumeration and the per-tenant read; wired into the `apps/jobs` binary at `apps/jobs/cmd/metaldocs-jobs/main.go:146` |
...
`LoadFrozenContentHash(ctx, tx, tenantID, instanceID)` (`infrastructure/approval_repository.go`; renamed from `LoadActiveDocumentContentHash`) is a bare `SELECT frozen_content_hash FROM approval_instances WHERE id=$1 AND tenant_id=$2` — no COALESCE, no fallback subquery against `document_revisions`. A missing row or NULL pin returns `ErrNoActiveContentHash`, mapped to `ErrContentHashMismatch` → HTTP 412 at the signoff call site (`decision_service.go`). `documents.content_hash_at_submit` remains the fallback **only** for the true no-instance/draft display case (`internal/modules/documents/infrastructure/active_instance_reader.go`, `ActiveInstanceForControlledDocument` — two explicit sequential reads, no COALESCE): frozen instance → its pin; no instance or not-yet-frozen → head-revision hash. This is the concrete application of the repo-wide no-fallback principle (`CLAUDE.md` "no-fallback-principle" memory) to the approval hash chain. **ADR 0085 update:** the old publish-time fail-closed guard (`publish_service.go`'s `PublishApproved`/`SchedulePublish`, deleted) is superseded — `frozen_content_hash` is now baked directly into the release generation's identity key (`domain.ReleaseGenerationKey.FrozenContentHash`, scoped in every generation lookup, e.g. `release_coordinator.go:190,785`), so a release evaluation structurally cannot run against a mismatched hash rather than checking it defensively at publish time.

### 8.9 In-flight disposition backfill tool (ADR 0085 Stage C)

`scripts/release-backfill/` (`main.go`, `backfill/backfill.go`, `backfill/wire.go`) is a `go run` one-shot tool — no fifth binary, no Docker image, no HTTP route, no long-lived state (precedent: `scripts/api-lint`, `scripts/req-trace`) — that gives a legacy, already-`approved` document the release prerequisites the coordinator needs (approval fact, materialization dispatch, evaluation) without re-approving it and without trusting any legacy artifact pointer.

- **Invocation:** `go run ./scripts/release-backfill -docs <uuid>[,<uuid>...] [-dry-run=false] [-dsn ... | DATABASE_URL]`. `-docs` is REQUIRED with no "all approved" mode — which documents are safe to backfill is an operator judgement, always supplied explicitly. `-dry-run` defaults `true`. Quarantined documents are excluded until Etapa 5 (out of this tool's scope — the release gate handles those). The DSN is never read from `.env` and never printed; every error is redacted before reaching stdout/stderr.
- **Per document, ONE transaction** (production parity with the terminal-approval release path): `SeedTxTenant` + `BypassSystem` → fail-closed preflight → `approvalapp.RecordApprovalFactTx` (synthesizes the missing approval fact) → `docapp.FreezeService.RepairMaterialization` (`internal/modules/documents/application/freeze_service.go:249` — re-enqueues materialization against the new generation; NOT `Pin`, since the freeze is already final and re-running it would recompute a `values_hash` the approver never signed) → `ReleaseEvaluationEnqueuer.EnqueueReleaseEvaluationTx` (arms the coordinator, `runAt=now`).
- **Preflight (`backfill.go:269`) fails closed** on every precondition: document must be `approved`, controlled, and pinned (`values_hash`/`values_frozen_at` BOTH set — a half-pinned document is a corrupt freeze, not a repairable one); exactly one `approved` `approval_instances` row with a `frozen_content_hash` (2+ is refused, not guessed); that instance must have at least one `approve` signoff (its actor becomes the recorded final approver — never invented).
- **Deliberately absent: legacy artifact fast-forward.** `final_docx`/`final_pdf` column presence on the legacy row is never readiness (ADR 0085's own framing) — no legacy artifact pointer is promoted into the new generation. Fresh artifacts are rendered from the frozen values snapshot by the production worker, which lands both artifact facts and re-enqueues evaluation; until then the generation honestly holds on `materializing`.
- **Idempotent/replay-safe:** the generation identity (tenant, subject, document, instance, revision, revision_version, frozen_content_hash) upserts, the staging outbox dedupe is generation-aware, and an already-backfilled document is detected in preflight (`loadExistingGeneration`) and reported `already-backfilled` without writing anything twice.
- **Outcomes:** `backfilled` / `already-backfilled` / `planned` (dry-run) / `failed` — one report line per document; a per-document failure rolls back only that document's transaction and does not abort the batch; process exit code is 1 if any document failed.
...
|---|---|
| Livre governance class: configured zero-stage route, submit auto-approves in-tx (both subjects); config-first universal, no route-less profile permitted for any class | [`wiki/decisions/0087-livre-configured-no-approval-route.md`](../decisions/0087-livre-configured-no-approval-route.md) — supersedes the livre arm of ADR 0081, extends ADR 0086/0082/0085 |
...
| Release generation | ADR 0085's unit of release state (`public.release_generations`) — one row per attempt at publishing a specific `(approval_instance, revision, frozen_content_hash)`; identity is the compound key `ReleaseCoordinator.Evaluate` re-derives on every trigger, never trusted from enqueue time |
| Readiness hold | A release generation's `hold_reason`/`hold_detail` state while `ReleaseCoordinator.Evaluate` has not yet released it — one of `awaiting_approval_fact`, `materializing`, `awaiting_effective_date`, `supersede_conflict`, `plan_invalid`, `failed` (`domain.Hold*` constants) |
| Release-hold reconciler | ADR 0085 Stage C W2: the `jobs/release_hold_reconciler` River periodic job (15 min) that alerts (ADR 0068, never auto-heals) on generations stuck in readiness hold past 30 min, via the published `ReleaseHoldReader` read-port |
| Release backfill | ADR 0085 Stage C's `scripts/release-backfill` one-shot tool (§8.9): repairs a legacy-approved document's release prerequisites (approval fact, materialization, evaluation) without re-approving it or trusting legacy artifact pointers |
| Auto-approve / zero-stage route | [ADR 0087](../decisions/0087-livre-configured-no-approval-route.md): a livre profile's active route is required but must carry exactly zero stages (`domain.Route.Validate`, `RoutePolicyNoApprovalStages`); submitting a subject bound to it completes the instance in the SAME transaction as submit (`SubmitService.autoApprove` → `Instance.AutoApprove`) — an ordinary approved instance with an `approval_auto_approved` governance event and an empty signoff set, never a bypass of the approval kernel |
| `RoutePolicy` (taxonomy-published) | The 3-valued published consequence of a profile's `GovernanceClass`, consumed via `ProfilePolicyReader` (ADR 0081): `require_approval_stage` (controlado), `approval_optional` (simples), `no_approval_stages` (livre, ADR 0087 — renamed from the pre-0087 `no_route_permitted`) |
...
| Release readiness incomplete (ADR 0085) | Evaluation holds; document stays `approved`/`scheduled` — hold reason `materializing` (artifact fact missing, `domain.HoldMaterializing`) | `ReleaseCoordinator.Evaluate` (`release_coordinator.go:123`) records the hold via `recordHold`/`touchEvaluatedTx` (`:766`) | No operator action; `RecordArtifactFactTx` (`release_facts.go:302`) re-enqueues evaluation when the missing DOCX/PDF fact lands |
| Planned effective date in the future (ADR 0085) | Evaluation schedules, not releases — hold reason `awaiting_effective_date` (`domain.HoldAwaitingEffectiveDate`) | `EvaluateRelease` (`domain/release.go:180-182`) returns `ReleaseActionSchedule` | Expected; the coordinator re-enqueues its own effective-date timer (`ReleaseEvaluateArgs`, `ScheduledAt=planned_effective_from`) — fires automatically, no operator action |
| Supersession target moved out from under a winning release | Hold reason `supersede_conflict` (`domain.HoldSupersedeConflict`) on rollback | `discoverSupersedeTargets`/in-tx target lock inside `releaseTx` (`release_coordinator.go:426,452`) | Coordinator re-evaluates on the next trigger; escalate only if the hold persists across retries |
| Lost release CAS (concurrent evaluation won) | Benign no-op, not an error — `IsLostReleaseCAS(err)` true | `errLostReleaseCAS` (`release_coordinator.go:534`), caught inside `Evaluate` (`:157`) | Expected under concurrent triggers (fact-landed + effective-date timer racing); no operator action |
| `metaldocs-jobs` host down | Release evaluations never fire — approved/scheduled documents never publish | `metaldocs-jobs` `/healthz`; River queue depth on the `temporal` queue | Restart jobs host; River persists pending `release_evaluate` jobs in PG |
...
| Release generation stuck in readiness hold past 30 min (ADR 0085 Stage C W2) | Lost fact, dead-lettered consumer, or dead timer — the generation never reaches `released` on its own | `jobs/release_hold_reconciler` 15-min periodic job reads via `ReleaseHoldReader.ListStuckHolds`; emits one `release.generation.stuck_alert` governance event + `slog.WarnContext` per stuck generation (alert-only, ADR 0068 — never re-enqueues evaluation, never mutates) | Operator investigates via `approval_instance_id`/`hold_reason` in the alert payload, then manually re-triggers the coordinator evaluation (e.g. via the one-shot `scripts/release-backfill` repair path for a legacy-approved document, or by fixing the underlying stuck consumer) |
...
| Signoff attempted on unfrozen instance (F6) | `ErrContentHashMismatch` (412) from the signoff call site | `decision_service.go` maps `ErrNoActiveContentHash` (`LoadFrozenContentHash`) → `ErrContentHashMismatch` | Indicates a code path reached signoff without ever freezing — fix-forward, do not bypass the guard. (ADR 0085: publish itself no longer has a separate fail-closed hash check — `frozen_content_hash` is baked into the release generation's identity key, see §8.8) |
...
- 2026-08-04 - [ADR 0087](../decisions/0087-livre-configured-no-approval-route.md) wiki-sync: livre governance class flips from "no active route permitted" to "active route required, ZERO stages, submit auto-approves in-tx" for both subjects (documents + templates). `RoutePolicyNoRoutePermitted` renamed `RoutePolicyNoApprovalStages`; `Route.Validate(policy)` stage-count check is now policy-homed (livre=0, controlado≥1 approval stage, simples≥1 stage) rather than a blanket structural floor. New shared terminal-approval helpers `document_terminal_approval.go`/`template_terminal_approval.go` replace duplicated completion logic in `decision_service.go`/`review_verdict_service.go` and are now also the auto-approve path's completion leg (closes the F-QA4-14 defect class for a third route). New event `approval_auto_approved`. DB: forward migration `0316` replaces `assert_route_shape` — zero-stage is DB-exclusive to livre, controlado/simples now require ≥1 stage at the DB line (previously app-only). Documents resolver: `GetFinalApprovalDate` fails closed (`ErrNoApprovalDate`) instead of fabricating `now()`; `GetApprovers` renders the auto-approval row only when pinned stage-instance count is 0. §header/§5.2/§9/§12/Cross-links updated.
- 2026-07-29 - ADR 0085 Stage C wiki-sync: new published read-port `ReleaseHoldReader` (`domain/release_hold_port.go`) + `ReleaseHoldReaderPG` adapter, consumed by the `jobs` module's new `release_hold_reconciler` River periodic job (15-min alert-only reconciliation sweep, ADR 0068) — job detail lives in `wiki/modules/jobs.md`, this doc documents only the published port + the Failure-modes row. New `scripts/release-backfill` one-shot in-flight-disposition tool documented in new §8.9. §5.2 gained the two new symbols; §12 Glossary gained 4 entries (release generation, readiness hold, release-hold reconciler, release backfill); Failure modes gained the stuck-hold row.
- 2026-07-28 - ADR 0085 Stage B wiki-sync (release coordinator replaces human-invoked publish): `POST /documents/{id}/publish`, `/schedule-publish`, `/supersede` and their Go files (`publish_service.go`, `scheduler_service.go`, `supersede_service.go`, `http/publish_handler.go`, `http/supersede_handler.go`) DELETED; River job kind `scheduled_publish_cutover` DELETED; capability `document.publish` RETIRED (migration `0311`) — `document.supersede` SURVIVES, re-homed to submit time (cross-document supersede targets named in the publication plan, checked against the TARGET's area in `submit_service.go`). New surface: `ReleaseCoordinator.Evaluate` (`application/release_coordinator.go:123`) + `RecordArtifactFactTx` (`application/release_facts.go:302`) drive publication off an approval-fact × artifact-fact × effective-date-gate × supersession-head predicate; new River job kind `release_evaluate` (`jobs/release_evaluate_args.go:27`, `ReleaseEvaluateWorker` `jobs/release_evaluate_job.go:23`) on the existing `temporal` queue — no HTTP entry point. §5.1/§5.2/§5.3/Route Truth Table/§3 C4/§8.8/§8 Failure modes updated below; see [ADR 0085](../decisions/0085-release-coordinator-approval-driven-publication.md).
