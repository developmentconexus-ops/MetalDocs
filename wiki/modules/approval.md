...
**Last verified:** 2026-06-01 (P2 consolidation: §3/§5 C4 fragments tagged as module-scoped with pointer to canonical diagrams; added Failure modes section; prior: 2026-05-25 5c + 5d medium sweep sync) | **Owner:** unassigned | **Status:** active | **Maturity:** L2
...
| `render/fanout` outbox dispatch lags | Final signoff committed but PDF not visible for N minutes | `pdf_dispatch_outbox` row count grows; `pdf_outbox_worker` lag metric | Worker drains async; if stuck, see [`render-fanout.md`](render-fanout.md) failure modes |
