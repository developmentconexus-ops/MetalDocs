...
| DELETE | `/api/v1/approval/routes/{id}` | `internal/modules/documents/approval/http/router.go:24` | `wrapper.DeactivateApprovalRoute` â†’ `h.DeactivateRouteHandler` | `/api/v1/approval/routes/{id}` | `deactivateApprovalRoute` | `DeactivateApprovalRoute` | Aligned | Body = `DeactivateRouteRequest {reason}` (wired in PR-2); `Idempotency-Key` + `If-Match: v<N>` required |
...
Source: `_artifacts/02-flow-submit.md`. Two upstream entries: documents-module `finalizeDocument` (`internal/modules/documents/delivery/http/handler.go:435`) and approval-module `SubmitHandler` (`http/submit_handler.go:14`). Both call `application/submit_service.go:43`.
