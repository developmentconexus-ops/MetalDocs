...
- **J1 eligibility (4th layer, signoff-only):** `enforce_signoff_eligibility_trg` (`db/baseline/0001_current_schema.sql:678` function, `db/baseline/0001_current_schema.sql:3744` trigger) â€” `BEFORE INSERT ON approval_signoffs`. Checks `eligible_actor_ids @> actor_user_id` on the parent stage row.
...
| Approval route lifecycle (terminal-on-deactivate, version OCC, in-use guard, capability pin, reason audit) | [`wiki/decisions/0018-approval-route-lifecycle.md`](../decisions/0018-approval-route-lifecycle.md) |
