...
**Last verified:** 2026-06-01 (F-002: document-scoped signoff replay check reordered to sit BETWEEN instance load and state/eligibility validation — cached outcomes now replay even after terminal transition or stage rotation; `stageIDForHash` falls back to empty string when `activeStage == nil` so the replay key stays stable across the transition; clients must rotate `Idempotency-Key` for fresh attempts. Stage-scoped handler unchanged. Prior fix: signoff content-pin source aligned with active-document endpoint — fixes pre-existing 412 `precondition.content_hash_mismatch` on every approval; hardened to fail-closed on missing client pin or NULL server hash) | **Owner:** unassigned | **Status:** active | **Maturity:** L2
...
- **Two-layer idempotency on signoff** â€” driver: Stripe IP-002 + the regulated path can never double-sign. HTTP `PostgresSignoffIdempStore` keyed by `Idempotency-Key` header; repository `ON CONFLICT (approval_instance_id, actor_user_id) DO NOTHING` + field-compare distinguishes replay from `ErrActorAlreadySigned`.
  - **HTTP-layer ordering contract (F-002 fix, 2026-06-01):** the document-scoped `SignoffByDocumentHandler` runs `BeginDocumentReplay` BETWEEN the instance load and state/eligibility validation. Once a `(tenant, actor, route, Idempotency-Key, payloadHash)` tuple has a cached outcome, retries return `was_replay: true` with the cached outcome even if the instance has since transitioned to a terminal state or the active stage has rotated past the original actor. `payloadHash` mixes `docID + inst.ID + stageIDForHash + decision + reason + content_hash`; when `inst.Active() == nil` the handler uses an empty `stageIDForHash` so the key stays stable across the terminal transition. Clients that want a fresh attempt MUST rotate the `Idempotency-Key` — reusing the same key against a now-eligible state will still replay the cached outcome. First calls to a terminal-state instance still return the terminal-state error and do not commit a successful outcome to the slot. Stage-scoped `SignoffHandler` is unaffected by design (path-supplied IDs make its replay-slot computation load-independent).
