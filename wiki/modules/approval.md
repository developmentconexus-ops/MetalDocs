...
**Last verified:** 2026-05-21 (approval generated-wrapper route mount sync) | **Owner:** unassigned | **Status:** active | **Maturity:** L2
...
```mermaid
C4Context
    title System Context â€” approval
...
```mermaid
C4Container
    title Container View â€” approval
...
RFC 9457 `application/problem+json`. `MapErrorToResponse` (`http/errors.go:23`) returns `*problem.Problem`; `WriteError` (`http/errors.go:136-141`) calls `problem.Write(w, prob)`. `http/contracts/errors.go` is now `package contracts` only (emptied Plan 7). `looksLikeValidationError` (`http/errors.go:166`) retained for string-match fallback on unknown validation errors; `domain.ErrNoActiveStage` now has an explicit 409 `state.instance_completed` branch (`http/errors.go:82-84`) â€” T-001 and T-003 both closed.
