...
- Template authoring upstream: [`wiki/modules/templates_v2.md`](templates_v2.md) — §8.7 / §1.1 defines `author_id`, `reviewer_id`, `approver_id` columns as the SoD probe surface; `TemplateAuthorChecker` previously referenced `iam.AuthorizationService` (deleted Plan 4 — T-003 closed); SoD is enforced via approval's own `domain/sod.go:15` `CheckSoD`
