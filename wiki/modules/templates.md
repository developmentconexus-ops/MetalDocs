...
| `internal/modules/templates/application/queries.go:51` | `GetDocxURLCmd` | struct | Presigned GET for stored DOCX (anchor corrected 2026-07-16 — was stale at `:41`; `GetDocxURL` method itself now spans `:61-80` after the object-existence gate was added, see §8.9a) |
...
### 8.9a Blank-template lazy DOCX provisioning — empty-state read-path contract (2026-07-16, Option B)

A newly created blank template commits its version 1 row with `DocxStorageKey` already populated — `CreateTemplate` (`application/create.go:63`) assigns `templateDocxKey(tenantID, templateID, 1)` up front — but **no object exists behind that key in MinIO** until the author's first autosave `Confirm` writes the bytes (`application/autosave.go`, `CommitAutosave`). Between template creation and that first autosave, the version row is in a "key assigned, object absent" state by design (lazy provisioning) — it is not corruption.

`Service.GetDocxURL` (`application/queries.go:61-80`) makes this state observable rather than papering over it:

1. `DocxStorageKey == ""` → `domain.ErrUploadMissing` (pre-existing branch).
2. `DocxStorageKey` set → calls `Presigner.Exists(ctx, key)` (`application/ports.go`; production impl `*objectstore.VerifiedStore.Exists`, `internal/platform/objectstore/verified_store.go:235`, `StatObject`-based).
   - `Exists` returns an error → **fail closed**: the raw error is returned unwrapped (no fallback, no "treat as missing"), surfacing as HTTP 500 via `MapErr`'s default case.
   - `Exists` returns `false` → `domain.ErrUploadMissing`.
   - `Exists` returns `true` → falls through to the existing `PresignGet` call, unchanged happy path.

`domain.ErrUploadMissing` maps to RFC 9457 409 `problem.CodeUploadMissing` (`delivery/http/errors.go:67`, pinned by `delivery/http/errors_test.go:28`). Before this change, the object-absent case fell straight through to `PresignGet` and returned a 200 with a presigned URL to a MinIO object that didn't exist — the browser hit a 404 fetching it. The frontend already tolerates the 409 empty state and renders the blank editor by design, so no FE change was required.

No tenant assertion is added to the `Exists` call — it mirrors the existing, deliberately unguarded `PresignGet` trust model: the key passed in is DB-sourced from a tenant-scoped `GetVersion` read two lines above, not attacker-influenced request input.

Pinned by 5 unit tests in `application/queries_test.go`: `TestGetDocxURL_EmptyStorageKey_ErrUploadMissing`, `TestGetDocxURL_ObjectMissing_ErrUploadMissing`, `TestGetDocxURL_ObjectPresent_ReturnsURL`, `TestGetDocxURL_ExistsError_FailsClosed`, `TestGetDocxURL_CrossTenant`.

**Contract-hygiene gap (not yet closed):** the `GET .../docx-url` route's OpenAPI entry (`api/openapi/v1/openapi.yaml:1731-1753`) still declares only `401`/`403`/`404`/`500` — no `'409'` response is enumerated for this operation (unlike `autosave/commit`, which does declare `'409'`). The 409 shipped here is schema-valid because the `Problem` response envelope is generic/unconstrained per operation and no runtime spec-vs-status validator is wired into the API binary, but the route's declared response surface under-documents actual behavior. An additive contract-lock request to add `'409': $ref '#/components/responses/Conflict'` to this route is pending with the hub as a follow-up spec-hygiene item — deliberately out of scope for this unit.

Charter: `docs/superpowers/analysis/2026-06-29-blank-template-docx-provision-system-impact.md` (Option B ratified; Option A — outbox-driven eager provisioning — explicitly rejected as over-engineering for a read-path honesty fix). Evidence: `docs/superpowers/reports/2026-07-16-unit-blank-docx-b-evidence.md`.
