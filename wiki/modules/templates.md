...
`templates` owns the lifecycle of DOCX-based document templates: authoring (DOCX upload + placeholder schema), versioning, two-stage approval (review â†’ approve), publishing, and obsoletion of the previous published version. Every document instance in MetalDocs is instantiated from a *published* template version â€” `documents` is the downstream consumer that snapshots `placeholder_schema` at finalize.
...
|---|---|---|
| 1 | Tenant isolation of templates and versions | tripwire on every `templates_*` mutation; query-side tenant guard on `GetVersion*` (currently NOT met â€” see T-002) |
...
- Authz: two-tier per `wiki/decisions/0007-two-tier-authz.md` â€” **applied Plan 5** (`WithDB` builder + `authz.Require` in all 6 mutation paths + DB tripwire on `templates_template` + `templates_template_version`; T-001 closed).
...
```mermaid
C4Context
    title System Context â€” templates
...
Inbound interfaces:
- 20 HTTP routes mounted at `/api/v1/templates/*` (8 generated via oapi-codegen, 12 hand-rolled â€” see Â§5.3).
...
- **Approval as state machine on `template_version.status`** â€” driver: ISO 9001 Â§7.5 traceability requirement. Transitions enforced by `domain.TemplateVersion.CanTransition` (`internal/modules/templates/domain/version.go`).
...
### 5.1 Whitebox â€” templates

```mermaid
C4Container
    title Container View â€” templates
    ...
    ContainerDb(db, "Postgres", "Postgres", "templates_template Â· templates_template_version Â· templates_approval_config Â· templates_audit_log")
...
|---|---|---|---|
| `internal/modules/templates/domain/template.go:10` | `VisibilityPublic`, `VisibilityInternal`, `VisibilitySpecific` | const | Template visibility scope enum |
...
| `internal/modules/templates/domain/schemas.go:48` | `CompositionConfig` | struct | (Deprecated per ADR `wiki/decisions/0008-placeholder-fixed-catalog.md` â€” composition removed 2026-04-27; struct retained for backward compat) |
| `internal/modules/templates/domain/approval.go:3` | `ApprovalConfig` | struct | Reviewer/approver role binding per template |
| `internal/modules/templates/domain/approval.go:17` | `CheckSegregation` | func | SoD enforcement (author â‰  reviewer â‰  approver) |
...
| `internal/modules/templates/application/ports.go:41` | `ListFilter` | struct | Filter for `ListTemplates` (tenant, doc_type, areas, visibility, includeArchived) |
...
| `internal/modules/templates/application/lifecycle.go:14..316` | `Service.SubmitForReview`, `Service.Review`, `Service.Approve`, `Service.PublishTemplateVersion`, `Service.ArchiveTemplate` | method | Lifecycle ops; `Approve` (lifecycle.go:159) and `PublishTemplateVersion` (lifecycle.go:265) are two parallel publish paths (see Â§4) |
...
| `internal/modules/templates/delivery/http/errors.go:10` | `MapErr` | func | Domain error â†’ HTTP status + code mapping |
...
Source: `_artifacts/01-surface.md` Â§1a + Â§1b. **All routes still mount under `/api/v1/templates` â€” Plan 2 has not yet flipped this module to `/api/v1/`.**
...
| PUT | `/api/v1/templates/{id}/versions/{n}/draft` | `SaveTemplateDraftV2` | `generated.SaveTemplateDraftV2` | `template.edit` |
| PUT | `/api/v1/templates/{id}/versions/{n}/schema` | â€” (hand-rolled) | `Handler.updateSchemas` | `template.edit` |
| POST | `/api/v1/templates/{id}/versions/{n}/docx-upload-url` | `PresignTemplateDocxUploadUrlV2` | `generated.PresignTemplateDocxUploadUrlV2` | `template.edit` |
| POST | `/api/v1/templates/{id}/versions/{n}/schema-upload-url` | `PresignTemplateSchemaUploadUrlV2` | `generated.PresignTemplateSchemaUploadUrlV2` | `template.edit` |
| POST | `/api/v1/templates/{id}/versions/{n}/autosave/presign` | â€” (hand-rolled) | `Handler.presignAutosave` | `template.edit` |
| POST | `/api/v1/templates/{id}/versions/{n}/autosave/commit` | â€” (hand-rolled) | `Handler.commitAutosave` | `template.edit` |
| POST | `/api/v1/templates/{id}/versions/{n}/submit` | â€” (hand-rolled) | `Handler.submitForReview` | `template.submit` |
| POST | `/api/v1/templates/{id}/versions/{n}/review` | â€” (hand-rolled) | `Handler.review` | `template.approve` |
| POST | `/api/v1/templates/{id}/versions/{n}/approve` | â€” (hand-rolled) | `Handler.approve` | `template.approve` |
| POST | `/api/v1/templates/{id}/versions/{n}/publish` | `PublishTemplateVersionV2` | `generated.PublishTemplateVersionV2` | `template.publish` |
| POST | `/api/v1/templates/{id}/archive` | â€” (hand-rolled) | `Handler.archiveTemplate` | `template.edit` |
| PUT | `/api/v1/templates/{id}/approval-config` | â€” (hand-rolled) | `Handler.upsertApprovalConfig` | `template.edit` |
| GET | `/api/v1/templates/{id}/versions/{n}/docx-url` | â€” (hand-rolled) | `Handler.getDocxURL` | `template.view` |
| GET | `/api/v1/templates/{id}/audit` | â€” (hand-rolled) | `Handler.listAudit` | `template.view` |
| GET | `/api/v1/templates/v2/placeholder-catalog` | â€” (hand-rolled) | `Handler.listPlaceholderCatalog` | (none) |
...
| PUT | `/api/v1/templates/{id}/versions/{n}/draft` | `internal/modules/templates/delivery/http/handler.go:45` | `generated.SaveTemplateDraftV2` | `/api/v1/templates/{id}/versions/{n}/draft` | `saveTemplateDraftV2` | `SaveTemplateDraftV2` | Aligned | Wrapper-mounted |
| POST | `/api/v1/templates/{id}/versions/{n}/publish` | `internal/modules/templates/delivery/http/handler.go:46` | `generated.PublishTemplateVersionV2` | `/api/v1/templates/{id}/versions/{n}/publish` | `publishTemplateVersionV2` | `PublishTemplateVersionV2` | Aligned | Wrapper-mounted |
| POST | `/api/v1/templates/{id}/versions` | `internal/modules/templates/delivery/http/handler.go:48` | `h.createNextVersion` | â€” | â€” | â€” | Spec missing |  |
| PUT | `/api/v1/templates/{id}/versions/{n}/schema` | `internal/modules/templates/delivery/http/handler.go:49` | `h.updateSchemas` | â€” | â€” | â€” | Spec missing |  |
| POST | `/api/v1/templates/{id}/versions/{n}/autosave/presign` | `internal/modules/templates/delivery/http/handler.go:50` | `h.presignAutosave` | â€” | â€” | â€” | Spec missing |  |
| POST | `/api/v1/templates/{id}/versions/{n}/autosave/commit` | `internal/modules/templates/delivery/http/handler.go:51` | `h.commitAutosave` | â€” | â€” | â€” | Spec missing |  |
| POST | `/api/v1/templates/{id}/versions/{n}/submit` | `internal/modules/templates/delivery/http/handler.go:52` | `h.submitForReview` | â€” | â€” | â€” | Spec missing |  |
| POST | `/api/v1/templates/{id}/versions/{n}/review` | `internal/modules/templates/delivery/http/handler.go:53` | `h.review` | â€” | â€” | â€” | Spec missing |  |
| POST | `/api/v1/templates/{id}/versions/{n}/approve` | `internal/modules/templates/delivery/http/handler.go:54` | `h.approve` | â€” | â€” | â€” | Spec missing |  |
| POST | `/api/v1/templates/{id}/archive` | `internal/modules/templates/delivery/http/handler.go:55` | `h.archiveTemplate` | â€” | â€” | â€” | Spec missing |  |
| PUT | `/api/v1/templates/{id}/approval-config` | `internal/modules/templates/delivery/http/handler.go:56` | `h.upsertApprovalConfig` | â€” | â€” | â€” | Spec missing |  |
| GET | `/api/v1/templates/{id}` | `internal/modules/templates/delivery/http/handler.go:58` | `h.getTemplate` | â€” | â€” | â€” | Spec missing |  |
| GET | `/api/v1/templates/{id}/versions/{n}/docx-url` | `internal/modules/templates/delivery/http/handler.go:59` | `h.getDocxURL` | â€” | â€” | â€” | Spec missing |  |
| GET | `/api/v1/templates/{id}/audit` | `internal/modules/templates/delivery/http/handler.go:60` | `h.listAudit` | â€” | â€” | â€” | Spec missing |  |
| GET | `/api/v1/templates/v2/placeholder-catalog` | `internal/modules/templates/delivery/http/handler.go:61` | `h.listPlaceholderCatalog` | â€” | â€” | â€” | Spec missing |  |
...
### 6.1 ListTemplates (read) â€” `GET /api/v1/templates`
...
### 6.2 UpdateSchema (write â€” placeholder catalog enforcement) â€” `PUT /api/v1/templates/{id}/versions/{n}/schema`
...
- Environment / config: `MinioClient`, `MinioBucket`, max object size `25*1024*1024` â€” all consumed at `apps/api/cmd/metaldocs-api/main.go:327`. No env vars or feature flags read inside `internal/modules/templates/**` (per `_artifacts/03-deps.md` Â§4).
...
|---|---|---|
| Tenant isolation | Authn'd user from tenant A calls `GET /api/v1/templates/{id-from-tenant-B}/versions/1` with a known version_id | 404 not found (currently: 200 with the row â€” T-002) |
...
1. **T-001 closed Plan 5** â€” `authz.Require` wired in all 6 mutation paths via `WithDB` builder; tripwire on both templates tables (migration 0188).
...
| Term | Definition |
|---|---|
| Template | A reusable DOCX skeleton bound to a `doc_type_code`, `tenant_id`, and visibility scope. Aggregate root; PK on `templates_template`. |
...
- Taxonomy coupling: `wiki/modules/taxonomy.md` â€” taxonomy's `TemplateVersionChecker` (`infrastructure/template_version_checker.go:11`) READ-joins `templates_template_version` + `templates_template` to verify `IsPublished` when binding a profile's default template; taxonomy Â§3.2 documents this IN-edge
...
- Predecessor wiki stub: `wiki/modules/templates.md` (kebab dash) â€” frontend-heavy; will be retired (see `backlog/templates-refactor.md#R-100`).
