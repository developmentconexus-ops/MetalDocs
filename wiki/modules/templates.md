...
- `internal/modules/templates/repository/postgres.go:88` — GetTemplate double-joins `templates_template_version` (`lv`/`pv` aliases) and scans `pv.revision_number` into the read model via `scanTemplateRead` (`repository/mappers.go:18`)
...
| `internal/modules/templates/repository/postgres.go:42` | `Repository` | struct | Postgres adapter implementing `application.Repository` |
| `internal/modules/templates/repository/postgres.go:48` | `New` | func | Repository constructor |
| `internal/modules/templates/repository/postgres.go:88` | `Repository.GetTemplate` | method | Double-joins `templates_template_version` (`lv`/`pv`) for latest/published refs; returns `*domain.TemplateRead` (ADR 0065) |
| `internal/modules/templates/repository/mappers.go:18` | `scanTemplateRead` | func | Scans the twin-join projection into `domain.TemplateRead` |
