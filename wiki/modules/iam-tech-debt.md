...
- **Surface (at time of closure):** `internal/modules/iam/area_membership/area_membership.go:53,65,77` (free `Grant`/`Revoke`/`List` taking `*sql.Tx`, calling `metaldocs.grant_area_membership` / `revoke_area_membership` SECURITY DEFINER funcs) vs `internal/modules/iam/application/area_membership_service.go:64,120` (`AreaMembershipService.Grant`/`Revoke`) calling `UserAreaRepository.GrantAtomic` (`infrastructure/postgres/user_area_repository.go:185`) with direct DML
- **Observation:** Two implementations of the same use case existed with different semantics. The v2 HTTP route at `/api/v1/iam/area-memberships` (POST) uses the application-service + repo path (artifact 02-flow-grant-membership). The `area_membership/` package was wired into none of the routes registered in `main.go` (per artifact 03 §3 DI touchpoints) — its callers were never in the IAM module. **2026-07-14 update:** `metaldocs.grant_area_membership` itself was DROPPED by migration `db/migrations/0307_drop_dead_grant_area_membership_fn.sql` — the free `area_membership/` Go wrapper's only remaining caller (an integration-only e2e-seed helper) was removed in ROADMAP unit 4.3, and the fn was unsatisfiable by construction (uppercase area_code gate vs. lowercase `area_code_format` CHECK). Sibling `revoke_area_membership` is untouched. The direct-DML `AreaMembershipService` path (still live) never read `metaldocs.actor_id` GUC via SECURITY DEFINER.
...
### T-010 · `auth` module imports `iam/domain.Role` — bidirectional dependency — CLOSED 2026-07-02 (ARC-06)
- **Severity:** minor (closed)
- **Surface (original):** `internal/modules/auth/application/service.go:18`, `internal/modules/auth/delivery/http/middleware.go:10`, `internal/modules/auth/domain/model.go:6`, `internal/modules/auth/infrastructure/memory/repository.go:10` (all import `iamdomain.Role`); IAM imports `auth/domain` from `internal/modules/iam/delivery/http/admin_handler.go:13`
- **Observation (original):** IAM depends on auth (for `ManagedUser` types) AND auth depends on IAM (for `Role` enum). The dependency is non-circular today because IAM-→auth lives in `delivery/http/` (admin handler) and auth-→IAM lives in `domain` and lower. If admin_handler types ever migrate up, the cycle becomes hard. Documented for future structural moves.
- **Resolution (2026-07-02):** `Role` (type + 8 canonical constants + `IsValidRole`/`IsAreaRole`/`AreaRoles`) moved verbatim to new neutral package `internal/platform/iamtypes` (no dependency on IAM's application/infrastructure layers or on auth — a pure leaf value type, correctly placed under `internal/platform` per the modules→platform legal dependency direction; `tools/cilint/internal/analyzers/platformboundary.go`'s reverse-direction guard (platform→modules) does not apply and required no allowlist change). `iam/domain.Role` is now `type Role = iamtypes.Role` (full type alias, not a copy) plus const-forwarding, so all existing `iamdomain.Role` / `domain.Role` call sites across the repo (~65 files, 78+ references spanning documents/controlleddocuments/templates/taxonomy/etc.) keep compiling with zero behavior change. `internal/modules/auth/**` now imports `iamtypes` directly for `Role`/`RoleSystemAdmin`/`RoleViewer`/`RoleEditor`/`IsValidRole` and no longer references `iamdomain.Role` anywhere (grep-verified: zero matches). `ErrInvalidRole`, `ErrUserNotFound`, etc. and the `RoleProvider`/`RoleAdminRepository`/`Capability`/`LoginContextPort` ports were explicitly NOT moved — out of this finding's "move the Role enum" scope; see `wiki/modules/auth-tech-debt.md` T-007 residual note for the coupling that remains. Verification: `go build`/`go vet`/`go test -count=1` clean on `internal/modules/iam/...`, `internal/modules/auth/...`, `internal/platform/...`; `gofmt -l` clean on all touched files; `go run ./tools/cilint` emits zero findings against the touched paths.
- **Evidence:** `_artifacts/03-deps.md` §1 + §2 (original); grep proof + test run 2026-07-02 (this closure).
