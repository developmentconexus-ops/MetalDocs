...
**Last verified:** 2026-08-04 ([ADR 0087](../decisions/0087-livre-configured-no-approval-route.md) — livre governance class flips from "no active route permitted" to "active route required, ZERO stages, submit auto-approves": `GovernanceClass.RoutePolicy()` (`internal/modules/taxonomy/domain/governance_class.go:64`) now returns `RoutePolicyNoApprovalStages` (`"no_approval_stages"`, `:55`) for `livre` — the pre-0087 `RoutePolicyNoRoutePermitted` value is DELETED, replaced (not aliased; no compat shim). `GovernanceClass` itself, `Reclassify`, and the DB shape (`document_profiles.governance_class` column, CHECK, `ProfileService.Reclassify` as sole class-write path) are UNCHANGED by this ADR — only the derived `RoutePolicy` published value and the `assert_route_shape()` DB helper's livre arm changed; the consuming approval module (`documents/approval` in this doc's stale name — see note below) never sees `GovernanceClass` itself, only the 3-valued policy (ADR 0081 invariant preserved). Migration `0295`'s `assert_route_shape()`/`enforce_profile_route_policy()` bidirectional DEFERRABLE triggers are REPLACED (`CREATE OR REPLACE`, same function names, same trigger bindings) by forward migration `db/migrations/0316_livre_zero_stage_route.sql` — livre arm flips from "RAISE always" to "active route MUST carry zero stages"; zero-stage becomes DB-EXCLUSIVE to livre (controlado/simples now require ≥1 stage at the DB line, previously app-only). Full detail lives in [`wiki/modules/approval.md`](approval.md) (the consuming module); this doc's stake is the `RoutePolicy()` derivation + the migration anchor. See ADR 0087 §3/§4. Prior: 2026-07-10 (G1 per-profile signature policy — ROADMAP 2.1, ADR 0081: taxonomy now OWNS a `GovernanceClass` enum {controlado,simples,livre} on `document_profiles` + a pure `RoutePolicy()` derivation published to `documents/approval`; class writes go ONLY through the new `ProfileService.Reclassify` use-case — the generic `Update` path never touches `governance_class`; DB migration 0295 adds the column + a shared `assert_route_shape()` helper behind DEFERRABLE INITIALLY DEFERRED constraint triggers on `approval_route_stages`, `approval_routes`, and `document_profiles`; reclassify conflict surfaces as `ErrClassChangeRouteConflict` → HTTP 409. Prior: 2026-06-12 Wave 2.12 sync — DBGovernanceLogger + governance_logger.go DELETED; AuditGovernanceAdapter is the sole GovernanceLogger; FamilyTx embeds db.Tx (Unwrap chain gone); module.go panics fail-loud on nil AuditWriter; domain no longer imports database/sql; two new cilint guards: nosqltxindomain + nodualmode; prior Wave 2 sync: GovernanceLogger.LogTx/RecordTx in-tx governance writes; CreateTx on repos; AreaService.SetParent DELETED) | **Owner:** unassigned | **Status:** active (intrinsic gaps; see §11) | **Maturity:** L3
...
> - `internal/modules/taxonomy/domain/governance_class.go:64` â€” `GovernanceClass` enum {controlado,simples,livre} + `RoutePolicy()` derivation (G1, ADR 0081); domain-pure, no SQL; `RoutePolicyNoApprovalStages` (`"no_approval_stages"`, `:55`) is the livre value since [ADR 0087](../decisions/0087-livre-configured-no-approval-route.md) — replaces the deleted `RoutePolicyNoRoutePermitted`
...
> - `db/migrations/0295_profile_governance_class.sql` â€” column + CHECK + original `assert_route_shape()` + `enforce_profile_route_policy()` bidirectional DEFERRABLE constraint triggers (last line); **superseded (not deleted, forward-only) by** `db/migrations/0316_livre_zero_stage_route.sql`, which `CREATE OR REPLACE`s `assert_route_shape()` for the ADR 0087 livre-arm flip — same trigger bindings, same DEFERRABLE semantics, function body only
...
|---|---|
| `GovernanceClass` → `RoutePolicy` published derivation (G1); livre = configured zero-stage route, not route-less (ADR 0087, supersedes the livre arm of 0081) | [`wiki/decisions/0081-per-profile-signature-policy.md`](../decisions/0081-per-profile-signature-policy.md), [`wiki/decisions/0087-livre-configured-no-approval-route.md`](../decisions/0087-livre-configured-no-approval-route.md) |
...
- 2026-08-04 - [ADR 0087](../decisions/0087-livre-configured-no-approval-route.md) sync: `GovernanceClass.RoutePolicy()`'s livre value renamed `RoutePolicyNoApprovalStages` (`"no_approval_stages"`, was `RoutePolicyNoRoutePermitted`/`"no_route_permitted"`) — livre now means "active route required, zero stages" instead of "no route permitted"; `GovernanceClass` enum, `Reclassify`, and the `document_profiles.governance_class` column are unchanged. Forward migration `0316` supersedes migration `0295`'s `assert_route_shape()` livre arm (DB line). Full consuming-side detail (submit auto-approve, terminal-approval helpers, new event) lives in `wiki/modules/approval.md`, not here — taxonomy's stake is the derivation + migration anchor only. Header/Key files updated.
