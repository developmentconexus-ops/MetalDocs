...
| File | Exports |
|---|---|
| `domain/model.go` | `Role` type + 8 consts (`RoleApprover`, `RoleAreaAdmin`, `RoleAuthor`, `RoleEditor`, `RoleQmsAdmin`, `RoleSigner`, `RoleSystemAdmin`, `RoleViewer`) — 3 area-only roles (`area_admin`, `qms_admin`, `signer`) added in PR-4; `Capability` type + **35 typed consts as of ADR 0069/M6** (2026-07-04; this row's enumeration below is the 29-const snapshot from this doc's last Key-files verification and predates ADR 0069's `CapDocumentReview` addition — see `iam/domain/model.go:89` and ADR 0069 for the current registry) (`CapDocument{View,Create,Edit,Submit,Signoff,Publish,Obsolete,Supersede}`, `CapTemplate{View,Create,Edit,Submit,Review,Approve,Publish,Archive}`, `CapControlledDocument{Create,Obsolete,Supersede}`, `CapTaxonomy{View,Manage}`, `CapMembership{View,Manage}`, `CapRouteManage`, `CapUser{View,Manage}`, `CapMetricsView`, `CapAuditRead`, `CapSessionManage`) — Plan 4 collapsed the dual namespace; Plan 5 added `CapControlledDocumentObsolete`/`CapControlledDocumentSupersede`; ADR 0022 Phase 10 minimized registry 33→29 (merged 4 redundant Phase-8 caps back into canonical caps) |
...
| New capability `document.review` (ScopeTenant); registry 34→35; gates the eQMS mark-reviewed workflow; `documents/UPDATE` tripwire arm additively widened (migration 0275) | [`wiki/decisions/0069-document-periodic-review-and-reason-for-change.md`](../decisions/0069-document-periodic-review-and-reason-for-change.md) |
...
| Term | Definition |
|---|---|
| Capability | Fine-grained permission string (e.g. `document.view`) consumed by tier-1 CanDo. Typed as `iamdomain.Capability`; **35 consts in `model.go` as of ADR 0069/M6** (2026-07-04) — `domain/model.go:13-92`, including `CapDocumentReview` (`model.go:89`, `ScopeTenant`, gates the mark-reviewed workflow); prior history: ADR 0022 Phase 10 minimized 33→29). |
...
| SECURITY DEFINER | Postgres function attribute that executes with the function owner's privileges, reading `metaldocs.actor_id` GUC. Used by `metaldocs.revoke_area_membership` (migration 0137). Sibling `metaldocs.grant_area_membership` was DROPPED by migration `db/migrations/0307_drop_dead_grant_area_membership_fn.sql` (2026-07-14) — zero product-runtime callers (only e2e-seed + integration-test infra, both removed by ROADMAP unit 4.3) and unsatisfiable by construction (uppercase area_code gate contradicted the lowercase `area_code_format` CHECK on `metaldocs.document_process_areas`). The Go `area_membership/` wrapper was deleted in Plan 4. |
