...
> - `internal/modules/iam/infrastructure/postgres/role_provider.go:28` — `RolesByUserID` (single LEFT JOIN, 1 round trip — Wave 2 collapsed from 2 queries); `RolesByUserIDs` at `:75` (batch `= ANY($1)` for N users); `UserActiveInTenant` at `:119` (EXISTS point-lookup for TenantMemberChecker)
> - `internal/modules/iam/domain/port.go:6` — `RoleProvider` interface now includes `RolesByUserIDs(ctx, tenantID, []string) (map[string][]Role, error)` batch method (Wave 2)
...
> - `internal/modules/iam/application/people_service.go:162` — `TenantMemberChecker` port; `WithTenantMemberChecker` setter at `:242`; `VerifyUserInTenant` at `:605` (EXISTS via checker when wired, fallback full-list scan in dev/memory mode)
...
- The membership service's `MembershipGovernanceLogger` argument is wired with **`nil`** (`main.go:325`) â€” governance log emission is dead in this binary. Recorded as T-007.
...
2. T-007 â€” `MembershipGovernanceLogger` wired with `nil` in `main.go:325`; grant/revoke produce no governance log on the application-service path while the SECURITY DEFINER path still does. Major.
