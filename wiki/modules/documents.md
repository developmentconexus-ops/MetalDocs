...
**Last verified:** 2026-07-29 (ADR 0085 Stage C wiki-sync: `GetDocument` (`internal/modules/documents/infrastructure/repository.go:302`) now composes the release-coordinator's readiness-hold state via a `LEFT JOIN LATERAL` over the approval-owned `public.release_generations` — new `domain.ReleaseProjection` type, new required-nullable `DocumentDetailResponse.release` wire field (new §8.9b); new `FreezeService.RepairMaterialization` (`freeze_service.go:249`) backs the ADR 0085 Stage C `scripts/release-backfill` one-shot tool (documented in `wiki/modules/approval.md` §8.9, not duplicated here) — added to §5.2 public surface. §5.2/§5.3/Route Truth Table/§9 updated below.) | prior: 2026-07-28 (ADR 0085 Stage B wiki-sync: `POST /documents/{id}/publish`, `/schedule-publish` and `/supersede` were deleted with their services and the `scheduled_publish_cutover` River job — the §5.3 HTTP-operations table drops those three rows; the §"State transitions" `approved→published` and `published→superseded` rows are rewritten to the release-coordinator model (coordinator-only transitions driven by the readiness predicate, system principal, no publish endpoint; supersession happens inside the winning release tx, with `document.supersede` re-homed to submit-time cross-document plan authorization); the glossary drops the `document.publish` contrast on `document.review` and gains an explicit retired-capability row pointing at migration 0311) | prior: 2026-07-12 (M3 approval-kernel-extraction wiki-sync: `internal/modules/documents/approval/` promoted to top-level `internal/modules/approval/` — [ADR 0082](../decisions/0082-approval-kernel-extraction.md); §4 Solution Strategy approval bullet rewritten (was stale: claimed the top-level path "does not exist"); §5.2/§5.3/§8.9a/§9 anchors re-pointed to `internal/modules/approval/...` with re-verified line numbers; found and flagged a real drift — `ComputeIdempotencyKey` (previously documented as the submit-path deterministic key derivation) no longer exists in the tree, `submit_service.go` now threads the raw client `Idempotency-Key` header directly; not fixed, out of M3's scope, flagged inline) | prior: 2026-07-06 (F9.4 doc-truth pass: F9.5 `repository/`→`infrastructure/` rename anchors fixed across §5.2 Public surface table — `repository.go`/`snapshot_repository.go` (now `WriteFreeze` family, was `WriteSnapshot`)/approval `postgres_approval_repository.go` (`InsertInstance:41`); §M6 review-port anchors `review_due_reader.go`/`review_surface_writer.go` re-pointed with fresh line numbers; §M6 `GetDocument`/`ListDocumentsPaginated` column-SELECT line refs flagged **verify** (dir fixed, exact offsets not independently re-confirmed this pass)) | prior: 2026-07-03 (ADR 0065 cross-link: §8.8 notes the coupled `current_revision_id`/`revision_version`/`revision_number` triple is the same parallel-scalars shape templates just replaced with `TemplateVersionRef` — a `DocumentRevisionRef` nested value object is Plan 2, deferred pre-v1, not yet applied; no code changed in this module) | prior: 2026-07-01 (CON-01 grade-A simplification: `POST /documents/{id}/finalize` is now a deprecated convenience wrapper over canonical `POST /documents/{id}/submit` (DEC-01) — `openapi.yaml` marks it `deprecated: true`, `If-Match` is now mandatory (OCC parity with submit), and tier-1 capability changed `CapDocumentSignoff` → `CapDocumentSubmit` (finalize never records a signoff); submit's own OpenAPI request/response schemas were documented for the first time (`SubmitDocumentRequest`/`SubmitDocumentResponse`); FE editor call site stays on finalize with If-Match wired in — RouteID/ContentHash are server-resolved-only, no FE equivalent exists at that call site) | prior: 2026-06-30 (ADR 0052 + ADR 0053 — documents now render through the shared controlled-artifact view layer (`ArtifactDetailView` / `ArtifactApprovalScreen`); approval cancel flow uses `CancelInstanceDialog` (replaces `window.prompt`); `resolveOwnerDisplay` + `mapApprovalChain` extracted to shared helpers; parity with templates manual versioning confirmed; prior: 2026-06-15 (M4/F4.1 — port ADR 0029 cross-link: documents/approval now consume `UserDisplayNameReader` port from iam for cross-module display-name reads; direct `iam_users.display_name` reaches removed) | **Prior:** 2026-06-14 (eigenpal vendor path updated to third_party/eigenpal/; Wave 2.12: db==nil branches removed from documents service (single-mode); fillin_service.go authz-bypass (NewFillInServiceNoAuthz + if s.db != nil) eliminated — requireDocEditDraft now unconditional at fillin_service.go:94; freeze_service.go retains ADR-0015 optional-tx-enlistment pattern annotated //cilint:allow-dualmode at lines 178/314/379 (deferred); approval reauth uses Postgres limiter unconditionally. Prior Wave 2: ForceReleaseSession+Archive audit writes in-tx WriteTx; GetFinalizePrereqs extracted; autosave/export routes rate-limited. Prior: Stage-1 backend audit drift patch: §6.2 renameDocument response corrected 204→200+body; §8.4 pagination corrected to keyset cursor, limit param default 20 cap 100, response {items,page:{next_cursor,has_more},total}; T-010 stale "not mounted" claim corrected to FLAG-03 adapter-discards-params; prior: 2026-06-09 (std-execution Family 5 path-param snake_case: document-comment routes `{libraryID}`→`{library_id}`; prior: 2026-06-08 Phase F F4/F8: handler.go + repository.go line anchors updated for keyset cursor migration and dead-code removal; prior: Phase E1 casing big-bang: `form_data_json`, `revision_number`, `current_revision_*` wire keys updated to snake_case; prior: 2026-06-01) (P2 consolidation: §3/§5 C4 fragments tagged as module-scoped with pointer to canonical diagrams; added Failure modes section) | prior: 2026-05-29 (QA `qa/documents-distribution`: `DocumentDistributionPage` wires real document identity via `useDocumentDetailQuery` (Code, RevisionNumber, Name) at honest surfaces (hero breadcrumb, hero badges, props-driven `DocRefCard`, em-breve banner). Mock distribution UI (KPIStrip / DonutCard / DistributionFacts / CoverageByArea / TimelineCard / RecipientsCard / `lib/distributionMeta.ts`) preserved as design scaffolding for unbuilt fanout/read-tracking feature — every illustrative section wrapped in `IllustrativeBlock` with `aria-hidden="true"` + `Dados ilustrativos · Em breve` watermark + `pointer-events:none` + saturate filter + diagonal-stripe overlay. `role="note"` banner above scaffolding names the real document and states numbers are illustrative. 4 hero CTAs `aria-disabled="true" title="Em breve"`. Preview proof on `PO-RH-002` / `REV00` / `DC_Template_Descricao_Cargo`: identity correct on every honest surface, 5 watermarks aria-hidden, only `GET /api/v1/documents/:id` called — zero phantom endpoints. Fanout + read-tracking Go module remains hard-stop / out of scope — see `backlog/distribuicao.md`. Previous: 2026-05-29 QA `qa/documents-detail` follow-up wired `ProcessAreaCodeSnapshot` + `ProfileCodeSnapshot` into `DocumentPublishedPage` via `useAreasQuery` + `useProfilesQuery` (5 hardcoded "—" sites replaced); removed mock `PLACEHOLDER_RELATED` + `PLACEHOLDER_COMMENTS` from §04/§05 → honest em-breve; fixed misleading status badge + wrong Iniciar-revisão title; tsc clean, 19/19 vitest. Previous: 2026-05-29 `qa/documents-editor` ambiguous-content_hash JOIN fix.) | **Owner:** unassigned | **Status:** active | **Maturity:** L3
...
- **Approval kernel consumed as a published-surface dependency** â€” driver: cohesion between document state transitions and the approval-instance + signoff lifecycle, revisited by [ADR 0082](../decisions/0082-approval-kernel-extraction.md) (M3, 2026-07-12). `internal/modules/documents/approval/` was promoted to the top-level bounded context `internal/modules/approval/` (the 15th module) — a pure relocate (no behavior change). `documents` now consumes it exactly like any other module: only its published `domain`/`application`/`api` layers, never its `http`/`infrastructure` internals (stricter than the retired nested-family boundary-guard exception). The kernel itself generalized to `(subject_kind, subject_key)` keying so it can also govern `templates` version approval — see [`wiki/modules/approval.md`](approval.md).
...
| `internal/modules/documents/application/freeze_service.go:249` | `FreezeService.RepairMaterialization` | method | **ADR 0085 Stage C.** Pin-equivalent repair (not `Pin` — the freeze is already final, re-running it would recompute a `values_hash` the approver never signed): re-enqueues materialization dispatch against a given release generation for an already-pinned document. Sole caller is `scripts/release-backfill`'s in-flight-disposition tool (`wiki/modules/approval.md` §8.9) |
| `internal/modules/documents/infrastructure/repository.go:302` | `Repository.GetDocument` | method | **ADR 0085 Stage C.** Single document-detail read-model query; folds in the readiness-hold projection via `LEFT JOIN LATERAL` over the approval-owned `public.release_generations` (latest `generation_seq`), same read-model-composition precedent as `active_instance_reader.go`. See §8.9a below |
...
| `internal/modules/approval/application/submit_service.go:67` | `SubmitService.SubmitRevisionForReview` | func | Now a thin wrapper over the subject-generic kernel (M3, [ADR 0082](../decisions/0082-approval-kernel-extraction.md)); document subject is `Subject{Kind: "document", Key: profile_code}` |
| `internal/modules/approval/application/decision_service.go:248` | `DecisionService.RecordSignoff` | func | Records one signoff vote — `approve` or `reject` only (`domain.Decision`, `approval/domain/signoff.go:18-19`). Publication/supersession are NOT decisions: since [ADR 0085](../decisions/0085-release-coordinator-approval-driven-publication.md) Stage B they are the release coordinator's reaction to the approval fact this call writes. |
...
| `internal/modules/documents/infrastructure/repository.go:1769` | `GetFinalizePrereqs` | func | **Wave 2**: inline finalize SQL extracted from handler to repository; returns `(FinalizePrereqs, error)`; domain sentinels: `ErrDocumentNotFound`, `ErrNotDraftRevision`, `ErrRevisionMissing` |
...
| `internal/modules/documents/infrastructure/repository.go:125` | `CreateDocumentTx` impl | func | Tx-scoped CD+document insert (ADR 0011); asserts `document.create` before INSERT and `document.edit` before pointer/snapshot UPDATEs |
| `internal/modules/documents/infrastructure/repository.go:330` | `UpdateDocumentName` | func | UPDATE inside tx with `authz.Require(CapDocumentEdit)` (T-003 closed Plan 5) |
| `internal/modules/documents/infrastructure/repository.go:1655` | `MarkArchived` | func | Archive UPDATE inside tx with `authz.Require(CapDocumentEdit)` and non-zero `RowsAffected` enforcement |
| `internal/modules/documents/infrastructure/repository.go:1693` | `Unarchive` | func | Archive reversal UPDATE inside tx with `authz.Require(CapDocumentEdit)` and non-zero `RowsAffected` enforcement |
| `internal/modules/documents/infrastructure/snapshot_repository.go:100` | `SnapshotRepository.WriteFreeze` family | funcs | Snapshot/freeze/final artifact UPDATEs enforce non-zero `RowsAffected` via `requireRowsAffected` (:43) |
| `internal/modules/documents/infrastructure/repository.go:524` | `ListDocumentsPaginated` | func | Keyset cursor (FD-2); returns (items, hasMore); pageSize clamped to 100 |
| `internal/modules/documents/infrastructure/repository.go:600` | `CountDocuments` | func | Sibling COUNT(*) for list |
| `internal/modules/approval/infrastructure/postgres_approval_repository.go:41` | `InsertInstance` | func | Tripwire-gated INSERT into `approval_instances`; tripwire arm is now subject-discriminated (`document.submit`/`template.submit` via `NEW.subject_kind`, migration `0299`, [ADR 0083](../decisions/0083-subject-discriminated-capability-tripwire.md)) |
| `internal/modules/approval/application/submit_service.go:81` | client `Idempotency-Key` trim | inline | **Contradiction found (M3 pass):** the previously-documented `ComputeIdempotencyKey` deterministic-derivation function no longer exists in the tree — `submit_service.go` now threads the raw client `Idempotency-Key` header (trimmed) straight into `approval_instances.idempotency_key`; flagged as runtime-truth drift, not verified as in-scope for M3, not fixed here |
...
Routes registered in `internal/modules/documents/delivery/http/handler.go` and `internal/modules/approval/http/router.go` (approval is now a top-level module, [ADR 0082](../decisions/0082-approval-kernel-extraction.md); full route truth table lives in [`wiki/modules/approval.md`](approval.md) §5.3, not duplicated here). **OperationID** column reflects current spec (`api/openapi/v1/openapi.yaml`); `â€”` = no spec op.
...
| POST | `/api/v1/documents/{id}/finalize` **(deprecated, CON-01 — use `/submit`)** | `finalizeDocument` | `Handler.finalizeDocument` (`handler.go:435`) | role + ownership + tier-2 `authz.Require(string(iamdomain.CapDocumentSubmit), areaCode)`; tier-1 gates on `CapDocumentSubmit` |
...
| GET | `/api/v1/documents/{id}` | `handler.go:116` (func `:316`) | `h.getDocument` | `/api/v1/documents/{id}` | `getDocument` | `GetDocument` | Aligned | 200 via `toDocumentDetailResponse`; `form_data_json` as `json.RawMessage`, validated, defaults to `{}`; **ADR 0085 Stage C:** response gains required-nullable `release` (`DocumentReleaseProjection`) — §8.9b |
...
| GET | `/api/v1/documents/{id}/revisions/{rid}/url` | `handler.go:135` (func `:953`) | `h.signedRevisionURL` | `/api/v1/documents/{id}/revisions/{rid}/url` | — | `GetApiV2DocumentsIdRevisionsRidUrl` | Aligned | Returns `200` + `RevisionUrlResponse {url}` (M9/F9.3 aligned spec `302`→`200` to the FE consumer contract); rate-limit variant at `handler.go:171` |
...
**Deprecated (CON-01, 2026-07-01):** `POST /documents/{id}/finalize` is now a
deprecated convenience wrapper — `openapi.yaml` marks the operation
`deprecated: true`. `POST /documents/{id}/submit` is canonical (DEC-01) and
should be used by new integrations; it takes `route_id`/`content_hash`
explicitly instead of resolving them server-side. Finalize now also requires
`If-Match` (mandatory, OCC parity with submit) and gates on `CapDocumentSubmit`
(previously `CapDocumentSignoff`, which contradicted capability semantics since
finalize never records a signoff). The frontend document editor
(`DocumentEditorPage.tsx`) still calls finalize because its call site has no
route lookup and no access to the server's canonical stored content hash — full
migration to submit would require a new endpoint and is out of scope for
CON-01.
...
| approved | scheduled | **no endpoint** — release coordinator ([ADR 0085](../decisions/0085-release-coordinator-approval-driven-publication.md)), `release_evaluate` River job | — (system principal, no human capability) | Readiness predicate = approval fact × artifact facts (DOCX+PDF) × effective-date gate × supersession head. A future `planned_effective_from` arms a timer and parks the document at `scheduled` |
| approved\|scheduled | published | **no endpoint** — release coordinator ([ADR 0085](../decisions/0085-release-coordinator-approval-driven-publication.md)), `release_evaluate` River job | — (system principal, no human capability) | Same predicate, effective-date gate now open: `documents.status='published'` + ACTUAL `effective_from` + `release_generations.released_at`, one governance + one lifecycle event, all in the winning release tx (source CAS ⇒ exactly one winner) |
| published | superseded | **no endpoint** — happens inside the winning release tx | `document.supersede` at **submit** time, on the **target's** process area, for a plan-named CROSS-document target only | Same-document supersession is implicit (the controlled document's published head is discovered by the coordinator); cross-document targets are named in the submission plan and authorized when the plan is submitted. Each superseded target gets its own governance + lifecycle event |
...
- **Read surface:** `GetDocument` (`internal/modules/documents/infrastructure/repository.go:285` (**verify** — line not independently re-confirmed against the exact SELECT list this pass)) and `ListDocumentsPaginated` (`infrastructure/repository.go:524` (**verify** — column-list line offsets not re-confirmed)) now SELECT `effective_from, effective_to, review_due_at, last_reviewed_at`; `domain.Document` carries all four; `DocumentSummary`/`DocumentDetailResponse` DTOs expose them. `GET /api/v1/documents?review_due=true` filters to published, currently-effective, `review_due_at <= now()` documents (`delivery/http/handler.go:428`, repository query at `infrastructure/repository.go:488-495` (**verify**)).
- **Two published read/write ports** (documents-owned, consumed by `jobs`, never raw SQL on `public.documents` from the caller side):
  - `ReviewDueReader.ListDueForReview(ctx, tx, now, limit)` — `internal/modules/documents/domain/review_due_port.go`; Postgres adapter `ReviewDueReaderPG` at `internal/modules/documents/infrastructure/review_due_reader.go:62`. Tenant scoping via RLS only (caller must have seeded tx tenant identity).
  - `ReviewSurfaceWriter.MarkSurfaced(ctx, tx, now)` — `internal/modules/documents/domain/review_surface_port.go`; adapter `ReviewSurfaceWriterPG` at `internal/modules/documents/infrastructure/review_surface_writer.go:61`. Idempotent: sets `review_surfaced_at`, guarded by `review_surfaced_at IS NULL OR review_surfaced_at < review_due_at` (migration `db/migrations/0276_document_review_surfaced_marker.sql`) so a rerun in the same cycle is a no-op; advancing `review_due_at` (mark-reviewed) re-arms the next cycle.
...
- **Deferred (ADR 0065, Plan 2):** the coupled `current_revision_id`/`revision_version`/`revision_number` triple on `DocumentSummary`/`DocumentDetailResponse` is the same "parallel scalars pointing at one version" shape templates just cut over to `TemplateVersionRef` (`wiki/architecture/api-contract.md` §5c). A `DocumentRevisionRef` nested value object is the target shape but is **not yet applied** — pre-v1, deferred. See ADR 0065 §Consequences.
...
Documents render through the **shared controlled-artifact view layer** (ADR 0053) — the same shell as templates.

Key files:
- `frontend/apps/web/src/features/documents/adapters/useDocumentArtifact.ts` — fetches document data, maps to `ArtifactViewModel`; document-specific business rules (scheduled→published-head "current version" override, live coverage denominators) live here.
- `frontend/apps/web/src/features/documents/adapters/useDocumentApprovalArtifact.ts` — approval-surface adapter.
- `frontend/apps/web/src/features/approval/pages/SignoffDetailPage.tsx` — thin approval route wrapper for documents; owns `CancelInstanceDialog` (replaces the previous `window.prompt` cancel flow) and other workflow dialogs.
- `frontend/apps/web/src/features/shared/controlled-artifact/` — shared shell (`ArtifactDetailView`, `ArtifactApprovalScreen`, etc.) and `types.ts`.
- `frontend/apps/web/src/features/shared/controlled-artifact/resolveOwnerDisplay.ts` — shared owner-name helper (current-user fallback); consumed by both document and template adapters.
- `frontend/apps/web/src/features/documents/lib/approvalWorkflow.ts` (`mapApprovalChain`) — shared stage→`ApprovalChainItem` mapping; consumed by `useDocumentArtifact` and `useDocumentApprovalArtifact`.
...
- `SubmitRequest` gains `ReasonForChange`/`ReasonCategory` (`internal/modules/approval/application/submit_service.go:52-53`) — a distinct structured 21 CFR Part 11 change-reason field, separate from the free-text `revision_title`. Wire contract: `contracts.SubmitRequest.ReasonForChange`/`ReasonCategory` (`approval/http/contracts/submit.go:25-26` — **verify**, not re-confirmed this pass), both `*string` (optional at REV0).
- Revision number is derived in-tx via `LoadGovernedRevisionNumber` (interface `internal/modules/approval/infrastructure/approval_repository.go:123`, impl `internal/modules/approval/infrastructure/postgres_approval_repository.go:692`, called from `submit_service.go:111`) — `normalizeReasonForChange(revisionNumber, reason, category)` (`submit_service.go:476`) requires a non-empty `reason_for_change` for REV≥1 (`ErrReasonForChangeRequired`, 422 `validation.reason_for_change_required`) and validates `reason_category` against the fixed enum `content|corrective|regulatory|periodic_review|administrative` (`ErrReasonCategoryInvalid`, 422 `validation.reason_category_invalid`) — mirrors DB CHECK `ck_documents_reason_category` (migration 0274).
...
### 8.9b Release readiness-hold projection (ADR 0085 Stage C)

`GetDocument` (`internal/modules/documents/infrastructure/repository.go:302`) folds the ADR 0085 release-coordinator's readiness-hold state directly into the document-detail read model instead of a second round-trip.

- **Query shape:** a `LEFT JOIN LATERAL` against the approval-owned `public.release_generations`, ordered `generation_seq DESC LIMIT 1` — always the document's LATEST generation, never a historical one. Module-boundary note (REQ-TOP-1): reading approval-owned state from `documents/infrastructure` follows the pre-existing read-model-composition precedent — `active_instance_reader.go` already reads `public.approval_instances` for the same reason; `scripts/check-module-boundaries.ps1` governs Go imports, not SQL tables, and no new Go import is added.
- **Domain shape:** `domain.ReleaseProjection{GenerationID, State, HoldReason, HoldDetail, ReleasedAt, LastEvaluatedAt}` (`internal/modules/documents/domain/model.go:104` — field `Release *ReleaseProjection` on `Document`, `:73`); `State` is derived server-side from `released_at` (`ReleaseStateReleased` when set, otherwise `ReleaseStateHold`, `:84-94`) — never persisted redundantly.
- **No-fallback:** a document with no release generation at all (pre-approval draft, legacy row not yet backfilled) gets `nil`, never a synthesized "hold" — mirrors the module's no-fallback discipline elsewhere (§8.8, `wiki/decisions/no-fallback-principle` memory).
- **Wire contract:** `DocumentDetailResponse.release` — a required-nullable `DocumentReleaseProjection` object (`api/openapi/v1/openapi.yaml`, schema `DocumentReleaseProjection`) carrying `generation_id`, `state` (`hold`\|`released`), `hold_reason` (6-value enum: `awaiting_approval_fact`/`materializing`/`awaiting_effective_date`/`supersede_conflict`/`plan_invalid`/`failed`), `hold_detail`, `planned_effective_from`, `released_at`, `last_evaluated_at`.
- **Frontend:** `frontend/apps/web/src/features/documents/lib/documentReleasePresentation.ts` (`getDocumentReleasePresentation`) maps the projection onto PT-BR copy (tone: `released`/`progress`/`scheduled`/`anomaly`) — the UI's ONLY source of release status; no client-side readiness inference. Consumed by `useDocumentArtifact.ts` and typed by `DocumentDetailRoute.tsx` (see [`wiki/architecture/frontend-structure.md`](../architecture/frontend-structure.md) §17.6).
...
| Cross-module display-name read via iam-owned `UserDisplayNameReader` port (consumer: approval module; direct `iam_users.display_name` reads removed — M4/F4.1) | [`wiki/decisions/0029-user-display-name-reader-port.md`](../decisions/0029-user-display-name-reader-port.md) |
| Approval kernel extracted to top-level module, subject-generic `(subject_kind, subject_key)` (`documents` and `templates` both consume it as a published dependency) | [`wiki/decisions/0082-approval-kernel-extraction.md`](../decisions/0082-approval-kernel-extraction.md) — M3, 2026-07-12 |
...
| Release coordinator drives publication off an approval-fact × artifact-fact × effective-date predicate; readiness-hold state projected into `GetDocument`'s read model (§8.9b); `FreezeService.RepairMaterialization` backs the in-flight-disposition backfill tool | [`wiki/decisions/0085-release-coordinator-approval-driven-publication.md`](../decisions/0085-release-coordinator-approval-driven-publication.md) — Stage C |
...
| `document.review` | Tier-2 capability string (ADR 0069, `ScopeTenant`) asserted at `mark_reviewed_service.go:101`; gates the mark-reviewed workflow, distinct from `document.edit` |
| `document.publish` | **Retired** by [ADR 0085](../decisions/0085-release-coordinator-approval-driven-publication.md) Stage B (migration `0311_retire_document_publish_capability.sql`). Publication is the release coordinator's reaction to durable readiness facts, not a human-invoked action, so no route and no service asserts it and it is gone from the Go registry and the reference-data seed. `document.supersede` survives, re-homed to submit-time cross-document plan authorization |
