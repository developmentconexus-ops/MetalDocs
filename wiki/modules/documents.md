...
**Last verified:** 2026-05-29 (QA `qa/documents-detail` follow-up: backlog `documento-publicado.md` claim that `DocumentResponse` was missing `area_code` / `profile_code` was stale — payload already exposes `ProcessAreaCodeSnapshot` + `ProfileCodeSnapshot` + `CreatedAt`. Wired both into `DocumentPublishedPage` via `useAreasQuery` + `useProfilesQuery` + existing `resolveAreaLabel` / `resolveProfileLabel`, replacing 5 hardcoded "—" sites: breadcrumb middle slot, DocCardMini header (area) + type (profile), hero `typeLabel` badge, Identificação e responsabilidade `Tipo` + `Área` facts. Preview proof on real draft `c1bb2112-21ea-46fc-ac1f-719d04994d41` (`ProcessAreaCodeSnapshot='rh'`, `ProfileCodeSnapshot='po'`): breadcrumb "Biblioteca › Recursos Humanos › PO-RH-002", DocCardMini header "RECURSOS HUMANOS", type "Procedimento Operacional", facts "Tipo: Procedimento Operacional / Área: Recursos Humanos". Also removed mock `PLACEHOLDER_RELATED` + `PLACEHOLDER_COMMENTS` arrays + reply-box shell from §04/§05; both sections now render an honest "em breve" empty state pending the relationship/display-comments backend modules (defers updated in `wiki/backlog/documento-publicado.md`). tsc clean, 19/19 vitest with new `useAreasQuery` + `useProfilesQuery` mocks. Earlier in same branch: fixed misleading status badge for draft/under_review/rejected + wrong Iniciar-revisão title. Bounded defers remaining: PDF download, Cobertura fanout, `/documents/:id/distribution` mock-only page (→ `wiki/backlog/distribuicao.md`). Previous: 2026-05-29 `qa/documents-editor` ambiguous-content_hash JOIN fix.) | **Owner:** unassigned | **Status:** active | **Maturity:** L3
...
- `GET /api/v1/documents/{id}` exposes governed `RevisionNumber` so frontend submission gates use business revision truth instead of technical `RevisionVersion`. It also surfaces current-head artifact metadata via `currentRevisionFileSizeBytes`, `currentRevisionPageCount`, and `currentRevisionPageCountSource`.
