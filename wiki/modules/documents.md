...
**Last verified:** 2026-05-29 (QA `qa/documents-detail`: `DocumentPublishedPage.getDocumentStatusPresentation` — default switch branch was returning `badgeLabel: 'vigente'` + `ownerMeta: 'publicado'` for ANY non-`published` status, so a draft document rendered the misleading badge "REV00 · VIGENTE" with subtitle "publicado em —". Added explicit `draft` / `under_review` / `rejected` cases and a safe default that surfaces the raw status. Also fixed "Iniciar revisão" disabled-button title which incorrectly cited "permissão" when the real reason was `!isPublished`. Preview-driven repro on real draft doc (`Status='draft'`, no `published_at`): badge now reads "REV00 · RASCUNHO", subtitle "Rascunho — ainda não publicado", owner-meta "rascunho", button title "Apenas documentos publicados podem iniciar uma nova revisão". Bounded defers: `PLACEHOLDER_RELATED` + `PLACEHOLDER_COMMENTS` mock arrays + area/profile "—" tracked in `wiki/backlog/documento-publicado.md`; entire `/documents/:id/distribution` mock-only page tracked in `wiki/backlog/distribuicao.md`. tsc clean, 19/19 vitest. Previous: 2026-05-29 `qa/documents-editor` ambiguous-content_hash JOIN fix.) | **Owner:** unassigned | **Status:** active | **Maturity:** L3
