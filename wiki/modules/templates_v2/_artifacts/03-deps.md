# templates_v2 — Cross-dependency map
...
Notes:
- All other intra-module imports (e.g. `templates_v2/domain`, `templates_v2/application`, `templates_v2/api`) are self-imports and are excluded per instructions.
...
## 2. Imports IN (packages that import templates_v2)
...
| `internal/platform/objectstore` | `templates_v2_presigner.go:13` | `domain.ErrUploadMissing` | Presigner adapter implements `application.Presigner`; uses domain error sentinel |
| `internal/platform/docgenv2` | `templates_v2_reader.go` (no Go import; raw SQL only) | — | `TemplatesV2TemplateReader` reads directly from `templates_v2_template_version` table; no Go import of templates_v2 packages |
| `internal/platform/docgenv2` | `templates_v2_snapshot_reader.go` (no Go import; raw SQL only) | — | `TemplatesV2SnapshotReader` reads directly from DB tables; no Go import of templates_v2 packages |
| `apps/api/cmd/metaldocs-api` | `main.go:34–36` | `tv2app.New`, `tv2repo.New`, `tv2http.New` | DI composition root; constructs and registers all templates_v2 layers |
...
No calls to `env.Get`, `os.Getenv`, `viper`, or feature-flag helpers were found anywhere in `internal/modules/templates_v2/**/*.go`.
