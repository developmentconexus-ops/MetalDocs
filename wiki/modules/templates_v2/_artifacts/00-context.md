...
- **Module name on disk:** `internal/modules/templates_v2/`
- **Wiki doc filename target:** `wiki/modules/templates_v2.md` (matches on-disk; rename to `templates.md` happens in same commit as `templates_v2/ → templates/` code rename — NOT this commit)
- **Status:** active production module; Plan 2 (commits ae1229e8..c84215f7) flipped `/api/v2/` → `/api/v1/` and started API design system spec (RFC 9457 envelope, cursor pagination). Code rename `templates_v2/ → templates/` pending.
- **Predecessor stub:** `wiki/modules/templates-v2.md` (kebab dash) — heavy on frontend wizard, light on backend; will be retired in same commit (replaced by `templates_v2.md` underscore, mirrors on-disk dir).

## Pre-flight reads (done)

| Doc | Key takeaway |
|---|---|
| `wiki/README.md` | Modules indexed; existing stub `modules/templates-v2.md` covers frontend wizard but lacks Arc42 + tech-debt register |
| `wiki/modules/templates-v2.md` | Frontend-heavy stub; backend section is `## API surface — TBD`. This doc will replace it (rename to underscore) |
| `wiki/concepts/placeholders.md` | Fixed 7-token catalog enforced by `templates_v2/application/validate_placeholders.go`; non-catalog names rejected at schema-save |
...
| `wiki/modules/documents.md` (cross-ref) | Documents are downstream consumers — every document instantiates from a template via `placeholder_schema_snapshot`; templates_v2 is upstream |
...
- API path: was `/api/v2/templates`, Plan 2 flipped to `/api/v1/templates` (commits ae1229e8..c84215f7).
- Code dir: still `internal/modules/templates_v2/`. Rename to `internal/modules/templates/` is pending.
- Wiki filename: this doc lands as `templates_v2.md`. Rename to `templates.md` happens in same commit as code rename — flagged in tech-debt as `maint:migration-cleanup`.
...
## Severity rubric for templates_v2 (apply in Phase 6)
...
- **Major:** RFC 9457 envelope absent on templates_v2 routes; OpenAPI/handler drift; concurrent autosave race; lifecycle idempotency gap.
- **Minor:** doc-cleanup; dep-bump; test-only; migration-cleanup (templates_v2/ → templates/ rename); docs-link.
