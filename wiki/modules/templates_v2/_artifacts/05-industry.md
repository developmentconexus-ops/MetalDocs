# Phase 5 — Industry comparison · templates_v2
...
**Applies to:** `internal/modules/templates_v2/delivery/http/handler.go:95-102` (`writeErr`).

**Status: NOT applied.** templates_v2 emits the legacy MetalDocs envelope `{"error":{"code":"...","message":"..."}}`. Plan 2 (`wiki/architecture/api-design-system.md`, commits ae1229e8..c84215f7) introduced `internal/platform/problem` for RFC 9457; templates_v2 has not been migrated. Same shape as `documents` T-001 and `auth` T-003 — module-by-module rollout debt.
...
**Applies to:** `internal/modules/templates_v2/application/lifecycle.go:265` (`PublishTemplateVersion`), `application/create.go:30` (`CreateTemplate`).

**Status: NOT applied.** No `Idempotency-Key` header parsing in templates_v2 routes; no lookup against `internal/platform/idempotency/`. Replays of `POST /api/v2/templates` and `POST /publish` either succeed twice (creating duplicate audit rows) or fail with `ErrInvalidStateTransition` after the first transition lands. Compare: `documents` module §6.6 (per `wiki/modules/documents.md`) records the same gap as T-006.
...
**Status: NOT applied — single tier missing entirely.** The architecture (`wiki/decisions/0007-two-tier-authz.md`) specifies tier-1 (`CapabilityService.CanDo` middleware) + tier-2 (`authz.Require` in-tx) + Postgres tripwire. templates_v2 has zero of the three:
...
3. Tripwire: no `metaldocs.asserted_caps` GUC; no SQL trigger checking it on INSERT/UPDATE/DELETE of `templates_v2_*` tables (per Phase 4 §3 — zero triggers/GUCs/functions installed).
...
**Applies to:** `internal/modules/templates_v2/api/api.gen.go` (oapi-codegen output) + hand-rolled handlers in `delivery/http/handler.go:48-61`.

**Status: PARTIAL.** Eight routes are generated (`api.gen.go:954-961`). Twelve hand-rolled routes (handler.go:48-61) are NOT in the spec — `POST /api/v2/templates/{id}/versions`, `PUT /api/v2/templates/{id}/versions/{n}/schema`, `POST /api/v2/templates/{id}/versions/{n}/{submit,review,approve}`, `GET /api/v2/templates/{id}/audit`, `GET /api/v2/templates/v2/placeholder-catalog`, etc. Same drift class as `documents` T-002 (spec/handler drift Critical).

## IP-006 — Forward-only migrations

> "Each change to the database is described by a migration script." — Fowler (2016)

**Applies to:** `migrations/0101..0165` (templates_v2 lineage).
...
**Applies to:** n/a — observability stack not yet wired in MetalDocs (per index note). No suggestion added; absence is repo-wide, not templates_v2-specific.
...
**Applies to:** `migrations/0120_templates_v2_init.sql` schema.

**Status: PARTIAL.** `templates_v2_template` and `templates_v2_audit_log` carry `tenant_id NOT NULL`. `templates_v2_template_version` and `templates_v2_approval_config` do NOT carry `tenant_id` — they inherit tenant scope only via FK to `templates_v2_template`. Repo methods `GetVersion(template_id, version_number)` and `GetVersionByID(version_id)` (`postgres.go`) accept no tenant arg — cross-tenant version-id lookups will succeed if the version_id is known (no constraint to prevent it). Compare IP-008 quote: "Add tenant_id to every multi-tenant table." — gap.
