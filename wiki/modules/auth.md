...
**Last verified:** 2026-06-12 (Wave 2.12 sync — Service.loginCtxPort is a REQUIRED constructor dependency (panics on nil — previously guarded with != nil check); InMemoryAuthFailureRateLimiter deleted; Postgres limiter wired unconditionally. Prior Wave 2 sync: RecordLastLoginContext removed from Repository; login-context write delegated to iamdomain.LoginContextPort; ListUsers N+1 fixed with RolesByUserIDs batch; SessionAdminQuery/SessionListItem promoted to auth/domain) | **Prior:** 2026-06-11 (adversarial-verification anchor corrections pass 2: handler line numbers + route registration lines corrected; prior: 2026-06-11 pass 1 — Key files section anchors) | **Owner:** unassigned | **Status:** active (RFC 9457 envelope; auth event audit emission wired) | **Maturity:** L2
...
> - `internal/modules/auth/application/service.go:126` â€” `NewService` (rejects `SessionSecret` shorter than 32 bytes; Wave 2.12: `loginCtxPort` arg is required — panics on nil at `:130-131`)
...
- 2026-06-12 — Wave 2.12 sync: `Service.loginCtxPort` is now a REQUIRED constructor dependency — `NewService` panics on nil at `service.go:130-131`; the prior `!= nil` call-site guard is gone. `InMemoryAuthFailureRateLimiter` deleted; Postgres limiter (`PostgresAuthFailureRateLimiter`) wired unconditionally. Key files §NewService anchor updated.
