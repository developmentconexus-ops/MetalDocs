...
- Request to service is built as `application.ListFilter{TenantID, AreaAny, DocTypeCode, Limit, Offset}` at `internal/modules/templates/delivery/http/routes_query.go:37-43`.
...
- `toTemplateResponse` emits fields: `id`, `tenant_id`, `doc_type_code`, `key`, `name`, `description`, `areas`, `visibility`, `specific_areas`, `latest_version`, `published_version_id`, `created_by`, `created_at`, `archived_at` (`internal/modules/templates/delivery/http/routes_create.go:89-109`).
...
| Repo query | `templates_template` SELECT with doc_type, area, visibility filters + `LIMIT/OFFSET` | `internal/modules/templates/repository/postgres.go:89-115` |
