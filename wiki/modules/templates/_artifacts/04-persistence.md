# Phase 4 — Persistence map · templates

> Date: 2026-05-10
> Source: `migrations/0120_templates_init.sql`, `internal/modules/templates/repository/postgres.go`, application service files.
...
All four tables are created in a single migration: `migrations/0120_templates_init.sql`.
...
| `published_version_id` | uuid | NULL; FK → `templates_template_version(id)` (ALTER ADD CONSTRAINT) |
...
### `templates_approval_config`

| Column | Type | Constraints |
|---|---|---|
| `template_id` | uuid | PRIMARY KEY; FK → `templates_template(id)` |
...
|---|---|---|---|
| `documents_v2` | documents (W1 scaffold, dropped per `wiki/modules/documents.md`) | R/W | `migrations/0121_documents_v2_link_template_version.sql` adds FK to `templates_template_version(id)` |
| `controlled_documents` | registry | R | `migrations/0124_registry_controlled_documents.sql` adds FK to `templates_template_version(id)` |
...
Compare: `documents` module installs `enforce_snapshot_on_submit_trg` (per `wiki/modules/documents.md §6`); `templates` installs no equivalent enforcement. All invariants are application-layer only.

## 4. Indexes

| Index | Table | Columns | Unique? | Purpose |
|---|---|---|---|---|
| `templates_template_pkey` | `templates_template` | `(id)` | yes | PK |
| `templates_template_tenant_id_key_key` | `templates_template` | `(tenant_id, key)` | yes | unique key per tenant |
| `idx_templates_template_tenant_doctype` | `templates_template` | `(tenant_id, doc_type_code)` | no | list filter |
| `templates_template_version_pkey` | `templates_template_version` | `(id)` | yes | PK |
| `templates_template_version_template_id_version_number_key` | `templates_template_version` | `(template_id, version_number)` | yes | version uniqueness |
| `idx_templates_version_template_status` | `templates_template_version` | `(template_id, status)` | no | status lookup |
| `templates_approval_config_pkey` | `templates_approval_config` | `(template_id)` | yes | PK |
| `templates_audit_log_pkey` | `templates_audit_log` | `(id)` | yes | PK |
| `idx_templates_audit_template_time` | `templates_audit_log` | `(template_id, occurred_at DESC)` | no | recent-events scan |

## 5. Tripwire pairing audit

Repo mutations (verified at `internal/modules/templates/repository/postgres.go`):

| Method (file:line) | Authz.Require called? | Cap + area arg | SQL verb | Table |
|---|---|---|---|---|
| `Repository.CreateTemplate` (postgres.go:29) | **NO** | (none) | INSERT | `templates_template` — VIOLATION |
| `Repository.UpdateTemplate` (postgres.go:134) | **NO** | (none) | UPDATE | `templates_template` — VIOLATION |
| `Repository.CreateVersion` (postgres.go:169) | **NO** | (none) | INSERT | `templates_template_version` — VIOLATION |
| `Repository.UpdateVersion` (postgres.go:236) | **NO** | (none) | UPDATE | `templates_template_version` — VIOLATION |
| `Repository.ObsoletePreviousPublished` (postgres.go:271) | **NO** | (none) | UPDATE | `templates_template_version` — VIOLATION |
| `Repository.UpsertApprovalConfig` (postgres.go:302) | **NO** | (none) | INSERT/UPDATE | `templates_approval_config` — VIOLATION |
| `Repository.AppendAudit` (postgres.go:318) | **NO** | (none) | INSERT | `templates_audit_log` — VIOLATION (audit) |
...
| 5 | `0120_templates_init.sql` | CREATE 4 owned tables (`templates_*`) + 3 indexes + FK | **canonical templates schema** |
| 6 | `0121_documents_v2_link_template_version.sql` | ALTER `documents_v2` → FK to `templates_template_version` | downstream coupling |
| 7 | `0124_registry_controlled_documents.sql` | CREATE `controlled_documents` w/ FK to `templates_template_version` | registry coupling |
