...
1. `internal/modules/templates/delivery/http/handler.go:46` route registration binds publish path.
   -> calls: `internal/modules/templates/api/api.gen.go:764` `ServerInterfaceWrapper.PublishTemplateVersionV2`
2. `internal/modules/templates/api/api.gen.go:764` wrapper parses path params.
   -> calls: `internal/modules/templates/api/api.gen.go:1419` `strictHandler.PublishTemplateVersionV2`
3. `internal/modules/templates/api/api.gen.go:1419` strict middleware decodes request body.
   -> calls: `internal/modules/templates/api/api.gen.go:1433` `sh.ssi.PublishTemplateVersionV2(...)`
4. `internal/modules/templates/delivery/http/routes_generated.go:185` handler edge logic.
   -> authz call: `h.authz(r, tenantID, "*", "template.approve")` (`routes_generated.go:188`)
   -> calls: `internal/modules/templates/application/lifecycle.go:265` `Service.PublishTemplateVersion`
5. `internal/modules/templates/application/lifecycle.go:265` publish flow.
...
6. `internal/modules/templates/application/create.go:115` next draft creation.
   -> `repo.CreateVersion` (`create.go:148`)
   -> `repo.UpdateTemplate` (`create.go:153`)
7. `internal/modules/templates/repository/postgres.go:269` `ObsoletePreviousPublished` SQL UPDATE.
8. `internal/modules/templates/repository/postgres.go:229` `UpdateVersion` SQL UPDATE.
9. `internal/modules/templates/repository/postgres.go:311` `AppendAudit` SQL INSERT.

**Transaction boundary:** No `BeginTx` / `sql.Tx` / `pgx.Tx` in templates handler/service/repo publish path. Repo methods execute `r.db.ExecContext(...)` independently (`postgres.go:253`, `:274`, `:324`).
...
|---|---|---|---|
| `internal/modules/templates/repository/postgres.go:269` | UPDATE | `templates_template_version` | none |
| `internal/modules/templates/repository/postgres.go:229` | UPDATE | `templates_template_version` | none |
| `internal/modules/templates/repository/postgres.go:151` | UPDATE | `templates_template` | none |
| `internal/modules/templates/repository/postgres.go:311` | INSERT | `templates_audit_log` | none |
| `internal/modules/templates/repository/postgres.go:118` | INSERT | `templates_template_version` | none |

Tripwire pairing (`authz.Require(...)` before mutating SQL on same tx):
- `authz.Require` in templates path: not found.
