...
For operation `UNVERIFIED` (HTTP `PUT /api/v1/templates/{id}/versions/{n}/schema`) in module `internal/modules/templates`, trace call flow and runtime behavior.
...
|---|---|---|---|---|
| `TemplateVersion.status` | `draft required` | `unchanged` | `UpdateSchemas` enforces draft and updates schemas only | `handler action "template.edit" via h.authz` (`internal/modules/templates/application/schema.go:25`, `:55-56`; `internal/modules/templates/delivery/http/routes_schema.go:21`) |

### 4. SQL touched

| File:line | Verb | Table(s) | Auth-area arg (if any) |
|---|---|---|---|
| `internal/modules/templates/repository/postgres.go:190-197` | `SELECT` | `templates_template_version` | none |
| `internal/modules/templates/repository/postgres.go:236-252` | `UPDATE` | `templates_template_version` | none |
| `internal/modules/templates/repository/postgres.go:318-322` | `INSERT` | `templates_audit_log` | none |

Tripwire pairing (authz before mutate SQL on same tx):
- `AUTHZ CHECK`: `internal/modules/templates/delivery/http/routes_schema.go:21` (`h.authz(...)`)
- `MUTATING SQL`: `internal/modules/templates/repository/postgres.go:253` (`ExecContext` UPDATE)
...
- Authz wiring default (`AUTHZ BYPASSED` risk): `Handler.New` replaces nil authz with allow-all no-op (`internal/modules/templates/delivery/http/handler.go:24-27`).
- ValidatePlaceholders catalog rule: fixed catalog of 7 names enforced by `placeholderCatalogSet` membership check (`internal/modules/templates/application/schema.go:79-82`, `:103-105`).
- ValidatePlaceholders also enforces name regex + uniqueness + computed/name coupling:
  - name regex `^[a-z][a-z0-9_]{0,49}$` (`internal/modules/templates/application/schema.go:77`, `:96-98`)
  - duplicate id/name rejected (`internal/modules/templates/application/schema.go:91-93`, `:99-101`)
  - named placeholders must be `PHComputed` with `resolver_key == name` (`internal/modules/templates/application/schema.go:106-108`)
  - additional constraint rejections: invalid regex/date/min-max/max_length/computed-resolver_key (`internal/modules/templates/application/schema.go:110-136`)
  - select options rules in service: select requires options; non-select forbids options (`internal/modules/templates/application/schema.go:47-52`)
- Domain placeholder types include fixed 7-type constants: `PHText`, `PHDate`, `PHNumber`, `PHSelect`, `PHUser`, `PHPicture`, `PHComputed` (`internal/modules/templates/domain/schemas.go:13-20`).
...
- Concurrency model:
  - pre-write stale check uses `ExpectedContentHash` vs loaded version hash (`internal/modules/templates/application/schema.go:28-30`)
  - UPDATE predicate is only `WHERE id = $1` (no content-hash/version guard in SQL) (`internal/modules/templates/repository/postgres.go:252`).
- Audit emission: yes, `AppendAudit` called with `Action: domain.AuditSaved` and `Details.kind = "schema"` (`internal/modules/templates/application/schema.go:62-69`).
- Repository `SaveSchema`: `UNVERIFIED` (symbol not found in `internal/modules/templates/repository/postgres.go`; available method is `UpdateVersion` at `:229`).
- ResolverRegistryReader runtime wiring in `main.go`: `UNVERIFIED`.
  ...
  - service constructor leaves `resolvers` nil when not passed (`internal/modules/templates/application/service.go:11-16`), and resolver-key registry validation is conditional on `s.resolvers != nil` (`internal/modules/templates/application/schema.go:35`).
