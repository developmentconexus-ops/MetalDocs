...
- **Severity:** major (closed)
- **Surface (resolved):** `internal/modules/controlleddocuments/infrastructure/repository.go:385-396` (`Create` opens its transaction through `platform/db.TxRunner.Do` and calls `authz.Require(CapControlledDocumentCreate)` at `:388`). `CreateTx` is the storage-only caller-tx seam (`repository.go:401-415`); the service performs the mandatory authz check before calling it on both manual and auto-code paths (`application/service.go:480-494,559-664`). `migrations/0188_tripwire_extend.sql:201-208` (legacy literal migration filename) attaches `trg_require_cap_asserted` to `public.controlled_documents` (INSERT + UPDATE, with OR-logic for `controlled_documents.obsolete|controlled_documents.supersede` on UPDATE) and `public.cd_sequence_counters` (line 206).
...
### T-009    Documents DI cycle resolved via post-construction setter — PARTIALLY HARDENED 2026-07-02 (ARC-05)
- **Severity:** minor
- **Surface:** `apps/api/cmd/metaldocs-api/main.go:361-363` (controlled-documents module built first), `:544-554` (documents module built, then `controlledDocumentsModule.Service().WithDocumentInitializer(docapp.NewCDDocumentInitializer(docMod.Service))`); `internal/modules/controlleddocuments/application/service.go:93-148` (`NewControlledDocumentService` + `WithDocumentInitializer`), `:328-338` (auto-create path guard, added).
- **Observation (original):** `controlledDocumentsModule.New(...)` constructs `ControlledDocumentService` with the `DocumentInitializer` argument `nil`. `main.go` calls `.WithDocumentInitializer(...)` after the documents module is built. Order-of-construction is a hidden contract — if a future caller forgets the setter, `Create` was believed to nil-panic on the port call.
- **Root-cause verdict (2026-07-02):** the cycle is genuine, not artificial. `documents.New` requires `ControlledDocumentDuplicator` (backed by `controlledDocumentsModule.Service()`) as a constructor argument, and `ControlledDocumentService` requires a `DocumentInitializer` backed by `docMod.Service` (the *documents* application struct, reached via unexported methods — `resolveTemplateStorageKey`, `templateArtifactExists`, `resolveTemplateVersionID`, `cloneIntoTx` — so the adapter (`internal/modules/documents/application/cd_initializer.go`) must live in-package against the concrete `*Service`, not a cross-package interface). This is a true bidirectional dependency between two sibling modules at construction time: CD needs docMod.Service; docMod needs CD.Service. Removing the setter entirely would require merging the two modules or introducing a mediating construction step — both are cross-module redesigns outside mechanical-wiring scope, so no such restructuring was attempted (stopped per task boundary).
- **Fix applied (ARC-05, evidence not just belief):** traced every `s.docInit` call site in `service.go`. Two of three call paths (`ensureTemplateArtifact` at `:482`, `CreateRevision` at `:764`) already guarded `s.docInit == nil` and returned a named error. The auto-create path in `Create` (`:328`, was unguarded) relied entirely on `ensureTemplateArtifact` (called immediately before it) already failing closed on the same nil condition — today's call order makes the guard-less call unreachable, so there was no LIVE nil-panic path, only a latent one if the two calls were ever reordered or `ensureTemplateArtifact` removed. Added an explicit `if s.docInit == nil { return nil, ErrTemplateArtifactInvariantUnconfigured }` guard directly before the `:328` call, mirroring the other two guarded sites, so the invariant is enforced locally at every call site instead of incidentally by call order. Verified the guard is load-bearing (not vacuous) by temporarily removing it and confirming `TestControlledDocumentService_Create_AutoPath_NilDocInitAfterArtifactCheck_FailsClosed` panics at the removed line, then restoring it and confirming the test passes.
- **Residual (unchanged, latent, accepted):** the post-construction setter itself (`WithDocumentInitializer`) remains — it is the correct shape for a genuine bidirectional module dependency without a merge/mediator redesign. `NewControlledDocumentService`'s constructor still accepts `docInit` as its 8th (nilable) argument and `main.go` still wires it via the setter after `docMod` is built. This residual is intentional, not a defect the mechanical-wiring fix can close.
- **Evidence:** `internal/modules/controlleddocuments/application/service.go:328-338`; test `internal/modules/controlleddocuments/application/service_test.go` (`TestControlledDocumentService_Create_AutoPath_NilDocInitAfterArtifactCheck_FailsClosed`); `go build ./...`, `go vet ./apps/api/... ./internal/modules/documents/... ./internal/modules/controlleddocuments/...`, `go test -count=1 ./internal/modules/controlleddocuments/... ./internal/modules/documents/... ./apps/api/cmd/metaldocs-api/...` all clean 2026-07-02.
...
### T-010    Parallel repository instance constructed outside module boundary — CLOSED (stale drift, confirmed 2026-07-02, actually fixed 2026-06-12)
- **Severity:** minor (closed — drift)
- **Surface:** `internal/modules/controlleddocuments/module.go:19,80,108-110` (`Module.Repo()` accessor).
- **Observation (original):** A second `PostgresControlledDocumentRepository` was believed instantiated standalone in `main.go` for search/resolver wiring, reaching into the internal infrastructure package via `controlleddocumentsinfra.NewPostgresControlledDocumentRepository` — a module-boundary leak.
- **Drift verdict (2026-07-02, ARC-05 re-verification):** this was already fixed by commit `730ea426` ("refactor(wave-z): Z-8 security boundary disposition + Z-9 CD repo exposed via module (F-06 residuals)", 2026-06-12), three weeks before the 2026-07-01 grade-A simplification review that re-raised this finding with stale line numbers. That commit added `Module.Repo()` (module.go:108-110, doc comment: "so the composition root can wire it into other modules without constructing a second instance") and replaced the second `NewPostgresControlledDocumentRepository` call in `main.go` with `controlledDocumentsModule.Repo()`, removing the `controlleddocumentsinfra` import from `main.go` entirely. Confirmed live in the current tree: `grep -c "NewPostgresControlledDocumentRepository" apps/api/cmd/metaldocs-api/main.go` = 0; the single construction lives inside `controlleddocuments.New` (`module.go:80`); the only consumers of `.Repo()` are the sweepers (`main.go:612-613`, `documents` module's own `.Repo()`) and `wiring.NewControlledDocumentsReader(ctlDocs.Repo())` at `main.go:914`, both legitimate reuse through the published accessor. Also independently re-checked the two other candidate "cross-module reader" constructions at the composition root (`CDFieldReaderPG` at `main.go:463`, `ActiveInstanceReaderPG` at `main.go:781`) — both are narrow, single/few-method stateless read-ports, each package-owned by the module that owns the table it queries, following the same documented ADR-0039 D3(b) published-read-port pattern used for `ProfileReader`/`AreaReader`/`AreaCatalogReader`/`TemplateVersionChecker`; neither duplicates a full repository. No live "parallel repository" defect exists in the current tree. No code change required for this half of ARC-05.
- **Evidence:** commit `730ea426ba4668020a70ea1f4d3d9db6c642d18e` (`git show --stat 730ea426`; diff removes `controlleddocumentsinfra` import and the standalone constructor call from `main.go`); `internal/modules/controlleddocuments/module.go:108-110`; grep confirmation `2026-07-02`.
- **Linked backlog row:** [`backlog/controlled-documents-refactor.md#R-010`](../backlog/controlled-documents-refactor.md) (can be closed)
- **Linked ADR:** missing-ADR
