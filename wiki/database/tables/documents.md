...
**Post-baseline note (migration `0265`, 2026-07-02):** `documents_status_check` was re-tightened from 10 to 9 values, dropping the dead `'finalized'` literal.

**Baseline regenerated 2026-07-29 (fold f1910ac1, migrations `0257`–`0315`):** `db/baseline/0001_current_schema.sql` now reflects a full replay through `0315`, not just `0265`. `documents_status_check` at `db/baseline/0001_current_schema.sql:1925` now reads 8 values, no longer including `'rejected'` (removed by folded migration `0272_documents_remove_rejected.sql`) — the DDL block above (line 198 of this doc) still lists `'rejected'` and 9 values and has **not** been re-verified against the regenerated baseline in this pass (**verify**, out of scope for the DB-bootstrap-fold documentation pass; see `wiki/database/migration-policy.md` for the fold procedure).
