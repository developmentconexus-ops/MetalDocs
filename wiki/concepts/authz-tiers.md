...
> **Key files:**
> - `internal/modules/iam/application/capability_service.go:48` — tier-1 `CanDo`
> - `internal/modules/iam/authz/authz.go:100` — tier-2 `Require`
> - `internal/modules/iam/authz/context.go:11` — `ErrActorContextMissing` / `ErrTenantContextMissing` typed errors; `MustActorID` at :49, `MustTenantID` at :62
> - `db/migrations/0231_db_hardening_tripwire_and_dead_schema.sql:36` — `enforce_capability_asserted()` function covering 12 tables (Plan 5 + hardening); trigger attachment follows inline in the same migration (**verify** — `db/migrations/` in the current tree starts at `0257`; migrations were squashed/renumbered independent of the F9.5 rename this pass covers; this anchor is flagged, not fixed, out of this pass's scope)
> - `internal/modules/iam/domain/model.go:75` — `Capability` typed consts (29 total; ADR 0016 added `CapMetricsView`, `CapMembershipView`, `CapUserView`, `CapTaxonomyView`; Plan 5 follow-up added `CapControlledDocumentObsolete`/`CapControlledDocumentSupersede`; ADR 0022 Phase 10 minimized 33→29)
...
- [`wiki/concepts/approval-routes.md`](approval-routes.md) — plain-language overview of the approval route catalogue (approval-route reads and writes both gate on `cap:route.manage` at tier-1 and tier-2 — ADR 0022 Phase 11 F4 superseded the ADR 0018 §6 `CapRouteView` split proposal; APP-05 closed 2026-07-02)
- [`wiki/decisions/0018-approval-route-lifecycle.md`](../decisions/0018-approval-route-lifecycle.md) — route lifecycle ADR; §6's proposed Tier-1 `route.view` split was superseded by ADR 0022 Phase 11 F4 (see `wiki/decisions/0022-authz-capability-coherence.md` "Phase 11" and `wiki/modules/approval-tech-debt.md` T-015/BE-9)
