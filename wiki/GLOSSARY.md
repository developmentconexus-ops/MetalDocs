...
**Capability** - Typed permission unit (`iamdomain.Capability` string type). 18 consts in `internal/modules/iam/domain/model.go` — e.g., `document.view`, `document.edit`, `template.approve`. Granted per role in `role_capabilities` table; checked DB-side by `CapabilityService.CanDo`. See `modules/iam.md`.
