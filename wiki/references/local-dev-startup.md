...
| password | `AdminMetalDocs123!` | `ApproverMetalDocs123!` | `AuthorTest123!` | `ApproverMetalDocs456!@` |
| iam_user_roles | `system_admin` | `approver` | `author` | `approver` |
| user_process_areas (rh) | `qms_admin` | `qms_admin` | `author` | `approver` |
| use case | full admin / wizard / list QA; tenant-wide `authz.Require` bypass (ADR 0022), so it must NOT also be the template's author when driving approve/publish (self-bypass would pass SoD's `actorID == authorID` check and defeat the real-path proof); only seeded identity that passes the direct `POST /publish` handler gate | approving identity for the kernel workflow path: signs off (approve) via `POST .../signoff`, which lands the version on `approved` — it does **not** publish (kernel signoff never auto-publishes; publish is a separate `POST .../publish` step, ROADMAP 3.1a); cannot itself publish (iam role `approver` lacks `template.publish` at the handler gate — see publish-capability note) | draft authoring; pair with `approver` (or `admin`) as the approving identity; author for direct-publish drives (`admin` publishes) | capability-gate negative QA identity: holds `template.approve` only (`template.review` capability retired, ROADMAP unit 3.1a S5 — seed grant removed); lacks `template.submit` / `template.publish` — use to prove 403 on publish, not to drive a full lifecycle |
...
**SoD note:** templates lifecycle requires the approver's `userId` to differ from the author's `userId` (`domain.CheckSegregation`, `internal/modules/templates/domain/approval.go:19`). `admin` authors → `approver` signs off (or `approver-test` for capability-gate-only checks). Two distinct seeded admin-role users (`admin` + `approver`) exist solely to satisfy this segregation locally; see [`migrations/0159_seed_dev_approver_user.sql`](../../migrations/0159_seed_dev_approver_user.sql).

**Publish-capability note (kernel-only, ROADMAP 3.1a — supersedes the legacy TST-03 note):** the kernel approval decision (`POST .../signoff`, approve) lands the version on `approved` and stops there — it never auto-publishes (the legacy `Approve(accept=true)`-publishes-directly path was deleted in 3.1a S4). Publishing is always a **separate** `POST /templates/{id}/versions/{v}/publish` step, which accepts a `draft` (direct) OR an `approved` (post-signoff) source (`internal/modules/templates/application/lifecycle.go:68`), enforces identity SoD (`CheckSegregation` — actor ≠ author/reviewer, `lifecycle.go:84`) + `CapTemplatePublish` in-tx (`lifecycle.go:122`); the role-binding gate was removed in 3.1a S2. Full kernel lifecycle recipe (three distinct users): `author-test` authors + submits (`POST .../submit-for-approval`), `approver` (or `approver-test`) signs off approve (`POST .../signoff`, password e-signature), `admin` publishes (`POST .../publish`).

The publish handler capability gate (`templatesAuthzFn` → `CapabilityService.CanDo`, `apps/api/cmd/metaldocs-api/main.go`) reads **`iam_user_roles` × `role_capabilities`** — NOT `user_process_areas` — and `iam_user_roles` allows exactly one role per user (`uq_iam_user_roles_user_tenant`) from `{system_admin, approver, author, editor, viewer}` (`qms_admin` is not a legal code there). `template.publish` maps only to `qms_admin`/`system_admin` in `role_capabilities`, so among seeded identities only `admin` (`system_admin`) passes the publish gate. Live-proven: `admin` publishes an `author-test`-authored template (SoD holds — distinct users); `approver-test` publishing → 403 `AUTH_FORBIDDEN` (capability gate intact). The seeded `approver` user's `rh` UPA row carries role `qms_admin` (superset of `approver` caps) so tier-2 in-tx `authz.Require` passes on the signoff path; a second concurrent UPA row is impossible (`ux_user_process_areas_one_active`, one active row per user+tenant+area) — see `db/dev-seeds/0001_local_dev_seed.sql`.

**Tripwire (kernel-only, ROADMAP 3.1a):** the `under_review`-stage decision now runs through the kernel signoff, which asserts `template.approve` (subject-discriminated tripwire, ADR 0083). The legacy reviewer-stage `template.review` arm on the `enforce_capability_asserted()` `templates_template_version` branch (added by migration `0269`) was **retired** by migration `0301_tripwire_template_review_retired.sql` (3.1a S5) when `CapTemplateReview` was removed; the historical 0269 note is superseded.

`approver-test` password was temporarily reset to `ApproverTest123!` on 2026-05-31 (template-editor QA Preview drive) via `PATCH /api/v1/iam/users/approver-test` — but any full re-seed (`dev-bootstrap-baseline.ps1 -WithDevSeed` drops the volume) reverts it to the seed value `ApproverMetalDocs456!@` in `db/dev-seeds/0001_local_dev_seed.sql`. The matrix above always reflects the seed values, which are the post-bootstrap truth.
