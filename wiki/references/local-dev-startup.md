...
| use case | full admin / wizard / list QA; tenant-wide `authz.Require` bypass (ADR 0022), so it must NOT also be the template's author when driving approve/publish (self-bypass would pass SoD's `actorID == authorID` check and defeat the real-path proof); only seeded identity that passes the direct `POST /publish` handler gate | approving identity for the workflow path: `Approve(accept=true)` **publishes directly** (satisfies SoD vs an `admin`-authored template); cannot use the direct `POST /publish` route (iam role `approver` lacks `template.publish` at the handler gate — see publish-capability note) | draft authoring; pair with `approver` (or `admin`) as the approving identity; author for direct-publish drives (`admin` publishes) | reviewer identity for reviewer-stage drives (`template.review`); capability-gate negative QA: lacks `template.submit` / `template.publish` — use to prove 403 on publish, not to drive a full lifecycle |
...
**Publish-capability note (TST-03, corrected to runtime truth 2026-07-03):** `Approve(accept=true)` publishes **directly** — including on reviewer-stage templates (`internal/modules/templates/application/lifecycle.go` Approve accept branch; proven live: draft → under_review → approved → published with 3 identities). There is no separate publish call in the workflow path. The reviewer-stage drive recipe is: `admin` authors, `approver-test` reviews, `approver` approves (which publishes) — SoD requires all three to be distinct users.

The separate `POST /templates/{id}/versions/{v}/publish` route is the **direct draft→published editor path** (draft-only, content-hash gate, SoD, approver-role binding, `CapTemplatePublish`). Its handler capability gate (`templatesAuthzFn` → `CapabilityService.CanDo`, `apps/api/cmd/metaldocs-api/main.go`) reads **`iam_user_roles` × `role_capabilities`** — NOT `user_process_areas` — and `iam_user_roles` allows exactly one role per user (`uq_iam_user_roles_user_tenant`) from `{system_admin, approver, author, editor, viewer}` (`qms_admin` is not a legal code there). `template.publish` maps only to `qms_admin`/`system_admin` in `role_capabilities`, so among seeded identities only `admin` (`system_admin`) passes the direct-publish gate; the role binding additionally requires the actor's iam role to equal the template's `approver_role`. Live-proven recipe: `author-test` authors a template with `approver_role='system_admin'`, `admin` publishes (SoD holds — distinct users); `approver-test` publishing → 403 `AUTH_FORBIDDEN` (capability gate intact). The seeded `approver` user's `rh` UPA row carries role `qms_admin` (superset of `approver` caps) so tier-2 in-tx `authz.Require` passes on the approve path; a second concurrent UPA row is impossible (`ux_user_process_areas_one_active`, one active row per user+tenant+area) — see `db/dev-seeds/0001_local_dev_seed.sql`.

**Tripwire fix (migration 0269):** the reviewer stage was bricked at the DB layer until 2026-07-03 — `enforce_capability_asserted()`'s `templates_template_version` branch omitted `template.review`, so every reviewer accept/reject failed SQLSTATE P0001 (500). Fixed in `db/migrations/0269_templates_version_tripwire_review_cap.sql`.
