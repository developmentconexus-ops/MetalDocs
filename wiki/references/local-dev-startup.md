...
**Publish-capability note (TST-03):** `template.publish` is granted only to the `qms_admin` and `system_admin` `role_capabilities` rows (`db/reference-data/0001_product_reference_data.sql`) — the plain `approver` UPA role covers only `template.approve` / `template.review` / `template.view`. When a template's approval config has a reviewer stage (`hasReviewer=true`), `Approve(accept=true)` stops at status `approved` and a separate `POST /templates/{id}/versions/{v}/publish` call (gated on `CapTemplatePublish`) is required to reach `published`. The seeded `approver` user's `rh` UPA row therefore carries role `qms_admin` (whose capability set is a superset of `approver`'s document/template caps) so the same distinct-from-`admin` identity can drive approve AND publish without weakening SoD. A second concurrent role row is impossible — partial unique index `ux_user_process_areas_one_active` allows one active row per (user, tenant, area) — see `db/dev-seeds/0001_local_dev_seed.sql`. Publish itself is SoD-checked (publisher ≠ author AND ≠ reviewer), so reviewer-stage drives need three identities: `admin` authors, `approver-test` reviews, `approver` approves+publishes.
