...
|---|---|---|---|
| `auth.account_inactive` | **403** — identity is proven; the account is deactivated, so re-authenticating cannot help | platform | `internal/platform/problem/codes.go:97` |
| `auth.account_locked` | **403** — identity is proven; the account is administratively blocked, so re-authenticating cannot help | platform | `internal/platform/problem/codes.go:95` |
| `auth.invalid_credentials` | 401 | platform | `internal/platform/problem/codes.go:89` |
| `auth.password_change_required` | **403** — identity is proven; the session is refused until the mandatory password change completes | platform | `internal/platform/problem/codes.go:103` |
| `auth.signature_invalid` | 401 | approval | `internal/modules/approval/http/errors.go:88` |
| `auth.tenant_forbidden` | **403** — identity is proven; the caller may not act on the requested tenant | platform | `internal/platform/problem/codes.go:99` |
| `auth.tenant_required` | **403** — identity is proven; the session carries no tenant claim for a tenant-scoped route | platform | `internal/platform/problem/codes.go:101` |
| `auth.unauthenticated` | 401 | platform | `internal/platform/problem/codes.go:87` |

## `conflict` — default 409

| Code | Status | Module | Declared at |
|---|---|---|---|
| `conflict.already_exists` | 409 | platform | `internal/platform/problem/codes.go:193` |
| `conflict.approval_route_duplicate_profile` | 409 | approval | `internal/modules/approval/http/errors.go:69` |
| `conflict.concurrent_modification` | 409 | platform | `internal/platform/problem/codes.go:194` |
...
| `conflict.duplicate_submission` | 409 | shared | `internal/platform/problem/codes.go:209` |
| `conflict.generic` | 409 | platform | `internal/platform/problem/codes.go:202` |
| `conflict.idempotency_key_reused` | 409 | platform | `internal/platform/problem/codes.go:199` |
| `conflict.mark_reviewed_stale_revision` | 409 | approval | `internal/modules/approval/http/errors.go:147` |
| `conflict.membership_exists` | 409 | platform | `internal/platform/problem/codes.go:205` |
...
| `conflict.stale_base` | 409 | platform | `internal/platform/problem/codes.go:204` |
...
| `notfound.document_profile` | 404 | shared | `internal/platform/problem/codes.go:145` |
...
| Code | Status | Module | Declared at |
|---|---|---|---|
| `permission.capability_denied` | 403 | shared | `internal/platform/problem/codes.go:116` |
| `permission.denied` | 403 | platform | `internal/platform/problem/codes.go:120` |
| `permission.iso_segregation_violation` | 403 | platform | `internal/platform/problem/codes.go:123` |
| `permission.origin_forbidden` | 403 | platform | `internal/platform/problem/codes.go:122` |
| `permission.signoff_actor_not_eligible` | 403 | shared | `internal/platform/problem/codes.go:131` |
| `permission.sod_cross_stage_duplicate` | 403 | approval | `internal/modules/approval/http/errors.go:72` |
| `permission.sod_submitter_cannot_sign` | 403 | shared | `internal/platform/problem/codes.go:132` |

## `precondition` — default 412

| Code | Status | Module | Declared at |
|---|---|---|---|
| `precondition.content_hash_mismatch` | 412 | shared | `internal/platform/problem/codes.go:223` |
...
| Code | Status | Module | Declared at |
|---|---|---|---|
| `request.body_too_large` | **413** — 413 Content Too Large is the dedicated status for an oversized body (RFC 9110 §15.5.14) | platform | `internal/platform/problem/codes.go:58` |
| `request.content_type_unsupported` | **415** — 415 Unsupported Media Type is the dedicated status for a rejected Content-Type (RFC 9110 §15.5.16) | shared | `internal/platform/problem/codes.go:77` |
| `request.cursor_expired` | **410** — 410 Gone: the cursor addressed an item that has been permanently removed, so the caller must restart pagination rather than fix the request syntax | platform | `internal/platform/problem/codes.go:61` |
| `request.cursor_invalid` | 400 | platform | `internal/platform/problem/codes.go:52` |
| `request.empty_body` | 400 | shared | `internal/platform/problem/codes.go:71` |
| `request.header_required` | 400 | approval | `internal/modules/approval/http/errors.go:104` |
| `request.idempotency_key_invalid` | 400 | platform | `internal/platform/problem/codes.go:65` |
| `request.idempotency_key_required` | 400 | platform | `internal/platform/problem/codes.go:66` |
...
| `request.json_decode` | 400 | shared | `internal/platform/problem/codes.go:70` |
| `request.json_type_error` | 400 | approval | `internal/modules/approval/http/errors.go:123` |
| `request.method_not_allowed` | **405** — 405 is the dedicated status for an unsupported method on an existing target (RFC 9110 §15.5.6); the request is otherwise well-formed, so the request. family is right and 400 is not | platform | `internal/platform/problem/codes.go:54` |
...
| `state.approval_instance_completed` | 409 | shared | `internal/platform/problem/codes.go:183` |
...
| `state.approval_route_missing` | 409 | shared | `internal/platform/problem/codes.go:159` |
| `state.approval_stage_not_active` | 409 | shared | `internal/platform/problem/codes.go:184` |
...
| `state.document_profile_archived` | 409 | shared | `internal/platform/problem/codes.go:173` |
...
| `state.transition_invalid` | 409 | platform | `internal/platform/problem/codes.go:155` |
| `state.upload_expired` | **410** — 410 Gone: unlike state.upload_missing, the upload session did exist and has permanently expired, which is the condition 410 asserts (RFC 9110 §15.5.11) | platform | `internal/platform/problem/codes.go:161` |
| `state.upload_missing` | 409 | platform | `internal/platform/problem/codes.go:168` |
...
| `validation.role_unknown` | 422 | platform | `internal/platform/problem/codes.go:234` |
