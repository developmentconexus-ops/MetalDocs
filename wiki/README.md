...
- [backlog/templates-refactor.md](backlog/templates-refactor.md) — templates refactor backlog (16 rows: R-001..R-014 code-quality / authz / spec / envelope / schema items; R-100 retire `templates.md` predecessor; R-101 rename module dir + route prefix) (Last verified: 2026-05-10)
...
- [backlog/controlled-documents-refactor.md](backlog/controlled-documents-refactor.md) — controlled-documents refactor backlog (Last verified: 2026-05-11)
...
- API status snapshot (Plan 8 baseline): `documents Partial`, `approval Raw`, `templates Partial`, `controlled-documents Generated boundary mounted`, `taxonomy Raw`, `audit Partial`, `iam Partial`, `auth/platform Partial` -> see `architecture/api-contract.md`.
...
- **[architecture/trusted-proxy.md](architecture/trusted-proxy.md) - trusted-proxy CIDR allowlist: `METALDOCS_TRUSTED_PROXY_CIDRS` env var, `config.LoadTrustedProxyCIDRs`, `security.IsTrustedRemote` / `ClientIP` helpers, consumers (CSRF origin gate, rate-limiter, auth audit log); fail-closed default; closes C1 + H4 from platform-2a security review (Last verified: 2026-05-21 / `def24e4a`)**
- **[architecture/rate-limiting.md](architecture/rate-limiting.md) - two limiter implementations (`security.RateLimiter` global envelope + `ratelimit.Middleware` per-route token bucket); route-to-limiter mapping; bounded-memory contract — sweep-on-admission, 100_000 fail-closed cap, context-cancellable sweeper; per-route limiter dormant in production wiring; closes C2 + M5 from platform-2a security review (Last verified: 2026-05-21 / `2f8f6dcc`)**
...
### Standards
- **[standards/golang/README.md](standards/golang/README.md)** - MetalDocs Go backend quality bar: typed boundaries, errors/logging, security, idempotency, persistence, handlers, testing, package layout, and refactor playbook; enforced by `.golangci.yml` + Go Lint workflow. (Last verified: 2026-05-22)

### Modules (one per backend module / frontend feature)
- **[modules/templates.md](modules/templates.md)** - **Arc42 + C4 living doc** — templates backend: authoring lifecycle (`draft → in_review → approved → published → obsolete`), 20 HTTP routes, placeholder catalog enforcement, SoD probing, MinIO presigned upload/download, `documents` downstream snapshot contract; §8.8 placeholder catalog enforcement; §8.7 SoD / author identity surface; **Plan 5: `authz.Require` wired in all 6 mutation paths + tripwire on templates tables (T-001 closed)**; companion tech-debt register + refactor backlog linked below (Last verified: 2026-05-11)
- [modules/templates-tech-debt.md](modules/templates-tech-debt.md) - templates tech-debt register (14 items: **T-001 authz wired CLOSED Plan 5**; T-002 tenant guard gap **partially closed Plan 5** (CreateNextVersion guard); T-003 X-Tenant-ID **RESOLVED Plan 3**; **T-004 content_hash + SoD + authz.Require added Plan 5** (role-binding check residual); **T-005 legacy error envelope CLOSED Plan 7**; T-006 partial codegen Major; T-007 parallel audit sink Major; T-008 resolver registry unwired Major; T-009..T-014 additional items) (Last verified: 2026-05-12)
- [modules/templates.md](modules/templates.md) - (predecessor — retire pending R-100) frontend-heavy doc: template authoring screens, List screen, Creation wizard Steps 1–5, `TemplateEditorPage`, `EditorChrome` wiring (Last verified: 2026-05-10)
- `modules/templates/_artifacts/00-context.md` … `06-selfreview.md` — research artifacts (index-only; do not link from other docs)
...
- **[modules/controlled-documents.md](modules/controlled-documents.md)** - **Arc42 + C4 living doc** — controlled-documents: CD catalog, atomic CD + first-revision create (`POST /api/v1/controlled-documents`), per-(tenant, profile, area) sequence numbering (`{PROFILE}-{AREA}-{NNN}`), lifecycle (`active → obsolete | superseded`), `DocumentInitializer` port, idempotency-key replay; Plan 3: all routes use `tenant.FromContext`; **Plan 5: `authz.Require` wired for lifecycle + create; tripwire on `controlled_documents` + `cd_sequence_counters`; `CapControlledDocumentObsolete`/`CapControlledDocumentSupersede` typed consts (T-001/T-004 closed)**; **Plan 7: RFC 9457 envelope + 422 `template_invalid` wired (T-003/T-007 closed)**; companion tech-debt register + refactor backlog linked below (Last verified: 2026-05-12)
- [modules/controlled-documents-tech-debt.md](modules/controlled-documents-tech-debt.md) - controlled-documents tech-debt register (12 items: **T-001 lifecycle authz wired CLOSED Plan 5**; **T-002 audit-trail gap CLOSED Plan 6a**; **T-003 legacy error envelope CLOSED Plan 7**; **T-004 tier-3 tripwire + authz.Require on Create/CreateTx CLOSED Plan 5**; T-005 tenant query-arg only Major; T-006 GetActiveDocument no authz Major; T-007 OpenAPI spec/handler drift — **T-007 422 template_invalid CLOSED Plan 7**; **T-008 cross-module audit sink CLOSED Plan 6a**; T-009..T-012 Minor) (Last verified: 2026-05-12)
- `modules/controlled-documents/_artifacts/00` … `06` — research artifacts (index-only; do not link from other docs)
...
- [concepts/placeholders.md](concepts/placeholders.md) - **CRITICAL:** fixed 7-token catalog, substitution at freeze; composition system deprecated 2026-04-27; links to templates §8.8 for backend enforcement (Last verified: 2026-05-10)
- [concepts/token-syntax.md](concepts/token-syntax.md) - `{name}` vs `{{uuid}}` - why it matters (Last verified: 2026-05-10)
- [concepts/controlled-documents.md](concepts/controlled-documents.md) - code generation (`{profile}-{area}-NNN`, 3-digit), atomic create endpoint (`POST /api/v1/controlled-documents`), preview endpoint, idempotency-key requirement, revision endpoint (Last verified: 2026-05-07)
...
- [concepts/authz-tiers.md](concepts/authz-tiers.md) - two-tier authz model: tier-1 CapabilityService (HTTP middleware) vs tier-2 authz.Require (in-tx area check); GUC setup, pitfalls; **Plan 5: tier-2 coverage table (6 modules, 12 tripwire tables), `CapControlledDocumentObsolete`/`CapControlledDocumentSupersede` added** (Last verified: 2026-05-11)
...
### Decisions (ADRs)
- [decisions/0001-eigenpal-adoption.md](decisions/0001-eigenpal-adoption.md) - why we picked eigenpal over CKEditor/BlockNote; affected modules section links templates (Last verified: 2026-05-10)
...
**Cross-refs:** Use full path + line numbers. Example: `internal/modules/templates/application/schema.go:42`.
