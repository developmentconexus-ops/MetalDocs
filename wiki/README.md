...
- [backlog/novo-template-wizard.md](backlog/novo-template-wizard.md) — **5 deferred items + 2 stub steps** for Template Creation Wizard (`/templates-v2/new`): `template-counts` aggregate endpoint, `chk-disabled` hardcode, `next-code-preview` endpoint, `key-generation` UX decision, `font-size-hero` token gap, `step3-docx-upload` / `step3-placeholder-extract` / `step3-editor-handoff` (3 Step-3 deferred items); Steps 2–3 shipped 2026-05-09; Steps 4–5 (Permissões/Confirmação) not yet implemented. (Last verified: 2026-05-09)
