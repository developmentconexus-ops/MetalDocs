...
> - `db/grants/0001_role_grants.sql` - privilege/role effects `pg_dump --no-privileges` can't carry; applied at bootstrap and unconditionally at every API startup
> - `db/dev-seeds/0001_local_dev_seed.sql` - optional local-only developer accounts/data
> - `db/migrations/` - post-baseline forward tail (currently empty except `README.md`; migrations `0257`-`0315` folded into the baseline 2026-07-29, see `wiki/database/migration-policy.md`)
