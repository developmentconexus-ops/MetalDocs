...
| R-010 | Enforce `ExpectedLockVersion` in `SaveTemplateDraft` — UPDATE … WHERE lock_version = $X; return 412 on mismatch | T-010 | S | Major | — | — | merged (partial) | 2026-05-17: generated `SaveTemplateDraft` uses CAS; legacy `/autosave/commit` remains hash-gated without lock-version input |
