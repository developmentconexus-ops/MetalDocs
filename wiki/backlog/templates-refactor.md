...
|---|---|---|---|---|---|---|---|---|
| R-001 | Wire `CapabilityService` into `tv2http.New` and call `authz.Require(cap, area)` in every `Service` mutation | T-001 | M | Critical | — | — | merged | Plan 5 (2026-05-11): `WithDB` builder + `authz.Require` in all 6 mutation paths + tripwire on both templates tables |
