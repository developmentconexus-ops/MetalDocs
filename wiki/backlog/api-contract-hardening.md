...
- **HARD-STOP boundary hit & reported — Go codegen NOT regenerated (deliberate):** Regenerating the 6 oapi-codegen packages was the obvious step to purge the generated `ApiErrorEnvelope` type, but it flips every runtime route from `/api/v1/*` to `/*`. Root cause: the committed `api.gen.go` files are **pre-existingly stale** — Phase A made the spec path keys relative (AD-1) but explicitly did **not** regenerate (its evidence: "api.gen.go NOT regenerated → served paths stay /api/v1/*"), and Phase C removed schemas from the spec without regenerating either. Proven by experiment: a fresh `go generate` against the **pristine base spec** produces a 658-line diff for the taxonomy package alone (relative routes + Phase-C-removed `DocumentTemplate*` types). oapi-codegen registers the spec path keys verbatim with no base-path option, so regen = relative routes = `404` on every `/api/v1/*` request (reproduced: the controlleddocuments/documents-wrapper/taxonomy contract tests all 404'd under a regen, all green again once reverted). Restoring the `/api/v1` base requires mounting the generated handlers under a stripped prefix — a cross-cutting routing/transport change owned by **Phase A/F**, not the error-envelope scope. The generated `ApiErrorEnvelope` type is confirmed **dead** (no handler returns any `*JSONResponse` error type — grep-verified), so the runtime error shape is already Problem after the emitter migration. **Bounded defer → a dedicated Go-codegen refresh (Phase F or standalone) once the base-path mounting is fixed:** regenerate all 6 packages so the generated `ApiErrorEnvelope` type disappears and `backend-codegen-drift` is honestly green (it is already drifted on base from Phase A/C; Phase D did not touch it, so it neither fixed nor worsened that pre-existing state).
...
| F-ENVELOPE-SPLIT | CRITICAL | `ApiErrorEnvelope` (legacy) vs `Problem` (RFC 9457) coexist | D | closed 2026-06-07 (21 spec refs + schema deleted; 92 inline + 21 legacy → shared `components/responses`; 3 Go emitters + FE fallback migrated; generated dead `ApiErrorEnvelope` type left to a Go-codegen refresh — see Phase D HARD-STOP note) |
| F-NO-500 | HIGH | no operation documents a `500` response | D | closed 2026-06-07 (shared `InternalServerError` response added to 42 error-documenting ops) |
