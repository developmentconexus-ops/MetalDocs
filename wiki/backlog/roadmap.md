...
> **Last verified:** 2026-07-06 (F9.4 doc-truth pass: Plan 5 Touches list flagged **verify** for F9.5 repository/→infrastructure/ rename (historical line-number snapshot not rewritten); Plan 6a Touches list path rewritten `templates/repository/postgres.go`→`templates/infrastructure/postgres.go`) | **Prior:** 2026-06-23 (eigenpal migration: vendored tarball retired, `@eigenpal/docx-editor-react@1.9.0` from npm; Plan 11/Plan 3 touch-points updated)
...
- Module dir rename: `internal/modules/templates/` → `internal/modules/templates/`. Same for any `_v2` suffix in code, columns (`approval_instances.document_v2_id`), or wiki files.
- Canonical audit sink: `metaldocs.audit_events` (audit module). Parallel sinks (`templates_audit_log`, `governance_events`) collapse onto it in Plan 6.
...
- **Touches:** `internal/modules/iam/infrastructure/postgres/role_admin_repository.go:33,72`, `user_area_repository.go:51,75,90`; `internal/modules/documents/infrastructure/repository.go:73,216,428,1071,1082` (**verify** — dir renamed from `repository/` per F9.5, historical Plan-5 snapshot line numbers not re-verified); `internal/modules/registry/infrastructure/repository.go:133,184,208,239`; `internal/modules/taxonomy/application/{family,profile,area}_service.go` + dispatcher (`apps/api/cmd/metaldocs-api/permissions.go:174` — add `MethodPatch`); `internal/modules/templates/delivery/http/handler.go:24` (wire real authz) + `apps/api/cmd/metaldocs-api/main.go:329`; `migrations/<next>_tripwire_extend.sql` (attach trigger to new tables).
- **Closes:** iam T-004/R-004; documents T-003/R-003; registry T-001/R-001, T-004/R-004; taxonomy T-003/R-003, T-006/R-006, T-013/R-013; templates T-001/R-001, T-002/R-002, T-004/R-004.
- **Critical rows closed:** 6 (registry T-001, taxonomy T-003, templates T-001/T-002/T-004 — T-002 needs tenant-scoped getters from Plan 3 + tier-2 enforcement here).
- **Blockers:** Plan 4 (stable cap names).
- **Status:** done 2026-05-11. Commits: `0dba5589` (CapRegistryObsolete/Supersede), `37cbcc7b` (PATCH taxonomy routes), `aa6d96a7` (obsolete/supersede cap routing), `1ab62d49` (migration 0187), `b9192b95` (IAM role_admin tier-2), `70aeccd2` (IAM user_area tier-2), `8b565d67` (documents tier-2), `2e156721` (registry tier-2), `d26ac392` (taxonomy tier-2), `4cd03873` (templates tier-2 + T-002 + T-004 + real authz wire), `fdcf90e7` (migration 0188). Spec: `docs/superpowers/specs/2026-05-11-plan-05-tripwire.md`.
...
- **Touches:** `internal/platform/problem` (exists from Plan 2 — confirm); per-module handler error helpers: `internal/modules/iam/delivery/http/middleware.go:129` + `routes_memberships.go:137`; `internal/modules/documents/delivery/http/handler.go:958,1013`; `internal/modules/auth/delivery/http/handler.go:166` + `middleware.go:65`; `internal/modules/documents/approval/http/errors.go:147`; `internal/modules/audit/delivery/http/handler.go:48`; `internal/modules/templates/delivery/http/handler.go:95`; `internal/platform/httpresponse/response.go:14` (registry + taxonomy consumers); frontend `frontend/apps/web/src/lib/api/` parser.
- **Closes:** iam T-006/R-006; documents T-001/R-001; auth T-003/R-003; approval T-001/R-001, T-003/R-003; audit T-002/R-002; templates T-005/R-005; registry T-003/R-003, T-007/R-007; taxonomy T-008/R-008.
- **Critical rows closed:** 1 (approval T-001).
- **Blockers:** prefer after Plan 6 so audit emits same shape.
- **Status:** done 2026-05-11. Commits: `11589032` (httpresponse cascade fix), `1ecfe674` (IAM), `97822581` (IAM test fix), `95ebedfc` (auth), `5b792150` (documents), `b8747d6a` (approval + T-003 fix), `2ca727d6` (audit), `bbe3933b` (templates), `395b0b24` (registry + T-007), `f0bb64c0` (taxonomy test), `c4f5535f` (frontend mutationClient). Spec: `docs/superpowers/specs/2026-05-11-plan-07-rfc9457.md`.
...
- **Closes:** documents T-002/R-002, T-004/R-004, T-010; approval T-002/R-002; templates T-006/R-006; registry T-007/R-007, T-011/R-011; audit T-008/R-008; taxonomy T-009/R-009.
...
- **Touches:**
  ...
  - Rename: legacy template module/routes into `internal/modules/templates/` + canonical URL `/api/v1/templates/*` (templates R-101). Mirror documents, controlled-documents, taxonomy, and approval doc-scoped routes to `/api/v1/*`. Frontend `lib/api/` updated.
...
- **Touches:**
  - Doc comments: every module flagged its own missing-doc T-row (iam T-013-equiv via R-013, documents implicit, auth T-011/R-011, approval T-010/R-010, audit T-012/R-012, templates T-014/R-014, registry T-012/R-012, taxonomy T-014/R-014).
