...
|---|---|---|
| `GET /api/v2/documents/:id/distribution` | KPI metrics (totalTargets, acknowledged, read, pending, overdue) + DistributionFacts sidebar data (publishedAt, deadline, policy, channel, reminders, groups) | High |
| `GET /api/v2/documents/:id/distribution/recipients` | Paginated recipient table with status, last activity, timestamps | High |
| `GET /api/v2/documents/:id/distribution/coverage` | By-area breakdown (area name, total, read, ack counts) | Medium |
| `GET /api/v2/documents/:id/distribution/timeline` | Cumulative daily read events for timeline chart | Low |
...
|---|---|---|
| Lembrete em massa | `POST /api/v2/documents/:id/fanout/reminders/bulk` | No module |
| Exportar relatório | `GET /api/v2/documents/:id/distribution/export` | No module |
| Adicionar destinatários | `POST /api/v2/documents/:id/fanout/recipients` | No module |
| Política de fanout | Settings modal (no design) | No design |
| Row action — mail | `POST /api/v2/documents/:id/fanout/reminders/:recipientId` | No module |
...
Real pagination requires `GET /api/v2/documents/:id/distribution/recipients?page=&pageSize=` with `Link`/`X-Total-Count` response headers (matching the library pagination pattern).
