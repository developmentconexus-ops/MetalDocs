...
- This is the route-admin half of the two-tier authz model (`wiki/decisions/0007-two-tier-authz.md`): the route admin **declares** a capability and the runtime freezes it at submit. **Status (2026-06): the per-stage `required_capability` is advisory config, NOT yet an enforcement input.** Sign-off eligibility is resolved on `required_role` + `area_code` (`submit_service.go resolveEligibleActors`), and the capability enforced in-tx at sign-off is the fixed `CapDocumentSignoff` (`decision_service.go`), not the per-stage pin. As of ADR 0022 the field IS bound to the IAM capability registry at config time (`validateRequiredCapability` — no phantom values), so wiring true per-stage enforcement later (its own ADR, per ADR 0022 scope) would start from validated data. The pin/freeze semantics above hold for whenever that enforcement lands.
