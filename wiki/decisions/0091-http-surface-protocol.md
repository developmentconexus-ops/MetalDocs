...
A failure here is `slog.Error("http surface", "err", err)` followed by `os.Exit(1)` (`main.go:880-884`) — a fatal, not a logged warning. The Definition of Done for this program requires a real boot to succeed with no `http surface:` fatal, on both the ordinary path and `METALDOCS_E2E=1`, and requires that commenting out one publisher produces exactly that fatal (verified in Task 15 step 9).
