...
| Auth route | — (never mounted; route + page deleted) | `auth/pages/AuthRoutePage.tsx` *(deleted)* | cut | cut (A2.2 — deleted) | **Deleted** in A2.2 dead-code burn-down (PR #119): was an unmounted `authRoutes` export with zero importers (`auth/routes.tsx` also deleted). `/login` remains the live public auth entry, unaffected. |
