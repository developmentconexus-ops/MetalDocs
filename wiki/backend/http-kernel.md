...
> **Key files:**
> - `apps/api/cmd/metaldocs-api/main.go` — 943-line composition root (config → DI → routes → lifecycle)
> - `apps/api/cmd/metaldocs-api/permissions.go` — tier-1 route → capability/visibility truth table (286 lines)
...
13. **HTTP server** — compose the middleware chain (`main.go:595-602`), resolve listen address (`:8080` default, `APP_PORT` override — the canonical dev script sets `APP_PORT=8081` at `scripts/start-api.ps1:241`), build `http.Server` with `ReadHeaderTimeout: 5s` only (`main.go:613-617`), log startup summary, serve in a goroutine feeding a `serverErr` channel, then block in `shutdownServer` (`main.go:627`).

---

## 5. Middleware chain

Composition at `main.go:598-602`, outermost → innermost:
...
5. **Presence bump** (`internal/modules/iam/presence/middleware.go:67-99`): if a user ID is in context, fire-and-forget goroutine updates `iam_users.last_seen_at`, debounced 60s/user (`presence/model.go:27`), 2s DB timeout (`middleware.go:47-49`). Wrapped into the chain only when an SQL DB exists (`main.go:599-601`).
6. **HTTP observability** (`internal/platform/observability/http.go:59-107`): resolves or creates `X-Trace-Id` into context; measures status + duration; aggregates per-route RED metrics with p50/p95/p99 ring buffers (`http.go:266-301`); emits one JSON `http_request` log line per request.
...
8. **ServeMux dispatch** to module handlers; module errors surface as RFC 9457 `application/problem+json` (`internal/platform/problem/problem.go:76-83`).

### Middleware ordering consequence

Because `httpObs` and `rateLimiter` sit inside authn, requests rejected by CORS, origin protection, or authn are **not counted in RED metrics and not rate-limited**. There is no pre-auth IP-keyed rate limit tier for `/api/v1/auth/login` at the middleware layer (login throttling exists separately as account lockout in `authn/config.go:88-104` via `METALDOCS_AUTH_LOGIN_MAX_FAILED_ATTEMPTS`/`_LOCK_MINUTES`). This is a documented deviation matching RF-2 in `../architecture/backend-target-architecture.md` (REQ-MW-4, REQ-MW-5).
...
| **Middleware order deviation:** `httpObs` and `rateLimiter` inside authn — 401/403/CORS rejects invisible to RED metrics; no pre-auth IP-keyed limit tier for `/api/v1/auth/login` | high — already registered | RF-2 |
| **No panic-recovery or request-ID middleware at the outermost layer** — a panic in any middleware crashes the connection with no `problem+json` or tagged log line | medium | RF-2 / REQ-MW-1 |
| **No chain-order test** — the composed handler order at `main.go:598-602` is not asserted by any test; reordering compiles silently | medium | REQ-MW-7 |
| **Server timeouts incomplete:** only `ReadHeaderTimeout: 5s`; no `ReadTimeout`, `WriteTimeout`, or `IdleTimeout` — slow-body clients hold connections indefinitely | medium | RF-9 |
