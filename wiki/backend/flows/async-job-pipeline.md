...
3. `platform/worker.Service.RunOnce` (`service.go:42`) calls `consumer.ClaimUnpublished` (`outbox/postgres/consumer.go:34`) — one `platform/db.TxRunner.Do` transaction runs the CTE and row scan, claiming up to `batchSize` rows from `outbox_events` with `FOR UPDATE SKIP LOCKED`, bumping `attempt_count`, and setting `next_attempt_at = now() + claimLease` as the in-flight lock.
...
| Worker crash (in-flight at claim time) | `next_attempt_at` elapses after `claimLease = max(RetryMaxSeconds, 5min)`; row becomes claimable again | `consumer.go:34` |
