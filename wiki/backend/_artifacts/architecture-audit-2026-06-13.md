...
| H-1 boundaries | ◑ H-1a resolved | (this commit) | `setAuthzGUC` ×4 dedup → canonical `authz.SeedTxIdentity`: deleted the approval + templates copy-paste def files and the inline CD copy; their 17/11/4 call sites swapped to `authz.SeedTxIdentity` (gains the empty-string guards + single batched GUC query the copies skipped). taxonomy kept its ctx-resolving wrapper (resolves tenant/actor from ctx) but delegates the duplicated GUC *SQL* to the canonical — its 24 call sites untouched (inlining ctx-resolution ×24 is worse DRY than one wrapper). Test fixtures (sqlmock + custom `database/sql/driver` fakes) migrated from the two-separate-query form to the batched 2-arg query; actor-capture fakes re-pointed `args[0]`→`args[1]`. Gates: build/vet/`test -p 2 ./...`/`api-lint -strict` 0/cilint 0. Contract-neutral. H-1b–e (CD→taxonomy port · approval delivery↛infra · `*sql.DB`→TxRunner · documents delivery redesign) open. |
