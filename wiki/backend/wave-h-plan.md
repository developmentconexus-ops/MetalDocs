...
#### H-1d — `*sql.DB` → `TxRunner` port (the hexagonal root fix)  ·  ✅ DONE — approval portion (this commit); CD+templates → H-1d′ (next commit)
...
**Execution (2026-06-13):**
- **Port:** `internal/platform/db/runner.go` — `type TxRunner interface { Do(ctx, fn func(tx *sql.Tx) error) error }`, `NewTxRunner(*sql.DB)` (panics on nil — wiring error, never runtime), `Do` = BeginTx → defer recover→rollback→repanic → `fn` (rollback + return-unwrapped on err) → Commit (wrapped). Direct unit tests in `runner_test.go` (nil-panic, commit-on-success, rollback+unwrapped-err, re-panic+rollback, begin-error wrap, commit-error wrap) — fake `database/sql/driver` recording begin/commit/rollback.
- **Deviation #1 — callback type `func(*sql.Tx)` not plan's `func(db.Tx)`:** deferred boundary #4 keeps `authz.Require`/`SeedTxIdentity` on `*sql.Tx`, so a `db.Tx` callback would force a `mustSQLTx` assert-back at every authz call inside every unit of work (a smell, not a win). Exposing the live `*sql.Tx` in the callback is the **deliberate bounded concession** documented in the `runner.go` doc comment. What the port removes is the application layer's dependency on the `*sql.DB` *pool* + its ownership of commit/rollback — that is the finding.
- **Deviation #2 — build runner inside delivery/job constructors, do NOT touch composition roots:** each delivery handler / job worker builds `db.NewTxRunner(database)` from the `*sql.DB` it already receives (nil-guarded where tests inject nil). Constructor signatures stay `*sql.DB`, so `apps/api/.../main.go`, `apps/jobs/.../main.go`, and `documents/module.go` wiring + their tests are **untouched** (zero churn to composition roots — supersedes the plan's "Update main.go wiring"). The application layer is now free of concrete `*sql.DB`; that is the boundary the finding targets.
- **Scope split:** this commit = **approval only** (application services + delivery http handlers + approval/jobs worker + `stuck_instance_watchdog` + `documents/delivery/http` + `documents/module.go` interface). **CD + templates** hold `s.db *sql.DB` as a *field* and call `s.db.BeginTx` internally (CD 4 sites, templates 11) — same root coupling, larger surface → **H-1d′ (next commit), same finding ID.**
- **cilint:** the port's own `BeginTx/Commit/Rollback` in `runner.go` tripped the `txownership` analyzer. Root-cause fix (not a suppress-directive): added `/platform/db/` to `allowedTxPackages` (`tools/cilint/internal/analyzers/analyzers.go`) — the TxRunner port is now the canonical centralized tx-lifecycle owner the allowlist comment already sanctions as "shared platform transaction infra." cilint self-tests green.
- **Test-fix strategy:** application test package gets a `newTxRunner(database *sql.DB) db.TxRunner` helper (`txrunner_test.go`, named to dodge the local `db` var that shadows the `db` package) — service call sites wrap `db` → `newTxRunner(db)`; builders untouched (direct-tx builders `newSQLEmitterTestDB`/`newTestDB`/`newSupersedeTestDB` stay `*sql.DB`). http/job/delivery test fakes retype the discarded `_ *sql.DB` param → `_ db.TxRunner`. The runner's begin→fn→commit/rollback mirrors the old service code, so all fake-driver/sqlmock expectations pass unchanged.

**Gates (all green):** `go build ./...` 0 · `go vet ./...` 0 · `go test -p 2 ./...` 0 failures · `api-lint -strict` 0 violations · `cilint` exit 0. Contract-neutral (internal application-layer wiring only — no OpenAPI/route/codegen change).

**Code review disposition (sonnet reviewer, 5 findings, all triaged non-actionable with evidence):** (1/2) *nil `h.runner` / `idempotency.New(nil)` panic risk* — **not a regression**: the old handlers stored a nil `h.db` and passed it to services, so a nil DB panicked at first use identically; verified `h.runner` is only ever *passed to* a service method (faked in tests), never dereferenced by a handler, and no test drives a real runner-consuming path with nil (tests pass non-nil `&sql.DB{}` or use fakes that ignore the runner arg); production wiring always passes a non-nil pool. The `database != nil` constructor guard is deliberate so `finalize_wiring_test.go` can test route wiring without a DB; adding speculative nil-guards would violate "no error handling for impossible scenarios." (3) *watchdog mixes `runner` + raw `BeginTx`* — **by design**: `acquireRunLock` needs a raw `db.Conn` for the session-level advisory lock (the runner cannot expose that), and `listStuckInstances`/`emitStuckAlert` are legitimate in-job tx owners (`jobs/` is txownership-allowlisted); `runner` is correctly used only at the cancel-**service** boundary. (4) *decision nested `runner.Do` for eligibility rejection* — reviewer confirmed clean (eligibility domain error returned unwrapped, `errors.Is` intact). (5) *read methods wrapped in default RW tx* — **not a regression**: authz GUC writes + `SELECT … FOR UPDATE` stage loads always required a tx, so the old read paths already used `BeginTx`; a `ReadOnly` opt is out of H-1d scope and negligible on a single primary. No code changes warranted.

#### H-1d′ — CD + templates `s.db *sql.DB` field → `TxRunner` (same finding)  ·  **split into two module-bounded commits: templates ✅ DONE · CD ☐ next**
`controlleddocuments/application/service.go` (`db *sql.DB` field; `s.db.BeginTx` at :197/:385/:501/:605) and `templates/application/service.go` (`db *sql.DB` field via `WithDB`/`DB()`; `s.db.BeginTx` at ~11 sites: autosave.go:125/180, approval_config.go:72, create.go:62/154, lifecycle.go:71/255/403/478/505, schema.go:74) hold the pool as a field and own commit/rollback internally — the same hexagonal-root coupling H-1d removed from approval, field-injected rather than per-call. Convert the field to `runner db.TxRunner`, rewrite each `s.db.BeginTx` block as a `runner.Do` closure. **Split rationale:** templates and CD are separate Go packages with no shared edit surface; splitting isolates the riskier CD `Create` restructure (conditional auto-code tx) into its own reviewable commit.

**H-1d′-templates ✅ DONE (this commit):** `templates/application/service.go` drops `WithDB`/`DB()`, gains `runner db.TxRunner` field + `WithRunner(db.TxRunner)`. All 11 `s.db.BeginTx` sites (create.go ×2, lifecycle.go ×5 incl. `updateVersionWithAuthzAndAudit`, autosave.go ×2, approval_config.go ×1, schema.go ×1) rewritten as `s.runner.Do(ctx, func(tx *sql.Tx) error {…})` — faithful conversion: SeedTxIdentity→Require→repo order preserved, error wraps verbatim (begin/commit wrappers dropped), closure errors propagate unwrapped, values read by the return (audit events, `next` draft) hoisted before the closure, **no `authz.WithCapCache` added** (templates has zero). The handler's idempotency store no longer reaches through `h.svc.DB()`: `templates/delivery/http/handler.go` gains a `db *sql.DB` field and `New(svc, authz, database *sql.DB)` 3rd arg, injected at the composition root (`main.go` `buildTemplatesModule` now passes `.WithRunner(db.NewTxRunner(deps.SQLDB))` to the service and `deps.SQLDB` to `templateshttp.New`) — fully removes `*sql.DB` from the application layer (delivery owns its own pool, mirroring CD's handler). Tests: 31 `.WithDB(x)`→`.WithRunner(newTxRunner(x))` across 6 app-package test files + 3 changes in `routes_create_test.go`; one `newTxRunner` helper per test package (`application_test`, `http_test` — the latter aliased `platformdb` to dodge the file's existing unaliased `db` import). **Gates (all green):** `go build ./...` 0 · `go vet ./...` 0 · `go test ./internal/modules/templates/... ./internal/platform/db/...` ok · `api-lint -strict` 0 · `cilint` 0 · completeness grep (residual `s.db`/`.WithDB`/`DB()` in prod files) empty. **Review:** tx-fidelity reviewer APPROVE 0 findings; wiring reviewer's 1 "major" (double `newPermissiveMockDB` in `newMux`) did not reproduce against disk (file shares one `db` instance — agent self-report was stale) + 1 minor (nil db in `TestNew_PanicsWithoutAuthz`, intentional — panics on nil authz before any db use). Contract-neutral. **Commit:** `refactor(templates): TxRunner port replaces *sql.DB field (H-1d′)`

**H-1d′-CD ✅ DONE (commit pending this run):** `controlleddocuments` — `db *sql.DB` field → `runner db.TxRunner`; 4 sites converted: PeekSeq + CreateRevision (var-hoist + `s.runner.Do` wrapper), changeStatus (`return s.runner.Do(…)`; FOR UPDATE area-load kept before `authz.Require` per ADR 0022 Phase 7), Create (split into manual-code branch — non-tx, no SeedTxIdentity/Require — and auto-code branch — single `runner.Do` closure: SeedTxIdentity→Require→override-resolve→ensureTemplateArtifact→NextAndIncrement→CreateTx→CloneTemplate — keeping the last three atomic in one tx). `module.go` builds `db.NewTxRunner(deps.DB)`; handler keeps its own pool for idempotency (unchanged). No public contract change. **Pre-existing asymmetries (NOT introduced here — preserved verbatim + flagged for operator):** manual-code Create bypasses tier-2 `authz.Require` (no SeedTxIdentity/Require/tx); CreateRevision double-wraps the clone error (errors.Is on `ErrActiveRevisionExists` still holds); `TestCreateRevision_MissingActor…` has stale sqlmock Begin/Rollback expectations (actor check fires before any tx — test still passes). All gates green (go build, go vet, go test ./internal/modules/controlleddocuments/…, api-lint 0, cilint); 3 adversarial reviews: authz-order/asymmetry APPROVE 0, tx-atomicity APPROVE + 1 pre-existing minor, wiring/tests APPROVE + 1 pre-existing minor. **Commit:** `refactor(controlleddocuments): TxRunner port replaces *sql.DB field (H-1d′)`

**H-1d″ ☐ follow-up (out of H-1d′ scope):** `documents/application` Service still holds `*sql.DB` via `documents/module.go:66 svc.WithDB(deps.DB)` (service.go:574/803/850, fillin_authz.go:20, fillin_service.go, reconstruct_service.go:27 + view_service.go:45 read-only, context_builder.go:25) — same field coupling, separate package → its own commit, same finding ID. (**next after H-1d′-CD**)
