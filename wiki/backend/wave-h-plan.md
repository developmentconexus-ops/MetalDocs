...
#### H-1d′ — CD + templates `s.db *sql.DB` field → `TxRunner` (same finding, next commit)
`controlleddocuments/application/service.go` (`db *sql.DB` field; `s.db.BeginTx` at :197/:385/:501/:605) and `templates/application/service.go` (`db *sql.DB` field via `WithDB`/`DB()`; `s.db.BeginTx` at ~11 sites: autosave.go:125/180, approval_config.go:72, create.go:62/154, lifecycle.go:71/255/403/478/505, schema.go:74) hold the pool as a field and own commit/rollback internally — the same hexagonal-root coupling H-1d removed from approval, field-injected rather than per-call. Convert the field to `runner db.TxRunner`, rewrite each `s.db.BeginTx` block as a `runner.Do` closure. **Rework `templates/delivery/http/handler.go:69` `h.svc.DB()`** (used for idempotency) — either expose the runner or keep a separate `*sql.DB` for the idempotency store. **Commit:** `refactor(controlleddocuments,templates): TxRunner port replaces *sql.DB field (H-1d′)`
