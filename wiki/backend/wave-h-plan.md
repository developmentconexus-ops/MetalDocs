...
**H-1d′-CD ☐ next:** `controlleddocuments` — convert the `db *sql.DB` field → `runner db.TxRunner`; 4 sites: PeekSeq(385)/changeStatus(501)/CreateRevision(605) are clean, **Create(146) is the tricky one** (conditional tx — auto-code path only, lines 197-324; manual-code path uses non-tx `s.docs.Create` with no authz.Require/seed — a **pre-existing asymmetry to PRESERVE and FLAG to the operator, never patch inside a tx refactor**). `module.go` builds the runner from `deps.DB`; the CD handler already receives `deps.DB` separately for its own idempotency. **Commit:** `refactor(controlleddocuments): TxRunner port replaces *sql.DB field (H-1d′)`

**H-1d″ ☐ follow-up (out of H-1d′ scope):** `documents/application` Service still holds `*sql.DB` via `documents/module.go:66 svc.WithDB(deps.DB)` (service.go:574/803/850, fillin_authz.go:20, fillin_service.go, reconstruct_service.go:27 + view_service.go:45 read-only, context_builder.go:25) — same field coupling, separate package → its own commit, same finding ID.
