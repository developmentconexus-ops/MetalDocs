...
| 0.4 | History rewrite: `git filter-repo --replace-text` + force-push; user re-clones | F-18 | ✅ | (rewrite itself) | Executed 2026-06-11 with explicit user authorization: `git filter-repo --replace-text` (old secret → `***REDACTED***`) on a fresh mirror clone, 3057 commits rewritten; force-pushed all `refs/heads/*` + `refs/tags/*` to origin. **Verified on a FRESH clone of origin: `git log --all -S '<secret>'` = empty.** Local repo reset onto rewritten origin. Residual: GitHub server-side cached/PR refs may retain old objects (support-ticket purge if ever needed — accepted per D-4, single-user repo) |
