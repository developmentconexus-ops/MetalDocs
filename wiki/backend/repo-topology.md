...
1. `deploy/docker/api.Dockerfile` uses `golang:1.26.5-alpine` as builder stage — matching `go.mod`'s `go` directive, enforced by `tools/verify` check `dockerfile-go-version-drift` (`scripts/check-dockerfile-go-version.sh`); the version is a TRANSITIONAL restated literal (a Dockerfile `FROM` line cannot read `go.mod`), preceded by an 11-line comment explaining why (`api.Dockerfile:1-11`). *(Corrected 2026-08-11: earlier revision said `golang:1.25-alpine`, from before the go.mod bump for GO-2026-5856; see `docs/runbooks/dockerfile-go-version-pin.md`.)*
2. `go build -o /out/metaldocs-api ./apps/api/cmd/metaldocs-api` produces a static binary (`api.Dockerfile:17`).
3. Final image is `alpine:3.21` + `ca-certificates`; `db/migrations/` is copied into `/app/db/migrations/` so the binary can run migrations at container start (`api.Dockerfile:19-24`).
4. Worker Dockerfile follows the same pattern but without copying migrations (`worker.Dockerfile:12-25`, preceded by the same guard comment block as api.Dockerfile at `worker.Dockerfile:1-11`).
