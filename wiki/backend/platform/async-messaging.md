...
- `consumer.go`: `NewConsumer` constructs a `platform/db.TxRunner` over the same pool (`outbox/postgres/consumer.go:25–29`). `ClaimUnpublished` runs one `runner.Do` transaction containing the complete CTE query and row scan (`:34–130`): `FOR UPDATE SKIP LOCKED`, bump `attempt_count`, set `next_attempt_at = now() + claimLease` as an in-flight lock, and return up to `limit` rows. `MarkPublished` sets `published_at = now()`. `MarkFailed` updates `next_attempt_at` with exponential backoff or sets `dead_lettered_at` when `attempt_count >= MaxAttempts`.
...
Two open `TODO(phase11)` markers at `outbox/postgres/consumer.go:40–41` (heartbeat configurability and partial-index alignment) and one at `outbox/postgres/publisher.go:29` (TEXT-backed idempotency key).
