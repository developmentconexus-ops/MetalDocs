...
// Task 10: postpones due_at on an instance's active stage — a recorded,
// auditable exception for ONE instance (Task 9's contract, see
// api/openapi/v1/openapi.yaml `extendApprovalSLA`). Response is the narrower
// ApprovalInstanceResponse (no `viewer`/`verdicts`), not the
// ApprovalInstanceByDocumentResponse `getInstance` returns — callers must not
// treat it as a drop-in cache replacement; see useExtendSLA's invalidation.
type ExtendSLARequest = components['schemas']['ExtendApprovalSLARequest'];
type ExtendSLAResponse = components['schemas']['ApprovalInstanceResponse'];
...
// Task 10: no `resourceId`/`ifMatch` — the backend does the forward-only
// due_at check and write in one transaction (no If-Match precondition), so
// this is the one approval mutation that never needs the ETag cache. The
// Idempotency-Key is still auto-generated by `mutate` (opts left empty).
export function extendSLA(
  instanceId: string,
  body: ExtendSLARequest,
): Promise<ExtendSLAResponse> {
  return mutate('POST', `${BASE}/approval/instances/${instanceId}/extend-sla`, body, {});
}
