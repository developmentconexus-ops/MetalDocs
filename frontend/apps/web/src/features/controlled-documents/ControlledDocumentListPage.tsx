...
function resolveDetailId(pathname: string, basePath: string) {
  const detailPrefix = `${basePath}/`;
  if (!pathname.startsWith(detailPrefix)) {
    return null;
  }
  const candidate = pathname.slice(detailPrefix.length).split('/')[0];
...
export function ControlledDocumentListPage({ basePath = '/controlled-documents', onOpenDocumentEditor }: Props = {}) {
  ...
  const detailId = resolveDetailId(location.pathname, normalizedBasePath);
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          ...
          <tbody>
            ...
            {docs.map((doc) => (
              <tr
                key={doc.id}
                onClick={() => navigate(`${normalizedBasePath}/${doc.id}`)}
