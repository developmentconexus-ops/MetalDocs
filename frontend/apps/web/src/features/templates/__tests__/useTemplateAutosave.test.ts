...
beforeEach(() => {
  ...
  // the jsdom realm, and webcrypto digest can do a strict instanceof check
  // against its own realm's ArrayBuffer. That is a cross-realm test artifact,
  // not a product defect: in a browser the buffer is same-realm. These tests
  // cover upload-failure handling (F-FE1/F-FE7), not hashing, so stubbing the
  // digest costs no coverage.
