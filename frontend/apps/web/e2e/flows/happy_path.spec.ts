...
test.describe.serial('happy_path', () => {
  ...
  test('submits document and badge transitions to under_review', async ({ page }) => {
    ...
    await expect.poll(() => stateBadgeText(page), { timeout: 5000 }).toBe('Em revis�o');
    await expect(page.getByRole('button', { name: /Documento em revis�o/i })).toBeVisible();
  });
  ...
  test('approver signs stage 2 -> approval completes (release follows asynchronously)', async ({ browser, baseURL }) => {
    ...
    try {
      ...
      // "Aprovado" until the coordinator releases it — and in an artifact-less harness
  ...
  test('timeline shows 4 nodes with actors', async ({ page }) => {
    ...
    const timelineNodes = page.locator('section[aria-label="Timeline de aprova��o"] li');
  ...
  test('governance event chain � exact types and order', async ({ request }) => {
    ...
    // instead of terminating it — assert the synchronous prefix, allow the async tail.
