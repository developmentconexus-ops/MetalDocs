...
let uploadedSize = 0;
async function buildTemplateDocx(bodyInnerXml) {
    const zip = new JSZip();
...
vi.mock('../../s3', async () => {
    return {
        makeS3Client: () => ({}),
        getObjectBuffer: async (_c, _b, _key) => templateBuf,
        putObjectBuffer: async (_c, _b, key, data) => {
            uploadedKey = key;
...
let app;
beforeAll(async () => {
    templateBuf = await buildTemplateDocx(`<w:p><w:r><w:t>{doc_code}</w:t></w:r></w:p>`);
