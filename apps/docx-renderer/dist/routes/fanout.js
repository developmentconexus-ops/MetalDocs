...
function frozenDocxKey(tenantId, revisionId) {
    return `tenants/${tenantId}/revisions/${revisionId}/frozen.docx`;
}
export function registerFanoutRoute(app, env, s3Factory) {
    app.post('/render/fanout', async (req, reply) => {
        ...
        const { body_docx_s3_key, tenant_id, revision_id, placeholder_values, composition_config, resolved_values, } = parsed.data;
        const output_key = frozenDocxKey(tenant_id, revision_id);
        const client = s3Factory();
        const bodyBuf = await getObjectBuffer(client, env.DOCGEN_V2_S3_BUCKET, body_docx_s3_key);
        ...
        await putObjectBuffer(client, env.DOCGEN_V2_S3_BUCKET, output_key, Buffer.from(result.buffer), DOCX_MIME);
