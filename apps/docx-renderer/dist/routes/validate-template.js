...
export function registerValidateTemplate(app, env, s3Factory) {
    app.post('/validate/template', async (req, reply) => {
        ...
        try {
            ...
            reply.code(422).send({
                valid: false,
                parse_errors: [{ type: 'malformed_schema', raw: String(e.message) }],
        ...
        try {
            ...
            reply.code(422).send({
                valid: false,
                parse_errors: [{ type: 'schema_invalid', raw: String(e.message) }],
                missing_tokens: [], orphan_tokens: []
            });
            return;
        }
