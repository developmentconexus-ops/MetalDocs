...
import { runPdfJob } from '../worker.js';
let fetchMock;
beforeEach(() => {
    fetchMock = vi.fn();
...
describe('runPdfJob', () => {
    test('reads DOCX, converts, writes PDF, returns metadata', async () => {
        fetchMock.mockResolvedValueOnce(new Response(Buffer.from('pdf-bytes'), { status: 200 }));
        const deps = makeDeps();
        const result = await runPdfJob(makeInput(), deps);
        expect(deps.getObject).toHaveBeenCalledWith('final/r.docx');
        expect(fetchMock).toHaveBeenCalledWith('http://gotenberg.local/forms/libreoffice/convert', expect.objectContaining({ method: 'POST' }));
        expect(deps.putObject).toHaveBeenCalledWith('final/r.docx.pdf', expect.any(Buffer), 'application/pdf');
