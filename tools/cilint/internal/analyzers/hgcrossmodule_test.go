...
func TestHGCrossModule_Negative_PendingBaseline(t *testing.T) {
	...
	// milestone ports it. Uses the real B1 site (documents/repository/repository.go
	// reading controlled_documents).
	src := "package repository\n" +
		"func q() string {\n" +
		"\treturn `SELECT profile_code FROM controlled_documents WHERE id = $1 AND tenant_id = $2`\n" +
		"}\n"
	path := hgFixture(t, "internal/modules/documents/repository/repository.go", src)
	findings := analyzers.HGCrossModule([]string{path})
	if len(findings) != 0 {
