...
# Same shape as Invoke-DbSqlFile but for a SQL statement built in-memory
# (a password rotation cannot come from a checked-in file). $Sql is piped
# straight into psql's stdin and is never written to $Label, Write-Host, a
# temp file, or any other place that could land in a transcript or log.
function Invoke-DbSqlString {
  param([string]$Sql, [string]$Label)

  Write-Host "[dev-bootstrap-baseline] Applying $Label"
  $Sql | docker compose -f $ComposeFile --env-file $EnvFile exec -T postgres `
    psql -v ON_ERROR_STOP=1 -U $env:POSTGRES_USER -d $env:POSTGRES_DB | Out-Host

  if ($LASTEXITCODE -ne 0) {
    throw "[dev-bootstrap-baseline] SQL apply failed: $Label"
  }
}
...
# thread #5 -- a hardcoded CREATE-time password was a committed credential in
# a public repo). Unconditional is safe here specifically because this
# script's first step, dev-db-reset.ps1, stops the app containers and tears
# down the Postgres container + volume before anything below this line runs:
# every invocation starts from an empty cluster with no live services
# attached, so there is nothing already depending on a prior rotation to
# clobber. Contrast tests/integration/testdb.ApplyCuratedBootstrap, which can
# run against an already-provisioned, already-serving shared Postgres and
# therefore rotates conditionally (only when the role still has no
# password). Do not simplify this back to unconditional over there.
$runtimePassword = if ([string]::IsNullOrWhiteSpace($env:METALDOCS_RUNTIME_DB_PASSWORD)) { "metaldocs_runtime_dev" } else { $env:METALDOCS_RUNTIME_DB_PASSWORD }
$ciPassword = if ([string]::IsNullOrWhiteSpace($env:METALDOCS_CI_DB_PASSWORD)) { "metaldocs_ci_dev" } else { $env:METALDOCS_CI_DB_PASSWORD }

Invoke-DbSqlString -Label "metaldocs_runtime password rotation" `
  -Sql "ALTER ROLE metaldocs_runtime PASSWORD '$($runtimePassword -replace "'", "''")';"
Invoke-DbSqlString -Label "metaldocs_ci password rotation" `
  -Sql "ALTER ROLE metaldocs_ci PASSWORD '$($ciPassword -replace "'", "''")';"
