...
const expectedRoleCapabilityRows = 112 // was 110; +2: approval accountability loop (2026-08-04) granted approval.sla_extend to qms_admin and system_admin — extending one instance's deadline past its route's standard is a governed write, so it is a capability of its own rather than a reuse of the read-only approval.oversee. Prior −2: ADR 0085 stage B retired the document.publish capability (area_admin/qms_admin grants) — publication is the release coordinator's reaction to durable facts, not a human-invoked capability. Prior −3: ROADMAP 3.1a S5 retired template.review grants (approver/qms_admin/system_admin); prior +7: M2b F3 approval.review (approver/area_admin/qms_admin/signer/system_admin) + approval.oversee (qms_admin/system_admin)
