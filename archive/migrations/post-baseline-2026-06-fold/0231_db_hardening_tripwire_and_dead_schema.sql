-- 0231_db_hardening_tripwire_and_dead_schema.sql
-- Base hardening: fail-close the tier-3 capability tripwire and drop the
-- confirmed-orphaned public.* membership stored procs. No behaviour change for
-- any live path.
--
--   #1 enforce_capability_asserted() ELSE → fail-closed.
--      The tripwire maps each guarded table/op to a required capability set in a
--      CASE. The fallthrough was `ELSE RETURN NEW` (silent pass-through). Today
--      every table carrying trg_require_cap_asserted is enumerated in the CASE,
--      so ELSE is unreachable — but a *security* trigger whose default is "allow"
--      inverts the principle: the day someone attaches the trigger to a new table
--      and forgets the CASE branch, that table silently loses tier-3 enforcement.
--      Flip ELSE to RAISE so a mis-wired trigger fails loud (fail-closed). Pure
--      defense-in-depth; no live table hits this branch.
--
--   #2 DROP public.grant_area_membership / public.revoke_area_membership.
--      Orphaned duplicates of the live metaldocs.* stored procs (the schema move
--      left these behind). Zero Go callers — the app calls metaldocs.* (see
--      internal/modules/iam/area_membership/area_membership.go). They also still
--      gate on the phantom capability 'workflow.route.edit', which no longer
--      exists in the registry. Dead, misleading, removed.
--
-- NOTE: metaldocs.document_access_policies was evaluated for removal and
-- deliberately KEPT. Despite zero Go runtime references, ADR 0022 (Phase 5)
-- documents it as the DB anchor (its capability CHECK) for the OpenAPI
-- AccessPolicyItem/AccessPolicyWriteItem vocabulary + FE generated types of a
-- deferred per-document ACL feature. Dropping it would desync the API contract
-- and reverse a recorded decision — out of scope for a dead-code pass.
--
...
-- ── #1 Fail-close the capability tripwire ────────────────────────────────────
...
-- Re-assert ownership + privilege posture (CREATE OR REPLACE preserves them, but
-- keep this idempotent for a clean fresh-replay; mirrors migration 0210).
DO $$
BEGIN
  BEGIN
...
-- ── #2 Drop orphaned public.* membership stored procs ────────────────────────
DROP FUNCTION IF EXISTS public.grant_area_membership(uuid, text, text, text, text);
DROP FUNCTION IF EXISTS public.revoke_area_membership(uuid, text, text, text, text);

INSERT INTO public.schema_migrations (version, description)
VALUES ('0231', 'DB hardening: fail-close enforce_capability_asserted ELSE branch; drop orphaned public.grant/revoke_area_membership duplicate stored procs')
