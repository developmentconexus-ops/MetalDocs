-- 0270_templates_template_tripwire_archive_cap.sql
-- Goal-3 grade-A final-review finding (register APP-09, live-proven 2026-07-03):
-- template archive is bricked at the DB layer. POST /templates/{id}/archive
-- legitimately UPDATEs public.templates_template (stamps archived_at) inside
-- Service.ArchiveTemplate, which asserts CapTemplateArchive
-- ('template.archive') via authz.Require — the correct, minimal capability
-- for that action (internal/modules/templates/application/lifecycle.go:584-596).
-- But enforce_capability_asserted()'s templates_template branch accepts only
-- {template.create, template.edit, template.submit, template.approve,
-- template.publish} — 'template.archive' is missing, so every archive fails
-- with SQLSTATE P0001 and surfaces as a 500 for ALL actors (including
-- system_admin — authz.Require records the asserted cap regardless of role;
-- the trigger checks the recorded set, not the role). Proven live pre-fix:
-- admin create -> archive returned 500
--   "ErrCapabilityNotAsserted: none of {template.create,template.edit,
--    template.submit,template.approve,template.publish} present in
--    asserted_caps on templates_template".
--
-- Same defect class as 0269 (reviewer stage bricked by a missing
-- 'template.review' entry on the sibling templates_template_version arm) and
-- never caught for the same reason: application tests are sqlmock, and no
-- integration test drives ArchiveTemplate against a tripwire-enforced DB.
--
-- Writer inventory of templates_template at HEAD (cap asserted per tx path):
--   CreateTemplateTx (create)                          -> template.create   (listed)
--   CreateNextVersion (create.go:167-174)              -> template.edit     (listed)
--   Approve accept (lifecycle.go:308-318)              -> template.approve  (listed)
--   PublishTemplateVersion (lifecycle.go:491-502)      -> template.publish  (listed)
--   ArchiveTemplate (lifecycle.go:588-591)             -> template.archive  (MISSING -> this fix)
--   (Repository.UpdateTemplate — non-tx variant — has zero production callers.)
--
-- Fix: add 'template.archive' to the templates_template branch ONLY.
-- Convention followed exactly (0269): CREATE OR REPLACE preserving every CASE
-- branch from 0269 byte-for-byte; only the templates_template arm gains
-- 'template.archive'. Ledger insert, BEGIN/COMMIT wrapper. Function-only swap;
-- no trigger attachments change, no backfill. Supersedes 0269 as the latest
...
VALUES ('0270', 'Grade-A review APP-09: add template.archive to the templates_template tripwire cap list — Service.ArchiveTemplate updates the template row under CapTemplateArchive and archive was fail-closed into a 500 (P0001) for every actor. Function-only swap; all other branches preserved from 0269.')
