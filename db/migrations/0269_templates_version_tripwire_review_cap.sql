-- 0269_templates_version_tripwire_review_cap.sql
-- Goal-3 live QA finding (2026-07-03): the reviewer stage of the template
-- workflow is bricked at the DB layer. POST /templates/{id}/versions/{n}/review
-- (accept) legitimately UPDATEs public.templates_template_version
-- (under_review -> approved, sets reviewer_id/reviewed_at) inside
-- Service.Review -> updateVersionWithAuthzAndAudit, which asserts
-- CapTemplateReview ('template.review') via authz.Require — the correct,
-- minimal capability for that action (internal/modules/templates/application/
-- lifecycle.go:166,193). But enforce_capability_asserted()'s
-- templates_template_version branch accepts only
-- {template.create, template.edit, template.submit, template.approve,
-- template.publish} — 'template.review' is missing, so every reviewer accept/
-- reject fails with SQLSTATE P0001 ("none of {...} present in asserted_caps")
-- and surfaces as a 500. Proven live: fresh-bootstrap DB, reviewer-stage
-- template, approver-test (role 'approver', holds template.review) accepting a
-- submitted version.
--
-- Why it was never caught: application-layer tests mock the DB (sqlmock), and
-- the canonical integration lifecycle test (TestLifecycle_NoAutoNextDraft)
-- drives the no-reviewer flow (submit -> approve), which asserts
-- template.approve and passes the existing list. The reviewer arm was never
-- exercised against a real DB with the tripwire installed.
--
-- Fix: add 'template.review' to the templates_template_version branch ONLY.
-- The templates_template branch is untouched — Review never writes the
-- template row (only Approve does, under template.approve, already listed).
-- This widens the tripwire to exactly the capability set whose service paths
-- write version rows; it does not weaken any other table's gate.
--
-- Convention followed exactly (0257/0258/0259): CREATE OR REPLACE preserving
-- every existing CASE branch from 0259 (db/migrations/0259_iam_documents_tripwire.sql)
-- byte-for-byte; only the templates_template_version arm gains
-- 'template.review'. Ledger insert, BEGIN/COMMIT wrapper. No trigger
-- attachments change and no backfill is needed (function-only swap).
...
VALUES ('0269', 'Goal-3 QA: add template.review to the templates_template_version tripwire cap list — Service.Review writes the version row under CapTemplateReview and the reviewer stage was fail-closed into a 500 (P0001) on every accept/reject. Function-only swap; all other branches preserved from 0259.')
