-- 0277_tenants_insert_tripwire_onboard_cap.sql
...
VALUES ('0277', 'M7 F7.2 (ADR 0070): add tenants/INSERT tripwire arm requiring tenant.onboard and attach trg_require_cap_asserted to metaldocs.tenants (BEFORE INSERT) -- the onboarding workflow asserts only tenant.onboard then INSERTs metaldocs.tenants, previously the one privileged provisioning write with no DB tripwire backstop. Additive-only; all other branches preserved from 0275; machine-generated from internal/platform/tripwire.')
