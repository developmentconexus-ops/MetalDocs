...
| F6 | HIGH | shared contract prerequisite | Version chip rendered `PUBLICADO · v{latest_version}` after publish, but `latest_version` advances on the auto-spawned `vN+1` draft created in the same publish transaction — chip lied about what was actually published. Same lie in documents wizard `StepTemplate.tsx:102` and `StepConfirm.tsx:68`. | **Bounded defer** — out of scope for screen-local patch; cross-module fix landed on `bug/templates-version-chip` (commit 57db4e53, PR: `fix(templates): honest version chip + TemplateDTO $ref refactor`). Adds `TemplateDTO.published_version_number` via LEFT JOIN, sets it at both publish paths, renders state-aware chip. |
