...
func TestCapabilityRegistrySize(t *testing.T) {
	...
	if got := len(validCapabilities); got != want {
		t.Fatalf("validCapabilities size = %d, want %d (bump only via ADR; current = 23 base + 4 ADR 0016 view caps + 1 ADR 0019 session.manage + ADR 0022 P1: -2 dead workflow.* +3 promoted doc.publish/doc.obsolete/template.archive + ADR 0022 P8: +4 registered phantoms + ADR 0022 P10: -4 redundant phantoms merged into document.edit/document.view doc.edit_draft/doc.reconstruct/doc.view_published/workflow.instance.cancel + ADR 0022 P12: doc.publish/doc.obsolete/doc.supersede renamed to document.* — count unchanged + ADR 0042 M2/F2.1c: +1 distribution.read + ADR 0043 M3/F3.1: +1 notification.read + SP-1: +2 token.view/token_dictionary.manage + ADR 0051: +1 template.manage + ADR 0069 M6/F6.2: +1 document.review + M7 F7.2: +1 tenant.onboard + M7 F7.3: +2 tenant.export/tenant.erase + M2b F3: +2 approval.review/approval.oversee + ADR 0082 phase c / unit 3.1a S5: -1 template.review retired with the legacy template-approval path + ADR 0085 stage B: -1 document.publish retired — publication is the release coordinator's reaction to durable facts, not a human-invoked capability + approval.sla_extend (approval accountability loop, 2026-08-04): a route's deadline is the standard; extending it for ONE instance is a governed exception and a WRITE. approval.oversee could not be reused — it is declared read-only oversight, and authorizing a write with it would erode that contract silently.)", got, want)
	}
