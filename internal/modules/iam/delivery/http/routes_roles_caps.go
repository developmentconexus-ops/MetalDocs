...
func (h *RolesCapsHandler) listRoleCapabilities(w http.ResponseWriter, r *http.Request) {
	...
	if h.roleCaps == nil {
		problem.Respond(w, r, problem.New(http.StatusNotImplemented, problem.CodeInternalUnknown, "role_capabilities repository is not configured"))
		return
	}
	links, err := h.roleCaps.ListRoleCapabilities(r.Context())
	if err != nil {
		problem.Respond(w, r, problem.New(http.StatusInternalServerError, problem.CodeInternalUnknown, "Failed to list role capabilities"))
		return
	}
