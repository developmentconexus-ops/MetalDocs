...
// three janitor jobs (stuck-instance watchdog, idempotency janitor, audit
// integrity validator). Both metaldocs-api and metaldocs-jobs define these
// periodic jobs on their River client config so that whichever process wins
// leader election is the one that enqueues them (River only enqueues periodic
// jobs from the elected leader's own Config.PeriodicJobs). Only
// metaldocs-jobs subscribes the "maintenance" queue and registers the actual
// Workers — this package holds ONLY the schedule/args wiring, no worker
// construction, no DB (ADR 0067).
package maintenance
...
// It must be included in the Config.PeriodicJobs of every River client that
