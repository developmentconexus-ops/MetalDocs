...
// cancelInTx is the tx-scoped core of CancelInstance: load + legality guard
// + authz, cancelling the instance and its stages, reverting the document to
// draft, and emitting the governance event. Returns the document ID.
func (s *CancelService) cancelInTx(ctx context.Context, tx *sql.Tx, in CancelInput) (string, error) {
	ctx = authz.WithCapCache(ctx)

	docID, areaCode, err := s.loadCancelTarget(ctx, tx, in)
	if err != nil {
		return "", err
	}
	...
	now := s.clock.Now()
	if err := s.applyCancelUpdate(ctx, tx, in, docID, now); err != nil {
		return "", err
	}

	if err := s.emitCancelEvent(ctx, tx, in, docID, now); err != nil {
		return "", err
	}
...
// loadCancelTarget loads the approval instance to get its document ID,
// verifies it is not already terminal, and fetches the document's area_code
// for the authz check. FOR UPDATE locks the document row to prevent
// concurrent area_code changes between authz decision and status update.
func (s *CancelService) loadCancelTarget(ctx context.Context, tx *sql.Tx, in CancelInput) (docID string, areaCode sql.NullString, err error) {
	inst, err := s.repo.LoadInstance(ctx, tx, in.TenantID, in.InstanceID)
	if err != nil {
		return "", sql.NullString{}, fmt.Errorf("cancel: load instance: %w", err)
	}
	if inst == nil {
		return "", sql.NullString{}, infrastructure.ErrNoActiveInstance
	}
	if inst.Status != domain.InstanceInProgress {
		return "", sql.NullString{}, infrastructure.ErrInstanceCompleted
	}
	...
	if err := tx.QueryRowContext(ctx,
		...
		return "", sql.NullString{}, fmt.Errorf("cancel: fetch area_code: %w", err)
	}
	return docID, areaCode, nil
}

// applyCancelUpdate sets the cancel GUC, marks the instance and its
// active/pending stages cancelled, and reverts the document to draft.
func (s *CancelService) applyCancelUpdate(ctx context.Context, tx *sql.Tx, in CancelInput, docID string, now time.Time) error {
...
// emitCancelEvent emits the governance event recording the cancellation.
func (s *CancelService) emitCancelEvent(ctx context.Context, tx *sql.Tx, in CancelInput, docID string, now time.Time) error {
