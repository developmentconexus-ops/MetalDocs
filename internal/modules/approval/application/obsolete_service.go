...
func (s *ObsoleteService) MarkObsolete(ctx context.Context, runner db.TxRunner, req MarkObsoleteRequest) (MarkObsoleteResult, error) {
	...
	err := runner.Do(ctx, func(tx *sql.Tx) error {
		res, err := s.markObsoleteTx(ctx, tx, req)
		if err != nil {
			return err
		}
		result = res
...
// markObsoleteTx is the tx-scoped core of MarkObsolete: lock + legality
// guard + authz, the OCC status transition, cancelling any in-progress
// approval instance, and emitting the governance/lifecycle events.
func (s *ObsoleteService) markObsoleteTx(ctx context.Context, tx *sql.Tx, req MarkObsoleteRequest) (MarkObsoleteResult, error) {
	ctx = authz.WithCapCache(ctx)

	// Step 2: fetch current status + revision_version under a row-level lock.
	priorStatus, areaCode, err := s.lockDocumentForObsolete(ctx, tx, req)
	if err != nil {
		return MarkObsoleteResult{}, err
	}
	...
	if err := s.applyObsoleteUpdate(ctx, tx, req, priorStatus); err != nil {
		return MarkObsoleteResult{}, err
	}

	// Step 5: cancel any in-progress approval instance (no error if none exist).
	if err := s.cancelInProgressApprovalInstance(ctx, tx, req); err != nil {
		return MarkObsoleteResult{}, err
	}

	// Step 6: emit "document_obsoleted" governance + lifecycle events.
	if err := s.emitObsoleteEvents(ctx, tx, req, priorStatus); err != nil {
		return MarkObsoleteResult{}, err
	}
...
// lockDocumentForObsolete row-locks the target document and returns its
// current status and process area code.
func (s *ObsoleteService) lockDocumentForObsolete(ctx context.Context, tx *sql.Tx, req MarkObsoleteRequest) (priorStatus, areaCode string, err error) {
	...
	return priorStatus, areaCode, nil
}

// applyObsoleteUpdate runs the OCC CAS UPDATE transitioning the document to obsolete.
func (s *ObsoleteService) applyObsoleteUpdate(ctx context.Context, tx *sql.Tx, req MarkObsoleteRequest, priorStatus string) error {
...
// cancelInProgressApprovalInstance cancels any in-progress approval instance
// for the document being obsoleted. A no-op (no error) when none exist.
func (s *ObsoleteService) cancelInProgressApprovalInstance(ctx context.Context, tx *sql.Tx, req MarkObsoleteRequest) error {
...
// emitObsoleteEvents emits the "document_obsoleted" governance event and,
// when a lifecycle enqueuer is wired, the additive F3.3 reader lifecycle event.
func (s *ObsoleteService) emitObsoleteEvents(ctx context.Context, tx *sql.Tx, req MarkObsoleteRequest, priorStatus string) error {
