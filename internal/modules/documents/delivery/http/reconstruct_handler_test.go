...
func newReconstructReq(docID string) *http.Request {
	req := httptest.NewRequest(http.MethodPost, "/api/v1/documents/"+docID+"/reconstruct", nil)
...
func TestReconstructHandler_CapabilityDeniedReturns403(t *testing.T) {
	h := NewReconstructHandler(fakeReconstructService{err: authz.ErrCapDenied{Capability: "document.edit", AreaCode: "AREA9", ActorID: "user-1"}})
