...
func TestListDocumentsPaginated_StatusFilter(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{
		...
		"effective_from", "effective_to", "review_due_at", "last_reviewed_at", "review_surfaced_at", "values_frozen_at", "total_count",
	}).
		AddRow("doc-1", "tenant-1", "tpl-1", "Doc B", "under_review", []byte(`{}`), "", "", nil, now, now, "user-1", nil, "", "prof", "area", int64(1), int64(0), nil, nil, nil, nil, nil, nil, int64(2)).
		AddRow("doc-2", "tenant-1", "tpl-1", "Doc C", "under_review", []byte(`{}`), "", "", nil, now, now, "user-2", nil, "", "prof", "area", int64(1), int64(0), nil, nil, nil, nil, nil, nil, int64(2))
...
func TestListDocumentsPaginated_CursorKeyset(t *testing.T) {
	...
	makeRows := func(n int) *sqlmock.Rows {
		...
		for i := 0; i < n; i++ {
			rows.AddRow("doc-id", "tenant-1", "tpl-1", "Doc", "draft", []byte(`{}`), "", "", nil, now, now, "user-1", nil, "", "prof", "area", int64(1), int64(0), nil, nil, nil, nil, nil, nil, int64(25))
		}
		return rows
	}
