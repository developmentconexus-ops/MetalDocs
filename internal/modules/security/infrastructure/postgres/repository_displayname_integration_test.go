...
// openLiveSecurityDB returns a reset-safe leased clone from the canonical
// testdb factory (ADR 0034), NOT a raw sql.Open against the shared dev
// database. The prior body read DATABASE_URL directly and skipped-as-green
// when unset — the cluster-4 framework bypass the 4.6 exit gate caught
// (§11.28). testdb.Open owns t.Helper, the leased-DB reset, cleanup, and a
// fail-loud ping.
