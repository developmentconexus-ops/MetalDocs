...
// WriteTenantObject writes server-generated bytes directly to a tenant-scoped
// key (PutObject), tenant-prefix guarded like Copy/PresignPut. Distinct from
// the presign->Confirm upload path: there is no untrusted-producer hashing
// step here because the content originates on the server itself (e.g. the
// M7 F7.3 tenant export artifact) rather than a browser upload, so no
// "verified pointer" needs to be established against a client-declared hash.
func (s *VerifiedStore) WriteTenantObject(ctx context.Context, tenantID, key string, data []byte, contentType string) error {
	if err := s.assertTenant(tenantID, key); err != nil {
		return err
	}
	...
	if err != nil {
		return fmt.Errorf("objectstore: write tenant object: %w", err)
	}
	return nil
}
